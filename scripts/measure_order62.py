"""Reproduce matched Shock Disembark submission and restore costs.
...
WORKLOAD_ID = "order62-shock-placement-restore-v1"
...
def main() -> None:
    ...
    helpers = importlib.import_module("tests.order62_shock_helpers")
    ...
    for _index in range(7):
        ...
        session = helpers.shock_session(enemy_x=30)
        ...
        request, proposal = helpers.shock_proposal(session)
        start = time.perf_counter()
        outcome = session.submit_parameterized_payload(
            request_id=request.request_id, result_id="order62:place", payload=proposal.to_payload()
        )
    ...
    report = {
        ...
        "timing_boundary": (
            "One Shock setup, no post-engagement, plus exact restore; "
