...
def main() -> None:
    ...
    for _ in range(args.samples):
        ...
        lifecycle, units = charge_lifecycle(
            ...
            game_id="order46-charge-benchmark",
        ...
        assert request is not None
        if request.decision_type == "select_charge_targets":
            option = next(
                o
        ...
        paths = tuple(
            (
                ...
                (
                    m.pose,
                    Pose.at(m.pose.position.x + 1.5, m.pose.position.y),
                    Pose.at(m.pose.position.x + 3, m.pose.position.y),
                ),
            )
    ...
    report = {
        "workload_id": "order46-charge-slice-v1",
        ...
        "hashes": {
            ...
            for name in (
                ...
                "scripts/measure_charge_distance.py",
            )
        },
        "samples": rows,
