"""Measure Order 68 pre-game roster validation on one provisional host."""
...
def main() -> None:
    ...
    enhancement = EnhancementDefinition(
        enhancement_id="order68-benchmark-enhancement",
        name="Benchmark Enhancement",
        source_id="order68:benchmark-enhancement",
        points=10,
    )
    catalog = replace(
        catalog,
        ...
        detachments=tuple(
            replace(row, enhancement_ids=(enhancement.enhancement_id,))
            for row in catalog.detachments
        ),
    )
    rows = []
    for count in (2, 4, 20):
        request = ArmyMusterRequest(
            enhancement_assignments=(
                EnhancementAssignment.from_payload(
                    {
                        "enhancement_id": enhancement.enhancement_id,
                        "target_unit_selection_id": "unit-0",
                        "source_id": "order68:benchmark-enhancement",
            ...
            warlord_selection=WarlordSelection.from_payload(
                {
                    "unit_selection_id": "unit-0",
                    "source_id": "order68:benchmark-warlord",
                    "model_profile_id": "core-character-leader",
                    "model_index": 1,
                }
            ),
    ...
    output = {
        "workload": "order68-roster-validation-v1",
        ...
        "hashes": {
            ...
            for name in ("scripts/measure_order68.py", "uv.lock")
        },
        "rows": rows,
    }
