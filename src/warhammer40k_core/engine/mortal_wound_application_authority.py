...
from warhammer40k_core.engine.mortal_wound_binding_source import validate_initial_binding_source
...
def validate_mortal_wound_application_authority_closure(
    *,
    ...
    for application_id, (started_event, authority) in inventory.items():
        ...
        completion_events = completion_events_for_authority(
            authority=authority,
            event_records=event_records,
        )
    ...
    for completion_event in _supported_completion_events(event_records):
        matching_application_ids = tuple(
            ...
            if completion_event
            in completion_events_for_authority(
                authority=authority,
...
def completion_events_for_authority(
    *,
    authority: MortalWoundApplicationAuthority,
    event_records: tuple[EventRecord, ...],
) -> tuple[EventRecord, ...]:
    ...
    if source_kind == "hazardous":
        return tuple(
            ...
            and event.payload.get("game_id") == authority.game_id
            and event.payload.get("application_id") == authority.application_id
            and event.payload.get("source_rule_id") == authority.source_rule_id
            and event.payload.get("source_context") == authority.source_context
            and event.payload.get("target_unit_instance_id") == authority.target_unit_instance_id
            and _completion_application_matches(authority=authority, payload=event.payload)
        )
