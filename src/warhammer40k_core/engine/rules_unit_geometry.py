...
def geometry_models_for_rules_unit(
    *, state: GameState, unit_instance_id: str
    ...
    for component in rules_unit.components:
        unit_placement = battlefield.unit_placement_or_none(component.unit.unit_instance_id)
        if unit_placement is None:
            if any(model.is_alive for model in component.unit.own_models):
                raise GameLifecycleError("Rules-unit geometry requires placed alive units.")
            continue
...
def present_geometry_models_for_rules_unit(
    *,
    state: GameState,
    unit_instance_id: str,
) -> tuple[GeometryModel, ...]:
    """Geometry for rules measurements, including fixed Fight On Death models."""
    battlefield = state.battlefield_state
    if battlefield is None:
        raise GameLifecycleError("Present rules-unit geometry requires battlefield state.")
    rules_unit = rules_unit_view_by_id(state=state, unit_instance_id=unit_instance_id)
    present_ids = {
    ...
    if not present_ids:
        return ()
    return geometry_models_for_rules_unit(state=state, unit_instance_id=rules_unit.unit_instance_id)
