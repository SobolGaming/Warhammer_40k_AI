...
# fmt: off
if TYPE_CHECKING:
    from warhammer40k_core.engine.phases.movement_action_decisions import _movement_action_options_for_selected_unit
    from warhammer40k_core.engine.phases.movement_validation import _movement_unit_candidates
        ...
        *,
        ...
        if active_selection is not None:
            if movement_state.pending_desperate_escape_battle_shock_continuation is not None:
                continuation_status = _resume_desperate_escape_battle_shock_continuation(
                    state=state,
                    decisions=decisions,
        ...
        return LifecycleStatus.waiting_for_decision(
            ...
            payload={
                ...
                "battle_round": state.battle_round,
                "active_player_id": _active_player_id(state),
                "legal_unit_count": len(unit_candidates),
            },
        )

    def invalid_movement_action_selection_status(
        self,
        *,
        state: GameState,
        request: DecisionRequest,
        ...
        if request.decision_type != SELECT_MOVEMENT_ACTION_DECISION_TYPE:
            raise GameLifecycleError("Movement action prevalidation received wrong request.")
        ...
        current_options = _movement_action_options_for_selected_unit(
            state=state,
            movement_state=movement_state,
            unit_instance_id=unit_instance_id,
            ruleset_descriptor=_ruleset_descriptor_for_handler(self),
    ...
    def invalid_proposal_submission_status(
        ...
        *,
        ...
        if request.decision_type == MOVEMENT_PROPOSAL_DECISION_TYPE:
            ...
            if (
                proposal_request.proposal_kind is ProposalKind.NORMAL_MOVE
                and _unit_already_made_normal_move_this_phase(
                    state=state,
                    player_id=_active_player_id(state),
        ...
        if request.decision_type == PLACEMENT_PROPOSAL_DECISION_TYPE:
            from warhammer40k_core.engine.phases.movement_placement_proposals import (
                invalid_placement_proposal_submission_status,
            )

            return invalid_placement_proposal_submission_status(
                state=state,
                request=request,
                result=result,
                decisions=decisions,
    ...
    def apply_decision(
        ...
        *,
        ...
        if result.decision_type == DICE_REROLL_DECISION_TYPE:
            ...
            if is_desperate_escape_battle_shock_reroll_request(reroll_record.request):
                return _apply_desperate_escape_battle_shock_reroll_decision(
                    state=state,
                    result=result,
                    decisions=decisions,
                    ruleset_descriptor=_ruleset_descriptor_for_handler(self),
        ...
        legal_candidates = _movement_unit_candidates(
            state=state,
            movement_state=movement_state,
        )
        ...
        expected_option = _movement_unit_options(candidates=matching_candidates)[0]
        if expected_option.payload != result.payload:
            raise GameLifecycleError("Movement unit selection payload drift.")
        ...
        decisions.event_log.append(
            "movement_unit_selected",
            {
                ...
                "phase": BattlePhase.MOVEMENT.value,
                "unit_instance_id": unit_instance_id,
                "unit_location": matching_candidates[0].location.value,
                "source_rule_id": core_movement_phase_2026_08.MOVE_UNITS_STEP_SOURCE_ID,
                "request_id": result.request_id,
                "result_id": result.result_id,
