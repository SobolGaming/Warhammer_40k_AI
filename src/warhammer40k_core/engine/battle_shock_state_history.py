...
_FORBIDDEN_ATTACHED_SPLIT_EVENT_TYPES = frozenset(
    {
...
def _replay_battle_shock_state_until(
    *,
    ...
    for event_index, event in enumerate(event_records[:stop_index]):
        if event.event_type in _FORBIDDEN_ATTACHED_SPLIT_EVENT_TYPES:
...
def _apply_resolved_event(
    *,
    ...
    expected_state_update = (
        BATTLE_SHOCK_STATE_ALREADY if not missing_target_ids else BATTLE_SHOCK_STATE_RECORDED
    )
