...
def _synthetic_pool_for_gathered_group(
    *,
    ...
    selection_context = base_pool.weapon_selection_context
    selected_ids = base_pool.selected_weapon_ability_ids
    ...
    if len(gathered_group.pool_indices) > 1:
        ...
        if selection_context is not None:
            from warhammer40k_core.engine.weapon_selection_context import (
                reidentify_selection_context,
            )

            selection_context, selected_ids = reidentify_selection_context(
                selection_context,
                weapon_instance_id=weapon_instance_id,
                profile_id=weapon_profile_id,
                profile_name=weapon_profile.name,
