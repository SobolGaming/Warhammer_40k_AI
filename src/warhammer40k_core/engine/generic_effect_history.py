"""Reconstruct selected generic effects at an authenticated historical boundary."""
...
class HistoricalEffectAuthority:
    game_id: str
    ...
    phase: BattlePhase
    rules_unit_at_event: Callable[[str, int], RulesUnitView]
    ...
    def army_for_player(self, player_id: str) -> ArmyDefinition:
        ...
        if len(matches) != 1:
            raise GameLifecycleError("Historical effect army authority is ambiguous.")
        return matches[0]


@dataclass(frozen=True, slots=True)
class _HistoricalDurationContext:
    state: HistoricalEffectAuthority
    player_id: str
    battle_round: int
...
def historical_generic_effect_inventory(
    *,
    historical: HistoricalEffectAuthority,
    runtime_content_bundle: RuntimeContentBundle,
    matches_payload: Callable[[dict[str, JsonValue]], bool],
    ...
    if not any(
        ...
        and (
            (event.event_type == "rule_execution_effect_applied" and matches_payload(event.payload))
            or any(
                matches_payload(_object(effect.effect_payload, "installed effect"))
    ...
    for event_index, event in enumerate(prior_events):
        ...
        if isinstance(event.payload, dict):
            for installed in _installed_effects(event.event_type, event.payload):
                ...
                if not matches_payload(payload):
                    continue
                ...
                if family == "detachment":
                    ...
                    if (
                        ...
                        raise GameLifecycleError(
                            "Historical generic effect installation lacks execution authority."
                        )
                    ...
                    raise GameLifecycleError(
                        "Historical generic effect installation family drifted."
                    )
        ...
        if effect is not None:
            ...
            if previous is not None and not _expired_at_context(
                ...
                raise GameLifecycleError("Historical generic effect creation is duplicated.")
...
def _effect_from_execution(
    *,
    ...
    if len(clauses) != 1:
        raise GameLifecycleError("Historical generic effect clause is not loaded.")
    clause = clauses[0]
    index = payload.get("effect_index")
    if type(index) is not int or not 0 <= index < len(clause.effects):
        raise GameLifecycleError("Historical generic effect effect slot drifted.")
    ...
    if (
        ...
        or context.battle_round > historical.battle_round
        or context.player_id not in historical.player_ids
        or payload.get("target_unit_instance_ids") != list(target_ids)
    ):
        raise GameLifecycleError("Historical generic effect creation context drifted.")
    _validate_creation_clock(historical, context, creation_index)
    _validate_source_unit(historical, context, creation_index)
    ...
    source_units = (
        ...
        if context.source_unit_instance_id is None
            ...
            component.unit
            for component in historical.rules_unit_at_event(
                context.source_unit_instance_id, creation_index
            ).components
        )
    )
...
def _validate_source_unit(
    historical: HistoricalEffectAuthority, context: RuleExecutionContext, creation_index: int
) -> None:
    source_id = context.source_unit_instance_id
    if source_id is None:
        if context.source_model_instance_id is not None:
            raise GameLifecycleError("Historical generic effect source model lacks a unit.")
        return
    source = historical.rules_unit_at_event(source_id, creation_index)
    if source.owner_player_id != context.player_id:
        raise GameLifecycleError("Historical generic effect source owner drifted.")
    if context.source_model_instance_id is not None and context.source_model_instance_id not in {
        ...
        raise GameLifecycleError("Historical generic effect source model drifted.")


def _validate_recorded_effect_copies(
    effect: PersistingEffect, historical: HistoricalEffectAuthority, creation_index: int
) -> None:
    ...
    def visit(value: JsonValue) -> None:
        if isinstance(value, list):
            ...
            if value.get("effect_id") == effect.effect_id and "expiration" in value:
                ...
                if recorded != effect:
                    raise GameLifecycleError("Historical generic effect effect evidence drifted.")
            else:
...
def _expired_at_test(expiration: EffectExpiration, historical: HistoricalEffectAuthority) -> bool:
    return _expired_at_context(
        expiration,
        _HistoricalDurationContext(
            state=historical,
            player_id=historical.player_id,
            battle_round=historical.battle_round,
...
def _identifier(value: JsonValue, context: str) -> str:
    if type(value) is not str or not value:
        raise GameLifecycleError(f"Historical generic effect {context} is invalid.")
    return value


def _object(value: JsonValue, context: str) -> dict[str, JsonValue]:
    if not isinstance(value, dict):
        raise GameLifecycleError(f"Historical generic effect {context} must be an object.")
    return value
...
def _validate_creation_clock(
    ...
    if (
        ...
        raise GameLifecycleError("Historical generic effect creation boundary drifted.")
...
def _attack_expiration_ids(
    ...
    if not isinstance(observed, list) or any(type(value) is not str for value in observed):
        raise GameLifecycleError(
            "Historical generic effect attack expiration inventory is invalid."
        )
    if set(expected) != {
        value for value in observed if isinstance(value, str) and value in effects
    }:
        raise GameLifecycleError("Historical generic effect attack expiration inventory drifted.")
    anchors = tuple(
        event
    ...
    if payload.get("game_id") != historical.game_id or len(anchors) != 1:
        raise GameLifecycleError(
            "Historical generic effect attack expiration lacks completion authority."
        )
    return expected
