...
class CatalogReturnOnDeathRuntime:
    ...
    def phase_end_bindings(self) -> tuple[TurnEndHookBinding, ...]:
        ...
        return (
            TurnEndHookBinding(
                ...
                candidate_handler=self.phase_end_candidates,
            ),
        )
    ...
    def unit_destroyed_bindings(self) -> tuple[UnitDestroyedHookBinding, ...]:
        ...
        return (
            UnitDestroyedHookBinding(
                ...
                maintenance_handler=self.unit_destroyed_handler,
                model_maintenance_handler=self.record_model_destroyed,
            ),
        )
    ...
    def unit_destroyed_handler(self, context: UnitDestroyedContext) -> None:
        ...
        for record in _return_on_death_records(index):
            ...
            for clause in catalog_rule_clauses_from_record(record):
                ...
                if pending is None:
                    continue
                if record_pending_return_on_death(
                    pending=pending,
    ...
    def pending_captures_for(
        self,
        context: PhaseEndObjectiveControlContext,
    ) -> tuple[tuple[str, PendingReturnOnDeath], ...]:
        ...
        return self.captures_for_events(
            state=context.state,
            phase=context.completed_phase,
            events=_model_destroyed_events_for_phase(context),
        )

    def captures_for_events(
        self,
        *,
        state: GameState,
        phase: BattlePhase,
        events: tuple[tuple[str, dict[str, JsonValue]], ...],
    ) -> tuple[tuple[str, PendingReturnOnDeath], ...]:
        captures: list[tuple[str, PendingReturnOnDeath]] = []
        consumed_keys = set(state.return_on_death_consumed_keys)
        for event_id, payload in events:
            ...
            for record in _return_on_death_records(index):
                for clause in catalog_rule_clauses_from_record(record):
                    ...
                    if pending is None:
                        continue
                    if pending.consumed_key() in consumed_keys:
                        continue
                    consumed_keys.add(pending.consumed_key())
                    captures.append((event_id, pending))
        return tuple(captures)

    def record_model_destroyed(self, context: ModelDestroyedContext) -> None:
        if not isinstance(context.event.payload, dict):
            raise GameLifecycleError("Return-on-death model occurrence requires an object.")
        for event_id, pending in self.captures_for_events(
            state=context.state,
            phase=context.phase,
            events=((context.event.event_id, context.event.payload),),
        ):
            record_pending_return_on_death(
                pending=pending,
                event_log=context.decisions.event_log,
                state=context.state,
    ...
    def phase_end_candidates(
        self, context: TurnEndRequestContext
    ) -> tuple[TimingRuleCandidate, ...]:
        from warhammer40k_core.engine.catalog_return_on_death_sequencing import (
            candidates_for_captures,
        )

        captures = self.pending_captures_for(
            PhaseEndObjectiveControlContext(
                state=context.state,
                event_log=context.decisions.event_log,
        ...
        return candidates_for_captures(context, captures)
...
def record_pending_return_on_death(
    *,
