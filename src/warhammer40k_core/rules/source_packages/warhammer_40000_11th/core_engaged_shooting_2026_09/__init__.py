"""Hash-pinned Core 17.03 engaged Monster/Vehicle targeting for P17."""
...
EXPECTED_ARTIFACT_SHA256: Final = "0dfed05d9cf2d7b842f66b9e7375a636e6b45ed73a09f8831521dc43e38692ab"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-engaged-shooting"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-21"
ENGAGED_TARGET_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:engaged-monster-vehicle-target"


class EngagedShootingSourceError(ValueError):
    """The reviewed EngagedShooting source identity or provenance has drifted."""


class EngagedShootingSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class EngagedShootingSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[EngagedShootingSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> EngagedShootingSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise EngagedShootingSourceError(
            "EngagedShooting source bytes drifted from their reviewed pin."
        )
    try:
        artifact = msgspec.json.decode(raw, type=EngagedShootingSourceArtifact)
    except msgspec.DecodeError as exc:
        raise EngagedShootingSourceError("EngagedShooting source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-engaged-shooting-source-v1"
        or artifact.source_package_id != SOURCE_PACKAGE_ID
        or artifact.source_version != SOURCE_VERSION
        or tuple(rule.source_id for rule in artifact.rules) != (ENGAGED_TARGET_SOURCE_ID,)
    ):
        raise EngagedShootingSourceError("EngagedShooting source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise EngagedShootingSourceError("EngagedShooting source transcription hash drifted.")
        if (
            ...
            raise EngagedShootingSourceError(
                "EngagedShooting source execution evidence is incomplete."
            )
    return artifact
...
def source_rules() -> tuple[EngagedShootingSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p17"),
                title="Reviewed maintained App-data EngagedShooting",
                source_texts=tuple(
