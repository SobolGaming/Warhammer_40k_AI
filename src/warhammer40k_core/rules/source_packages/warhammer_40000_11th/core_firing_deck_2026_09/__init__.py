"""Hash-pinned Core 24.14 FiringDeck rule for P24B."""
...
EXPECTED_ARTIFACT_SHA256: Final = "8ecab220d6c9c8491f438b5c702df77ad21ea63d8cedb6fd49ecafac85f93de4"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-firing-deck"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-20"
FIRING_DECK_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:firing_deck"


class FiringDeckSourceError(ValueError):
    """The reviewed FiringDeck source identity or provenance has drifted."""


class FiringDeckSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class FiringDeckSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[FiringDeckSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> FiringDeckSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise FiringDeckSourceError("FiringDeck source bytes drifted from their reviewed pin.")
    try:
        artifact = msgspec.json.decode(raw, type=FiringDeckSourceArtifact)
    except msgspec.DecodeError as exc:
        raise FiringDeckSourceError("FiringDeck source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-firing-deck-source-v1"
        or artifact.source_package_id != SOURCE_PACKAGE_ID
        or artifact.source_version != SOURCE_VERSION
        or tuple(rule.source_id for rule in artifact.rules) != (FIRING_DECK_SOURCE_ID,)
    ):
        raise FiringDeckSourceError("FiringDeck source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise FiringDeckSourceError("FiringDeck source transcription hash drifted.")
        if (
            ...
            raise FiringDeckSourceError("FiringDeck source execution evidence is incomplete.")
    return artifact
...
def source_rules() -> tuple[FiringDeckSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p24b"),
                title="Reviewed maintained App-data FiringDeck",
                source_texts=tuple(
