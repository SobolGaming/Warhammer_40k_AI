"""Hash-pinned Core 01.05.07/08 dice result semantics for P01G."""
...
EXPECTED_ARTIFACT_SHA256: Final = "b35120d15c542113ba3ab11eede412e95966ae2a7202f41eb19f4503f2ebd3c6"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-dice-results"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-24"
HIGHEST_LOWEST_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:highest-lowest"
TREATED_AS_SET_TO_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:treated-as-set-to"


DEFAULT_CRITICAL_HIT_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:default-critical-hit"
DEFAULT_CRITICAL_WOUND_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:default-critical-wound"


class DiceResultsSourceError(ValueError):
    """The reviewed DiceResults source identity or provenance has drifted."""


class DiceResultsSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class DiceResultsSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[DiceResultsSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> DiceResultsSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise DiceResultsSourceError("DiceResults source bytes drifted from their reviewed pin.")
    try:
        artifact = msgspec.json.decode(raw, type=DiceResultsSourceArtifact)
    except msgspec.DecodeError as exc:
        raise DiceResultsSourceError("DiceResults source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-dice-results-source-v1"
        ...
        != (
            ...
            TREATED_AS_SET_TO_SOURCE_ID,
            DEFAULT_CRITICAL_HIT_SOURCE_ID,
            DEFAULT_CRITICAL_WOUND_SOURCE_ID,
        )
    ):
        raise DiceResultsSourceError("DiceResults source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise DiceResultsSourceError("DiceResults source transcription hash drifted.")
        if (
            ...
            raise DiceResultsSourceError("DiceResults source execution evidence is incomplete.")
    return artifact
...
def source_rules() -> tuple[DiceResultsSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p01g"),
                title="Reviewed maintained App-data DiceResults",
                source_texts=tuple(
