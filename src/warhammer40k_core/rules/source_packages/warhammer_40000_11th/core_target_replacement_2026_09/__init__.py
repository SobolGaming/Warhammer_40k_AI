"""Reviewed, immutable target-replacement source authority."""
...
EXPECTED_ARTIFACT_SHA256: Final = "0ff886fafa5d120ca406986f33e9932b762e1cf9b4ef0beccce66df000fd32ed"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-target-replacement"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-12"
TARGET_REPLACEMENT_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:target-no-longer-eligible-or-viable"


class TargetReplacementSourceError(ValueError):
    """The reviewed target-replacement source or its provenance has drifted."""


class TargetReplacementSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class TargetReplacementSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[TargetReplacementSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> TargetReplacementSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise TargetReplacementSourceError(
            "Target replacement source bytes drifted from their reviewed pin."
        )
    try:
        artifact = msgspec.json.decode(raw, type=TargetReplacementSourceArtifact)
    except msgspec.DecodeError as exc:
        raise TargetReplacementSourceError("Target replacement source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-target-replacement-source-v1"
        or artifact.source_package_id != SOURCE_PACKAGE_ID
        or artifact.source_version != SOURCE_VERSION
        or tuple(rule.source_id for rule in artifact.rules) != (TARGET_REPLACEMENT_SOURCE_ID,)
    ):
        raise TargetReplacementSourceError("Target replacement source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise TargetReplacementSourceError("Target replacement transcription hash drifted.")
        if (
            ...
            raise TargetReplacementSourceError(
                "Target replacement execution evidence is incomplete."
            )
    return artifact
...
def source_rules() -> tuple[TargetReplacementSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    return RuleSourcePackage(
        source_catalog=SourceCatalog(
            ...
            documents=(
                SourceDocument(
                    document_id=SourceDocumentId(package_id=package_id, document_id="p04"),
                    title="Reviewed maintained App-data rules target-replacement",
                    source_texts=tuple(
