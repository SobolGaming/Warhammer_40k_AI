...
from ._artifacts import (
    LuciusRuleIrArtifactError,
    LuciusRuleIrPackageArtifact,
    lucius_rule_ir_package_artifact_from_json_bytes,
)
...
def _load_artifact() -> LuciusRuleIrPackageArtifact:
    try:
        ...
        raise LuciusRuleIrArtifactError(
            "Lucius generated RuleIR artifact could not be loaded."
        ) from exc
    return lucius_rule_ir_package_artifact_from_json_bytes(raw)


_ARTIFACT: Final[LuciusRuleIrPackageArtifact] = _load_artifact()
...
REVIEW_ROW_ID: Final = _ARTIFACT.review_row_id
REVIEW_TREATMENT: Final = _ARTIFACT.review_treatment
...
def validate_generated_artifact_bytes(raw: bytes) -> None:
    lucius_rule_ir_package_artifact_from_json_bytes(raw)
