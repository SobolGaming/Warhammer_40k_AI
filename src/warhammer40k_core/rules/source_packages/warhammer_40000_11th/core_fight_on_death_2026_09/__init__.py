"""Hash-pinned 05.04.05 and retained-presence FAQ for P05B."""
...
EXPECTED_ARTIFACT_SHA256: Final = "2465e094b0c30db3321924367db7a995660e1524c7fde5876482397bd69acd9d"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-fight-on-death"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-07"
RETAINED_PRESENCE_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:retained-presence"
FIGHT_ON_DEATH_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:fight-on-death"


class FightOnDeathSourceError(ValueError):
    """The reviewed Fight On Death source identity or provenance has drifted."""


class FightOnDeathSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class FightOnDeathSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[FightOnDeathSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> FightOnDeathSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise FightOnDeathSourceError(
            "Fight On Death source bytes drifted from their reviewed pin."
        )
    try:
        artifact = msgspec.json.decode(raw, type=FightOnDeathSourceArtifact)
    except msgspec.DecodeError as exc:
        raise FightOnDeathSourceError("Fight On Death source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-fight-on-death-source-v1"
        ...
        != (RETAINED_PRESENCE_SOURCE_ID, FIGHT_ON_DEATH_SOURCE_ID)
    ):
        raise FightOnDeathSourceError("Fight On Death source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise FightOnDeathSourceError("Fight On Death source transcription hash drifted.")
        if (
            ...
            raise FightOnDeathSourceError("Fight On Death source execution evidence is incomplete.")
    return artifact
...
def source_rules() -> tuple[FightOnDeathSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p05b"),
                title="Reviewed maintained App-data Fight On Death",
                source_texts=tuple(
