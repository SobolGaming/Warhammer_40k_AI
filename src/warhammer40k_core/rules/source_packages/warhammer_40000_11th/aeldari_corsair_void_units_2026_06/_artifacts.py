...
CORSAIR_VOID_UNITS_RULE_IR_ARTIFACT_SCHEMA = "core-v2-corsair-void-units-datasheet-rule-ir-v1"
EXPECTED_SOURCE_PACKAGE_ID = "gw-11e-aeldari-corsair-void-units-datasheets-2026-06-14"
EXPECTED_DATASHEETS = {
...
class CorsairVoidUnitsRuleIrArtifactError(ValueError):
    """Raised when the generated Corsair void-unit RuleIR artifact is invalid or stale."""


class _CorsairVoidUnitsRuleIrRecordArtifact(
    msgspec.Struct,
    frozen=True,
    ...
    def validated_rule_ir(self, *, source_row_id: str, source_package_id: str) -> RuleIR:
        ...
        try:
            ...
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit generated RuleIR payload is invalid."
            ) from exc
        if cast(dict[str, object], rule_ir.to_payload()) != self.rule_ir:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit generated RuleIR payload has unsupported fields."
            )
        expected_source_id = f"{source_package_id}:datasheet:{source_row_id}"
        if rule_ir.source_id != expected_source_id:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit RuleIR source identity drifted."
            )
        normalized_hash = hashlib.sha256(rule_ir.normalized_text.encode()).hexdigest()
        if normalized_hash != self.normalized_text_sha256:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit normalized rule text hash is stale."
            )
        if not rule_ir.is_supported:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit generated RuleIR must be fully supported."
            )
        return rule_ir


class CorsairVoidUnitsRuleIrPackageArtifact(
    msgspec.Struct,
    frozen=True,
    forbid_unknown_fields=True,
):
    ...
    records: dict[str, _CorsairVoidUnitsRuleIrRecordArtifact]
    package_hash: str

    def validate(self) -> None:
        if self.artifact_schema != CORSAIR_VOID_UNITS_RULE_IR_ARTIFACT_SCHEMA:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit RuleIR artifact schema is unsupported."
            )
        if self.source_package_id != EXPECTED_SOURCE_PACKAGE_ID:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit source package identity drifted."
            )
        if self.source_snapshot_filename != "Datasheets_abilities.json":
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit source snapshot filename drifted."
            )
        ...
        if self.datasheets != EXPECTED_DATASHEETS:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit datasheet inventory drifted."
            )
        if set(self.records) != EXPECTED_SOURCE_ROW_IDS:
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit RuleIR source-row inventory drifted."
            )
        _validate_sha256("package_hash", self.package_hash)
        if self.package_hash != _package_hash(self):
            raise CorsairVoidUnitsRuleIrArtifactError(
                "Corsair void-unit RuleIR package hash is stale."
            )
...
def corsair_void_units_rule_ir_package_artifact_from_json_bytes(
    raw: bytes,
) -> CorsairVoidUnitsRuleIrPackageArtifact:
    try:
        artifact = msgspec.json.decode(raw, type=CorsairVoidUnitsRuleIrPackageArtifact)
    except msgspec.DecodeError as exc:
        raise CorsairVoidUnitsRuleIrArtifactError(
            "Corsair void-unit generated RuleIR artifact is invalid."
        ) from exc
    artifact.validate()
    return artifact


def _package_hash(artifact: CorsairVoidUnitsRuleIrPackageArtifact) -> str:
    payload = msgspec.to_builtins(artifact)
    if type(payload) is not dict:
        raise CorsairVoidUnitsRuleIrArtifactError(
            "Corsair void-unit RuleIR artifact payload is invalid."
        )
...
def _validate_non_empty_string(field_name: str, value: object) -> str:
    if type(value) is not str or not value.strip() or value != value.strip():
        raise CorsairVoidUnitsRuleIrArtifactError(
            f"Corsair void-unit RuleIR artifact {field_name} must be non-empty stripped text."
        )
    return value
...
def _validate_sha256(field_name: str, value: object) -> str:
    ...
    if len(token) != 64 or any(character not in "0123456789abcdef" for character in token):
        raise CorsairVoidUnitsRuleIrArtifactError(
            f"Corsair void-unit RuleIR artifact {field_name} must be lowercase SHA-256."
        )
    return token
