...
from ._artifacts import (
    FulgrimRuleIrArtifactError,
    FulgrimRuleIrPackageArtifact,
    fulgrim_rule_ir_package_artifact_from_json_bytes,
)
...
def _load_artifact() -> FulgrimRuleIrPackageArtifact:
    try:
        ...
        raise FulgrimRuleIrArtifactError(
            "Fulgrim generated RuleIR artifact could not be loaded."
        ) from exc
    return fulgrim_rule_ir_package_artifact_from_json_bytes(raw)


_ARTIFACT: Final[FulgrimRuleIrPackageArtifact] = _load_artifact()
...
def validate_generated_artifact_bytes(raw: bytes) -> None:
    fulgrim_rule_ir_package_artifact_from_json_bytes(raw)
