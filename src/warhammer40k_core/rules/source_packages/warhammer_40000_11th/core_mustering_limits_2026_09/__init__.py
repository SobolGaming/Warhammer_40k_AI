"""Hash-pinned Core 25.03/25.04 mustering limits for P25A."""
...
DETACHMENT_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:single_three_dp_detachment"
UPGRADE_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:upgrade_accounting"


class MusteringLimitsSourceError(ValueError):
    """The reviewed MusteringLimits source identity or provenance has drifted."""


class MusteringLimitsSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class MusteringLimitsSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[MusteringLimitsSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> MusteringLimitsSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise MusteringLimitsSourceError(
            "MusteringLimits source bytes drifted from their reviewed pin."
        )
    try:
        artifact = msgspec.json.decode(raw, type=MusteringLimitsSourceArtifact)
    except msgspec.DecodeError as exc:
        raise MusteringLimitsSourceError("MusteringLimits source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-mustering-limits-source-v1"
        ...
        != (BATTLE_SIZE_SOURCE_ID, DETACHMENT_SOURCE_ID, UPGRADE_SOURCE_ID)
    ):
        raise MusteringLimitsSourceError("MusteringLimits source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise MusteringLimitsSourceError("MusteringLimits source transcription hash drifted.")
        if (
            ...
            raise MusteringLimitsSourceError(
                "MusteringLimits source execution evidence is incomplete."
            )
    return artifact
...
def source_rules() -> tuple[MusteringLimitsSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p25a"),
                title="Reviewed maintained App-data MusteringLimits",
                source_texts=tuple(
