"""Hash-pinned Core 24.13 and 01.02 Fights First scope for P24J."""
...
EXPECTED_ARTIFACT_SHA256: Final = "501db53231ec964f4c60af8dca80b726bca7902ba3618023e7a04919d712593d"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-fights-first"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-21"
FIGHTS_FIRST_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:fights-first"
UNIT_GRANT_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:unit-grants"


class FightsFirstSourceError(ValueError):
    """The reviewed FightsFirst source identity or provenance has drifted."""


class FightsFirstSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class FightsFirstSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[FightsFirstSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> FightsFirstSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise FightsFirstSourceError("FightsFirst source bytes drifted from their reviewed pin.")
    try:
        artifact = msgspec.json.decode(raw, type=FightsFirstSourceArtifact)
    except msgspec.DecodeError as exc:
        raise FightsFirstSourceError("FightsFirst source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-fights-first-source-v1"
        ...
        != (FIGHTS_FIRST_SOURCE_ID, UNIT_GRANT_SOURCE_ID)
    ):
        raise FightsFirstSourceError("FightsFirst source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise FightsFirstSourceError("FightsFirst source transcription hash drifted.")
        if (
            ...
            raise FightsFirstSourceError("FightsFirst source execution evidence is incomplete.")
    return artifact
...
def source_rules() -> tuple[FightsFirstSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p24j"),
                title="Reviewed maintained App-data FightsFirst",
                source_texts=tuple(
