"""Hash-pinned Core 25.03/25.04 roster construction for P25C."""
...
EXPECTED_ARTIFACT_SHA256: Final = "1f3b02b45720ccc6408804eebb9cb8deb9174e7481566c6b5beb05731e620da8"
SOURCE_PACKAGE_ID: Final = "gw-11e-core-roster-construction"
SOURCE_VERSION: Final = "maintained-app-mirrors-observed-2026-09-20"
DUPLICATE_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:duplicate-detachment"
CONSTRAINTS_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:constraints"
ADMISSION_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:unit-admission"
SUPPORT_SOURCE_ID: Final = f"{SOURCE_PACKAGE_ID}:support-attachment"


class RosterConstructionSourceError(ValueError):
    """The reviewed RosterConstruction source identity or provenance has drifted."""


class RosterConstructionSourceRule(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    source_id: str
    section_id: str
    source_text: str
...
class RosterConstructionSourceArtifact(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    artifact_schema: str
    source_package_id: str
    source_version: str
    rules: tuple[RosterConstructionSourceRule, ...]
    evidence: tuple[RuleEvidencePayload, ...]
    package_hash: str


def validate_source_artifact_bytes(raw: bytes) -> RosterConstructionSourceArtifact:
    if hashlib.sha256(raw).hexdigest() != EXPECTED_ARTIFACT_SHA256:
        raise RosterConstructionSourceError(
            "RosterConstruction source bytes drifted from their reviewed pin."
        )
    try:
        artifact = msgspec.json.decode(raw, type=RosterConstructionSourceArtifact)
    except msgspec.DecodeError as exc:
        raise RosterConstructionSourceError("RosterConstruction source schema is invalid.") from exc
    if (
        artifact.artifact_schema != "core-v2-core-roster-construction-source-v1"
        ...
        != (DUPLICATE_SOURCE_ID, CONSTRAINTS_SOURCE_ID, ADMISSION_SOURCE_ID, SUPPORT_SOURCE_ID)
    ):
        raise RosterConstructionSourceError("RosterConstruction source identity drifted.")
    for rule in artifact.rules:
        if hashlib.sha256(rule.source_text.encode()).hexdigest() != rule.transcription_sha256:
            raise RosterConstructionSourceError(
                "RosterConstruction source transcription hash drifted."
            )
        if (
            ...
            raise RosterConstructionSourceError(
                "RosterConstruction source execution evidence is incomplete."
            )
    return artifact
...
def source_rules() -> tuple[RosterConstructionSourceRule, ...]:
...
def _build_source_package() -> RuleSourcePackage:
    ...
    catalog = SourceCatalog(
        ...
        documents=(
            SourceDocument(
                document_id=SourceDocumentId(package_id=package_id, document_id="p25c"),
                title="Reviewed maintained App-data RosterConstruction",
                source_texts=tuple(
