...
`25.0.0`, declared in `openapi.yaml`, `manifest.json`, and
...
2. The proposed contract is compared with the oldest committed baseline for
   its current major, currently `compatibility/25.0.0-shape.json`. Breaking
   changes are rejected while the bundle major remains `25`, preserving the
...
Contract 25 implements oversized setup and required mission edge identity. Only
major 25 is supported. See [24-to-25.md](migrations/24-to-25.md); retain 24.x through
