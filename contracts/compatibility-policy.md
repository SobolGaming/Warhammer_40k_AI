...
`19.0.0`, declared in `openapi.yaml`, `manifest.json`, and
...
2. The proposed contract is compared with the oldest committed baseline for
   its current major, currently `compatibility/19.0.0-shape.json`. Breaking
   changes are rejected while the bundle major remains `19`, preserving the
   ...
   13.0.0, 14.0.0, 15.0.0, 16.0.0, 17.0.0 and 18.0.0 baselines
