...
`15.1.0`, declared in `openapi.yaml`, `manifest.json`, and
...
2. The proposed contract is compared with the oldest committed baseline for
   its current major, currently `compatibility/15.0.0-shape.json`. Breaking
   changes are rejected while the bundle major remains `15`, preserving the
...
The reference server supports one contract major at a time. Contract 15 changes
