...
embarked unit, `transport_unit_instance_id`. The selected unit then receives
...
placements complete the selected unit activation. Accepted Tactical
Disembark keeps that same unit active and immediately returns it to
`select_movement_action` for its required follow-up Normal Move or Advance.
