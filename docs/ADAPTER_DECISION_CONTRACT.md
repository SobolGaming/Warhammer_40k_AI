...
as complete rule execution. Its source text is an uncaptured repository
transcription: no official App version, build, URL, screenshot, or binary is
retained. The separate evidence record therefore marks 40k.app corroboration as
secondary mirror-only and the current engine path as partial. A later behavioral
PR must close the recorded attack-sequence timing/removal gap against captured
primary evidence; adapters must not infer or patch that gap locally.
