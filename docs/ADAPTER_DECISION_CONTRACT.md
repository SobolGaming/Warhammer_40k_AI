...
- `FightPhaseState`: replay-safe Fight phase state containing Start/Pile In/Fight/Consolidate/End step exposure, ordering bands, start-of-phase engagement eligibility, Fights First sources, activations, passes, resolved fight interrupt IDs, and resolved fight interrupt source effect IDs.
...
Phase 15C does not perform Phase 15D pile-in displacement, melee target declaration, melee attack resolution, or consolidation movement. It records deterministic activation/pass/interrupt state and emits `phase15d_resolution: "deferred"` on activation events so later Phase 15D work can attach the movement and melee attack host without adding a second adapter path.
