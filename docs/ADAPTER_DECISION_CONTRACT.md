...
- `CatalogUnitMoveCompletedMortalWoundsTargetSelection`: finite Charge phase catalog RuleIR answer selecting one engine-emitted enemy rules unit within Engagement Range after a source unit ends a Charge move for per-model mortal-wound resolution.
...
Phase 17G Cavalcade of Chaos adds two Movement-phase Stratagem surfaces through the normal `use_stratagem` decision path. `generic:ingress-move` is offered to the active player at the end of their Movement phase after ordinary reserve arrivals are declined or resolved. Accepted use spends CP, records the Stratagem use, then emits `submit_placement_proposal` with proposal kind `strategic_reserves_placement`, placement kind `strategic_reserves`, and context containing `stratagem_handler_id: "generic:ingress-move"`, the serialized Stratagem use, the selected `reserve_state`, `from_start_of_battle: true`, and `placement_scope: "strategic_reserves_only"`. The placement submission is validated by the same Strategic Reserves engine path as ordinary reserve arrivals; adapters must not set reserve arrival state directly.

Phase 17G also adds `generic:force-desperate-escape` for opponent Movement phase Fall Back selection. After the active player selects `select_movement_action` with Fall Back, the Movement engine may open a reaction-window `use_stratagem` request for the non-active player when an eligible friendly unit is engaged with that falling-back enemy unit. Accepted use spends CP and records an engine-owned persisting effect on the enemy unit, then the parent Movement action resumes with a `submit_movement_proposal` Fall Back request whose context preserves the declared mode under `declared_fall_back_mode` and forces `fall_back_mode: "desperate_escape"` with replay-safe `forced_desperate_escape_source_rule_ids` and `forced_desperate_escape_stratagem_use_ids`. Adapters must submit a Fall Back movement proposal using the emitted forced mode; they must not locally downgrade or bypass Desperate Escape.
...
`WEAPON_ABILITY_SELECTION_DECISION_TYPE` is intentionally not a top-level `GameLifecycle.submit_decision(...)` dispatch entry today. It is the single documented nested-decision allowlist entry: `select_weapon_ability_instance` is embedded inside Shooting declaration target-candidate payloads and resolved by copying the selected descriptor ID into the later declaration proposal. Keep this constant available for future hosts that need the same selection shape, including Necrons Tesseract Vault DAMAGED profile support, but do not treat it as an adapter-submittable top-level pending decision unless that future work updates this contract and registers a lifecycle handler.
...
- request context includes `movement_mode: "charge"`, `maximum_distance_inches`, `reachable_target_unit_instance_ids`, `reachable_target_distances_inches`, optional `charge_move_required_target_unit_instance_ids`, and the source `charge_roll` payload.
...
Phase 17G adds opt-in Fight-start decisions for faction runtime content. These decisions are emitted only when the current battle phase is Fight, before the normal `FightPhaseState` opens, and only when a registered Fight-start hook has at least one legal source-backed option. The current implemented hook is Chaos Daemons Shadow Legion Malice Made Manifest.
...
Required Phase 17K post-shoot catalog tests:
...
- persisted `catalog_post_shoot_hit_target_status` effects are replay-safe and expire at the current Shooting phase end;
...
- multiple source groups from one completed attack-sequence event are emitted and resolved sequentially before later completed-sequence windows proceed;
...
## Phase 17K Charge-End Catalog Mortal-Wound Target Decisions

Phase 17K adds Charge phase catalog RuleIR decisions for abilities that trigger after a source unit ends a Charge move, require the source player to select one enemy unit within Engagement Range of the source rules unit, then roll one D6 for each model in the source rules unit and inflict D3 mortal wounds on the selected enemy unit for each successful threshold roll. The supported semantic IR shape is a timing-window trigger with `edge: "after"`, `phase: "charge"`, `timing_window: "charge_move_end"`, a target of one enemy unit within Engagement Range of this unit, and an `INFLICT_MORTAL_WOUNDS` effect with `roll_expression: "D6"`, `roll_count_scope: "each_model_in_this_unit"`, `success_threshold: 4`, `mortal_wounds_expression: "D3"`, and `target_scope: "selected_enemy_unit"`.

The finite decision type is `select_catalog_unit_move_completed_mortal_wounds_target`. It is emitted from the unit-move-completed hook after a `charge_move_completed` event for the source unit and before the Charge phase continues. The pending request actor is the charging unit's player. Request payloads include `submission_kind: "select_catalog_unit_move_completed_mortal_wounds_target"`, `hook_id: "catalog-ir:unit-move-completed-mortal-wounds"`, game ID, battle round, phase `charge`, active player ID, player ID, catalog record ID, ability ID/name, source rule ID, source rules-unit ID, source unit ID, clause ID, effect index, roll threshold, `roll_expression: "D6"`, `roll_count_scope: "each_model_in_this_unit"`, `mortal_wounds_expression: "D3"`, source roll model IDs, charge-move-completed trigger event ID, movement action, `available_target_unit_instance_ids`, and `available_unit_move_completed_mortal_wounds_target_options`.

Option payloads repeat the request context and include `selected_unit_move_completed_mortal_wounds_target` with the selected option ID, target unit ID, and target player ID. Option IDs are deterministic over the catalog record, source rules unit, clause, effect index, trigger event, and selected target. Target discovery is based on current group-aware model geometry for the source attached rules unit and enemy rules units. Adapters must select one emitted option ID and must not infer Engagement Range locally, invent target IDs, roll dice, apply mortal wounds, or resolve Feel No Pain locally.

Accepted selections record an engine-owned selected-target event, then the existing unit-move-completed mortal-wound resolver rolls one D6 per source roll model ID through the deterministic dice manager. Each D6 result meeting the effect threshold emits replay-safe roll payloads and routes D3 mortal wounds through the shared mortal-wound and Feel No Pain paths against the selected target unit. Engine state mutation remains owned by the hook resolver and damage allocation path; adapters only submit the selected option.

Malformed, stale, wrong-actor, wrong-game, wrong-round, wrong-phase, wrong-active-player, wrong-hook, unsupported-option, option-payload drift, source-record drift, trigger-event drift, movement-action drift, target drift, no-longer-enemy target, no-longer-engaged target, and closed-window submissions reject before queue pop, before a `DecisionRecord` is created, and before any selected event, dice roll, mortal-wound event, or damage mutation is recorded.

Charge-end mortal-wound target choices are public table information in the current Phase 17K rules scope because they are based on a public charge move and visible Engagement Range. If a future charge-end ability hides choices or target eligibility, pending requests, option lists, decision records, selected events, damage events, projections, and event deltas must be viewer-scoped and must not leak hidden information through option counts, target snapshots, selected payloads, damage routing, or derived engine values.

Required Phase 17K charge-end catalog mortal-wound tests:

- valid target selection through `FiniteOptionSubmission -> DecisionResult -> GameLifecycle.submit_decision(...)` or the shared Charge phase result path, including request/result payload round-trip;
- one deterministic D6 roll event per source rules-unit model and replay-safe D3 mortal-wound routing for each successful roll;
- group-aware Engagement Range target discovery for attached rules units and component units;
- stale, drifted, malformed, wrong-context, and ineligible-target submissions reject before mutation;
- deterministic JSON-safe decision, selected-event, roll-event, lifecycle, and replay payload round-trip;
- viewer-scoped projection/event redaction for any future hidden charge-end mortal-wound selections.
...
Malformed, stale, wrong-actor, wrong-faction, duplicate-selection, unsupported-option, active/available Dread drift, and option-payload drift submissions reject before the pending queue is popped and before a `DecisionRecord`, `FactionRuleState`, persisting effect, destruction-reaction source, or event is created.
