...
- `core:fire-overwatch`: opponent Movement phase `after_enemy_unit_ends_move` proposal for one friendly unit that can shoot. Accepted use spends CP, records the Stratagem use, creates an out-of-phase shooting state with the parent phase and trigger payload, and emits the normal `submit_shooting_declaration` proposal for the Overwatch unit. The resulting declaration and attack-sequence decisions are submitted through `GameLifecycle.submit_decision(...)`, and the Phase 12A reaction frame resumes only after the out-of-phase shooting state completes.
