...
On first entry to the later 08.03 Battle-shock step, the engine snapshots each active-player attached rules unit that has an alive model on the battlefield and is currently Battle-shocked or at/below Half-strength. That ordered canonical-rules-unit snapshot and its completed-request progress persist across optional reroll decisions, serialization, replay, and lifecycle re-entry. Each snapshot member receives exactly one required test even when it satisfies both predicates. A successful required test clears carried phase-start Battle-shock; a failed test preserves or applies it through the shared Battle-shock outcome path. The public `battle_shock_step_snapshot_created` event pins the game, round, active player, Command phase, phase-start Battle-shocked canonical unit IDs, and complete ordered `BattleShockTestRequest` payloads. After the final roll and any accepted or declined reroll, public event `battle_shock_modifier_applications_recorded` pins the same source-context fields and exact test request plus an ordered `battle_shock_modifier_applications` list. Each row contains the loaded producer `hook_id`, the actual modifier `source_id`, and the exact ordered serialized modifiers attributed to that producer/source pair; rows sort by `(hook_id, source_id)`, and modifier IDs are globally unique for the test. The event is engine evidence, not an adapter extension point: clients may display it but must not add, remove, reorder, recompute, or apply modifiers. Each public `battle_shock_test_resolved` payload then carries `state_update` plus `cleared_battle_shocked_unit_ids`; successful recovery uses `state_update: "cleared_battle_shocked"` and lists the stored rules-unit identity IDs removed by the engine, while failures and successes that do not recover an existing status carry an empty list. The public `battle_shock_step_completed` event pins the exact ordered completed request IDs and complete ordered result payloads. Restored lifecycle entry validates those events and the intervening request, dice, optional-reroll, modifier-source, resolved-result, auto-pass, state-update, and Command-step-anchor authority before advancing, including after live `CommandStepState` has been discarded. Adapters must not construct or alter the snapshot, collapse or duplicate Attached Unit components, roll the test, interpret identity lineage into a local clear, or mutate Battle-shock state. Embarked and Strategic Reserve rules units with no placed model remain deferred to P01.

Every Attached Unit split first emits public event `attached_rules_unit_split_reconciled`. Its payload pins `game_id`, `battle_round`, `active_player_id`, `phase`, owner `player_id`, the historical `attached_unit_instance_id`, the complete starting `component_unit_instance_ids`, the complete ordered `surviving_unit_instance_ids`, and the immutable `starting_attached_unit_record`. This is the shared engine-owned identity transition for Battle-shock and every other split-aware state owner; adapters may display it but must not derive survivors, rewrite ownership, or apply a local split. If that historical Attached Unit was Battle-shocked, the immediately following public event is `battle_shock_state_transferred_after_attached_unit_split`. Its payload pins the same occurrence and survivor identity, the exact `source_battle_shocked_unit_state`, and the exact ordered `successor_battle_shocked_unit_states`. Replay and primary-boundary authority consume both transitions at their event-time position, so a later final-state identity cannot rewrite a valid earlier clear and a partial, deleted, duplicated, reordered, foreign-lineage, or fabricated transfer rejects. Both events are public because Battle-shock and Attached Unit composition are public; adapters may render them but must not derive successor state, apply the transfer, or locally clear any identity.

An optional Battle-shock reroll continues to use finite decision type `select_dice_reroll` and its existing option IDs, but P08B extends `payload.battle_shock_context` with required fields `passed_state_policy` and `additional_modifier_applications`. The only policy tokens are `preserve` for forced tests whose success must not clear an existing status and `clear_if_step_start_shocked` for the required 08.03 Command-step test. `additional_modifier_applications` is a canonical sorted array of source-producer rows; each row contains exact `hook_id`, `source_id`, and a non-empty canonical `modifiers` array using the existing `RollModifier` payload. An empty array is required when the source producer contributes no modifier outside the loaded Battle-shock hook registry. The context also carries the source kind, game/round/phase/active-player identity, exact Battle-shock request and initial roll state, phase-start Battle-shocked unit IDs, resolved-event types, and base result payload. Adapters must submit one emitted reroll option without changing any context field or nested application/modifier row. Before queue pop or `DecisionRecord` creation, lifecycle validation requires the exact source occurrence, loaded provider and permission, request semantics, initial roll, base payload, resolved-event inventory, and complete modifier applications. Command additionally requires the Battle-shock step, exact next request in the persisted snapshot, and exact completed-result prefix. Malformed, stale, reordered, omitted, inserted, or drifted context returns a typed invalid status without consuming the pending request or mutating Battle-shock.

After a required test resolves, an outcome hook may enqueue an existing nested decision such as Healing Wounds model selection or revival placement. That queue head preempts the next snapshot member: Command records the completed-test prefix, returns the exact nested pending request, and does not emit or roll the next required test until the nested path finishes through its own lifecycle contract. Adapters must answer the pending queue head and must not reorder a Battle-shock reroll behind it or locally resume the Command loop.
