...
- `ModelDestructionCauseAuthority`: engine-private, producer-owned
  ...
  field. Fight On Death retention binds an exact unconsumed logical-death cause;
  physical removal consumes it once after deferred destruction reactions finish.
...
- `start_mission_action`: before ordinary Shooting-unit selection, the engine
  ...
  session is invalid.
  Every participation and executor completion is bound to its preceding accepted
  declaration before filtering attack kind, sequence or model identity. Orphaned,
  renamed or duplicate completion pairs are invalid even when labelled non-shooting
  or given empty/foreign model lists. Melee classification requires its original
  accepted melee request/result and deterministic sequence identity; a fabricated
  melee declaration cannot replace a shooting origin. Relabelling a completed shot,
  with or without removing its effect, is invalid. Genuine melee and accepted
  partial-executor checkpoints remain valid.
...
- `core:rapid-ingress`: parameterized target proposal for a friendly canonical rules unit currently in Strategic Reserves, excluding current model-owned `AIRCRAFT`, at the end of the opponent Movement phase in battle round 2 or later. Availability, finite options, target proposals, and pending placement preflight share authoritative eligibility. First-round use, excluded targets, and changed proposal timing contexts reject before queue pop, CP spend, or use accounting. Standalone restoration binds pending target/placement requests to their original decision event and accepted use provenance; a generic `from_start_of_battle` placement permission cannot waive Rapid Ingress eligibility. Accepted use spends CP and records the Stratagem use, then emits a `submit_placement_proposal` request using the existing placement proposal contract. The placement answer must also go through `GameLifecycle.submit_decision(...)`. When Rapid Ingress is offered from a Phase 12A reaction window, the reaction frame continues from the target proposal to the placement proposal and the parent resumes only after a valid placement resolves. Rule-invalid but well-formed placement proposals are recorded as rejected attempts and emit a fresh pending placement request for retry; stale, malformed, or wrong-context placement proposals are rejected before queue pop.
  Pending Rapid Ingress target restoration uses the same target-aware affordability
  authority as request creation, after rebuilding active runtime cost modifiers.
  A legal automatic target discount can therefore preserve a pending request at
...
- `submit_shooting_declaration.payload.proposal_request.target_candidates`: current JSON-safe per-copy target candidates carrying the selected `weapon_instance_id`, legality, violation diagnostics, visible-and-in-range target model IDs, line-of-sight witness, visibility cache key, engine-enumerated `shooting_types`, hit modifier, targeting rule IDs, and `required_weapon_ability_selections` for adapter-visible duplicate `[ANTI]` descriptor choices when a selected weapon profile has more than one matching Anti descriptor for that target. Ordinary ranged targets require at least one present living or retained model. A candidate with neither uses `target_has_no_placed_living_models` (the retained diagnostic identifier) and empty visible/in-range model inventories. Fight On Death-only units remain declaration targets. Shooter lock and target Engagement contexts use the shared symmetric physical Engagement query, so a retained-only enemy can lock a living shooter and a retained base can establish the Engagement context of an otherwise living mixed target. Visible/in-range model IDs include retained models, subject to the same visibility, range and target restrictions. Damage allocation remains living-only. Slash-separated Anti keyword groups match if the target has any listed keyword, while `match_mode: "missing_keyword"` descriptors match only when the target has none of the listed keywords. `[HUNTER X]` is represented as target eligibility: candidates for non-matching targets use violation code `hunter_target_keyword_mismatch`, and legal matching candidates carry `weapon-ability:hunter` in `targeting_rule_ids`.
...
`[INDIRECT FIRE]` profiles. Ordinary weapons still require visible, otherwise
legal targets. Only an INDIRECT FIRE profile fired in this mode receives the
Indirect Cover grant, no-hit-reroll marker and unmodified failure range; this
applies to visible and unseen targets. A qualifying stationary friendly-observed
attack fails on unmodified 1–3; other Indirect attacks fail on 1–5. Normal mode
does not inherit these restrictions. The engine retains existing targeting IDs,
pool fields, dice restriction IDs and viewer-scoped projections. Clients consume
the emitted options and results through the same submission path; they do not
recompute visibility, stationary status, Cover or reroll opportunities.
...
immediately and preserves the fixed placement until the attacking rules unit
finishes its attacks and emits `attack_sequence_attacks_resolved`. At that
boundary the engine first offers applicable Fight On Death grants through the
retention context below. Acceptance retains the original placement and defers
other destruction reactions. Decline or a failed grant continues mandatory
reactions, physical removal and the remaining optional reactions in ordinary order.
`AttackSequence` retains ordered destruction records, original damage and
attacks-resolved event IDs, original placements and captured reaction sources.
Shooting and Fight share this owner; adapters cannot advance, reorder or remove
models independently of the pending engine decision.
...
Order 17/P24D adds no decision type, option family, or proposal kind. After the
shared Shooting/Fight attack resolver has completed every attack for a rules
unit, it selects each physical attack-pool `weapon_instance_id` whose profile
has the canonical `HAZARDOUS` keyword and makes one simultaneous D6 packet with
that quantity. The public `hazardous_test_resolved` and
`hazardous_mortal_wounds_applied` events carry `source_phase`, the ordered
one-to-one `hazardous_weapon_instance_ids` and
`hazardous_weapon_profile_ids`, `failed_hazardous_weapon_instance_ids`, the
complete roll state, mortal wounds per failed roll, and the aggregate mortal
wound packet. Repeated profile IDs are intentional and must not be deduplicated
by adapters. If that packet pauses for an existing mortal-wound model or Feel
No Pain choice, its source context preserves the same physical identities,
phase, roll, failure mapping, and wound total. Restore and pre-submission
validation reject drift before queue pop or mutation. These events and choices
are public battlefield information in the current rules scope and continue to
use the shared viewer-scoped projection and event-delta path; clients must not
roll, assign failed dice to weapons, apply wounds, or infer Shooting/Fight
origin locally.

Phase 13E implements this destroyed-model attack-resolution decision:

- `select_destruction_reaction` remains a finite controlling-player choice. Options
  are applicable source IDs plus `decline_destruction_reaction`; source options
  carry `source_id`, `reaction_kind` and `optional: true`. Mandatory destruction
  reactions are automatic engine work and never decline-capable options.

Order 30 / P05B adds `destruction_context.context_kind: "fight_on_death_retention"`
before physical removal, using the same finite submission and dispatch registry.
Its private context binds `cause_id`, `logical_death_event_id`, `model_instance_id`,
physical `target_unit_instance_id`, `destroyed_model_controller_player_id` and
`retention_sha256`. The exact source inventory, applicable grant outcomes,
original placement and attack/rule continuation belong to engine state. A stale
source, granting condition, controller, phase, pose or request snapshot returns
typed invalid status before queue pop, dice or mutation. Selecting one source
does not combine multiple grants or repeat trigger rolls.

The same request supports `shoot_on_death`, `fight_on_death` and
`shoot_or_fight_on_death` source descriptors. Single-action grants retain their
source ID as the option ID. A source authorizing either action emits
`<source_id>:shoot` and `<source_id>:fight` alongside the ordinary decline option.
Each grant option carries `source_id`, `reaction_kind`, `optional` and `action`; selection consumes
one entitlement. The choice never grants both actions or another model's weapons.
The selected action is recorded in the retained state and public selection event.

The shared retained-destruction owner covers attacks, direct rule destruction,
already-applied mortal wounds and collateral casualties. Its stages are
`offered`, `waiting_for_unit_attack`, `ready_for_destruction_reactions`,
`resolving_destruction_reactions`, `waiting_for_retained_casualty`,
`removed_resolving_reactions`,
`declined` and `not_triggered`. Accepted models keep their exact original bases
...
can suspend for mortal-wound allocation, Feel No Pain, a collateral Fight On
Death choice, a remaining optional destruction reaction or destroyed-Transport
hazard/placement decisions. Parent causes wait for children; resumption uses
the original source owner without replaying attacks, damage, dice or removal.
After removal, ordinary optional reactions use the existing destruction
context with damage, removal, provenance and continuation evidence.

Shoot On Death uses the ordinary out-of-phase shooting declaration, target,
weapon, attack, allocation and reroll decisions after the attacking unit's
attacks. Its source context names `retained_destruction_shooting` and the exact
retained model. Only that model's ranged weapons are enumerated. Completion
uses `model_shoot_completed`, resolves its deferred destruction rules and
...
`attack_sequence_models_attacked` records the exact declared models at the
shared completed-attacks boundary, with game, round, active player, actual phase,
sequence, unit and attack phase. Source-authorized prior-action restrictions
consult this model history before rolling. Restoration binds that history to
the declarations and completed-attack events. The source-backed For the Chapter!
consumer also admits its model's own Hazardous death and automatically passes
Hazardous tests for the resulting shots. Unending Fidelity costs 1 CP, applies
in the opponent's Shooting phase or either Fight phase, targets the selected
friendly GREY KNIGHTS INFANTRY unit, and forbids a reaction by a model that has
already shot or fought this phase. Its one 4+ grant authorizes the Shoot-or-Fight
choice. These consumers do not certify complete faction or datasheet support.

`fight_on_death_retention_selected` exposes the selected source, stage, model
and original placement. `fight_on_death_destruction_completed` exposes final
removal and its completion reason. Opening, trigger, suspended and progressed
authority events, cause IDs, logical-death IDs and continuation hashes are
engine-private through the single shared redaction module for both viewers.
`retained_shooting_started` and `hazardous_destruction_routing_started` are also
private because they carry resumable source authority. Public
`retained_shooting_attacks_completed`, `retained_shooting_resumed_parent`,
`retained_shooting_hazardous_automatically_passed` and
`hazardous_model_destruction_completed` events omit internal cause identifiers.
Restore cross-validates retained effects, source requests/results, original
logical death, attack boundaries, source liabilities, fixed placement, nested
progress and exact-once completion against the event and decision histories.
...
presence services. The offline generator preserves transcriptions, observation
fingerprints and the maintained-mirror audit; runtime never fetches source text.
The separate `retained_attack_sources_2026_09` artifact preserves the two faction
rule transcriptions observed in the 40k.app browser mirror on 2026-09-08,
its displayed App-data version 946, source-linked RuleIR, historical official
PDF provenance and exact source hashes. The generic ability consumer and
Stratagem provider load these descriptors without display-name branching.
Contract 12 records this mutation/redaction change and the target witness rename
to `present_model_instance_ids`; see `contracts/migrations/11-to-12.md`.
...
living or movable; accepted retention grants target geometry until cleanup. For multi-model mortal-wound routing, an
...
- `core:fire-overwatch` (Order 45, external contract 17): one optional `end_phase` opportunity at the end of the opponent's Movement phase, independent of movement or setup history. Its deterministic window ID is `fire-overwatch-end-movement-round-{round:02d}-player-{player}`; the trigger payload contains only `timing_window_id` and `trigger_window: "end_opponent_movement_phase"`. The existing `submit_stratagem_target_proposal` selects one friendly living, present, unengaged, non-`TITANIC` rules unit that can shoot and has at least one legal Snap declaration. Window discovery follows the existing optional proposal/affordability contract and shared basic shooter/present-enemy checks; it does not solve every possible shot before the player decides whether to use the Stratagem. Current CP cost, Battle-shock, Action restrictions, source-target protections and loaded target-restriction hooks are revalidated before accepting the proposed shooter. A submitted proposal must preserve the exact issued context. Accepted use spends CP once and opens the shared out-of-phase shooting state. Its canonical `submit_shooting_declaration` offers all eligible enemy rules units, including stationary enemies, without a triggering-enemy binding. The player chooses one visible enemy within 24 inches of the shooting rules unit, measured across all present attacking components; every declared physical weapon copy must target that same enemy using `snap`. An attached component without a ranged weapon can supply the unit-range measurement. Removed components cannot supply it. Per-weapon range and visibility still apply. Candidates whose source-forced type is unavailable have `is_legal: false`, an empty `shooting_types` list and `forced_shooting_type_unavailable`. Unknown, stale, drifted, protected or otherwise invalid targets reject before queue pop, record creation or mutation. Attached shooter and target identities are canonical rules-unit IDs. Non-automatic hits require an unmodified six regardless of BS or modifiers unless an explicitly Snap/Overwatch-authorized source changes the threshold; Torrent still auto-hits, and Hit rolls cannot be rerolled. Hit payloads retain `unmodified_success_threshold_active`, `minimum_unmodified_success` and source provenance. Completion applies the shared post-Snap Action lock until the parent phase ends. Selected-to-shoot grants, target/group selection, Precision, allocation, saves, Feel No Pain and destruction interruptions use the ordinary engine decision path. Persistence and exact replay retain the chosen enemy and parent reaction state. Phase-end sequencing resumes only after the attack completes, with one Overwatch opportunity regardless of the number of enemy moves; simultaneous effects continue through the shared sequencing authority. See [the contract migration](../contracts/migrations/16-to-17.md).
...
Malice Made Manifest accepts living or retained ability authority in the
Enhancement-bearing source component and living or retained presence in each
eligible enemy rules unit. Engagement Range uses the shared physical scenario.
...
ability-bearing source component and each enemy target. Its Engagement Range
snapshot uses shared physical geometry, including retained-only units in
`eligible_enemy_rules_unit_ids` when the other source conditions are met.
...
continuation binds its original logical-death cause before consumption. Physical
removal consumes that cause exactly once after deferred destruction reactions finish. Recovery cross-validates every
...
## Order 41 — Explosives model selection

The existing finite submission envelope and shared public Stratagem redaction
contract cover the new effect-selection variant above. It contains only public
battlefield unit/model IDs, canonical source identity and CP/effect evidence.
Pending choices, attempts, checkpoint/restore and replay use the normal engine
path. Source-model or target changes invalidate a pending choice before mutation;
...
## Order 62 — passenger post-placement Shock engagements

Contract 28 implements the owner-resolved 18.07 interpretation. The forced queue
uses every enemy engaged with the passengers after setup, regardless of earlier
engagement with the Transport or other units. Both attached components and
retained destroyed enemy bases contribute through the shared physical query.
`start_engaged_enemy_unit_instance_ids` remains required and empty for Shock.
`post_engaged_enemy_unit_instance_ids` is required on the completion event and
queue-skip evidence; ordinary disembark completion emits an empty list.
The latter evidence is authenticated against reconstructed physical geometry,
not current positions or mutually matching saved copies. Phase history excludes
already selected enemies; the opponent must resolve each remaining eligible
unit one at a time through the existing Fight decisions before Movement resumes.
No new decision or submission shape is introduced. Both players see the same
