...
Flawless Blades Daemonic Patrons reuses `select_faction_rule_fight_phase_end_option` only when its optional selected-to-fight grant was accepted in the current Fight phase and no enemy model was destroyed by an attack made by any model in that rules unit during that phase. Its request payload contains `submission_kind: "select_failed_fight_activation_model_destruction"`, game/round/phase/active-player/player context, hook and source rule IDs, the RuleIR hash, activation and destruction clause IDs, the accepted persisting-effect ID, the canonical rules-unit ID, and `eligible_model_instance_ids`. It emits one deterministic `destroy-model:<model_instance_id>` option per currently alive model in the rules unit; the selected option adds `selected_model_instance_id`. There is no decline option because the consequence is mandatory once its conditions are met. Accepted selection destroys exactly that model through engine-owned state mutation, removes its battlefield placement, consumes the activation effect, and emits replay-safe `model_destroyed` and `catalog_failed_fight_activation_model_destroyed` events. Adapters must not infer whether a kill qualifies, choose an unlisted model, apply damage, remove a placement, or suppress destruction locally.

The Daemonic Patrons kill check requires a current-round Fight-phase `model_destroyed` event whose `destroying_player_id` is the Flawless Blades controller, whose `attacking_model_instance_id` belongs to the activated rules unit, and whose target rules unit belongs to an opponent. Destruction by another friendly unit, non-attack destruction, destruction of a friendly model, another phase/round, or the Daemonic Patrons consequence itself does not satisfy the condition. Rule/source/hash/effect/unit/model snapshot drift or a selected model that is no longer alive returns a typed invalid status without mutation.
...
- Daemonic Patrons accepted-grant persistence, 3+ Critical Wound consumption, exact attack-kill attribution, mandatory model selection, and engine-owned destruction;
- stale, drifted, malformed, wrong-context, and ineligible submissions reject before mutation;
