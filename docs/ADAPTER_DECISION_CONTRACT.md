...
`HealingEffect`; adapters still answer only emitted `select_healing_model`
options and must not apply the wound or revival result locally.
...
Phase 17G adds opt-in setup decisions for faction runtime content and source-backed catalog RuleIR during Declare Battle Formations. These decisions are emitted only when the mustered army's runtime contribution or catalog ability index registers a battle-formation hook for the player. The current implemented hooks are Death Guard Nurgle's Gift plague selection, updated from the 11th Edition faction-pack `RULES UPDATES` section, Aeldari Corsair Coterie Archraider model selection, Aeldari Wraithlord Fated Hero keyword selection, and Imperial Knights Code Chivalric oath selection.
...
## Phase 17K Post-Shoot Catalog Hit-Target Status And Effect Decisions
...
Generic option payloads repeat the request context and include `selected_catalog_target_effect` with the selected option ID and canonical target rules-unit ID, plus `generic_rule_effect_records` containing the engine-prepared phase-scoped `generic_rule_execution` persisting-effect payloads. Option IDs are deterministic over the catalog record, source unit/model, selection clause, canonical selected target, and attack-sequence-completed event. Hit-target discovery is based on successful hit events from the just-completed attack sequence. Adapters must select one emitted option ID and must not infer eligible targets from display text, event-log inspection, local attack simulation, wound success, damage, destruction, or target visibility.

Accepted generic selections create engine-owned `PersistingEffect` records through the end of the current Shooting phase. Each persisted effect carries the original RuleIR clause/effect payload, replay-safe execution context, canonical selected target rules-unit ID, selected-target gate parameters, catalog record IDs, source unit/model IDs, attack sequence ID, and attack-sequence-completed event ID. Attack resolution retains the exact attacking model and its physical owning component. The shared generic attack-effect matcher resolves attacker and target component IDs to canonical rules-unit identities for persisting-effect lookup, attacker/target role membership, and rules-unit conditions, while retaining the exact model identity for component ownership, wargear ownership, and model-scoped conditions. A selected-unit attacker effect, such as suppression, gates on the selected attacking rules unit; a source-unit attack effect that was selected against an enemy continues to gate on the current defending target. Adapters must not apply wound rerolls, damage bonuses, status grants, roll modifiers, or selected-target gates locally.
