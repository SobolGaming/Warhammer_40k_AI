...
Malformed, stale, wrong-actor, wrong-faction, duplicate-selection, unsupported-option, active/available Dread drift, Shadow Form source/hash/record drift, and option-payload drift submissions reject before the pending queue is popped and before a `DecisionRecord`, `FactionRuleState`, persisting effect, destruction-reaction source, or event is created.
