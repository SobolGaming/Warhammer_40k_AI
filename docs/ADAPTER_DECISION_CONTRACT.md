...
Fire Overwatch is not emitted from the active player's normal Shooting phase and is not represented by a persisting marker. Until a real out-of-phase shooting state can host Movement or Charge reaction-window shooting and resume the parent phase through the Phase 12A reaction contract, its source-backed descriptor remains `unsupported:` and submissions fail before CP spend or state mutation.
