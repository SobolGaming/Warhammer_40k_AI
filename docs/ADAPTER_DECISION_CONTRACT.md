...
Malformed, stale, wrong-actor, wrong-game, wrong-round, wrong-phase, wrong-active-player, wrong-target-owner, self-target, missing target, destroyed target, wrong-rules-unit-owner, destroyed rules unit, off-battlefield rules unit, duplicate target or activation, unsupported-option, and option-payload drift submissions reject before the pending queue is popped and before a `DecisionRecord`, persisting effect, healing effect, reroll permission, modifier state, or event is created.
