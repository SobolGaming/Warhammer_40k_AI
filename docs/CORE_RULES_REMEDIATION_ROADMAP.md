...
- `C09-03` owns the actual turn/phase occurrence of Normal Move history and its
...
- `C01-06` owns actual phase-start friendly revival anchors under P01F, distinct
  from the immediately-pre-return enemy engagement authority of P01E.
...
`C24-03B` / P24C2 (Order 56), the parent `C24-03` family and category 24 remain open.
...
At this historical milestone Heroic Intervention used natural 2D6, already
within 1–12. Order 50 supersedes that flow with the shared modified Charge roll
and a source limit applied before separate movement-distance effects. Positive reactive distance bonuses are
...
- **Finding IDs:** `C15-09` / `P15I`; review corrections `R48-001` on `a6ab86f7` and `R48-002` on `9dd78ed3`.
...
- **How it is currently done:** before this change, the catalog used a non-enumerable target, Charge completion omitted the opportunity, ownership and geometry could select one attached component, runtime Toughness modifiers were omitted, and self wounds were uncapped. The shared Core Stratagem wound path also removed casualties without invoking Deadly Demise.
- **How it should be done:** enumerate the charging rules unit, engaged enemy and engaged living source model in one finite choice; spend once, freeze one Toughness-based roll, cap both packets at six, and route allocations/FNP and casualty reactions through their existing engine owners. The owner explicitly approved including the shared destruction repair; Explosives now uses the same retained-packet service.
- **Specific authoritative maintained direct App-data mirror rule/statement and source ID:** Crushing Impact 15.05, `gw-11e-core-stratagems:core:crushing-impact`: after a friendly MONSTER/VEHICLE finishes a Charge move in its owner's Charge phase, select an engaged enemy and an engaged source model; roll D6 equal to that model's Toughness; each 1 inflicts a self mortal wound, each 5+ an enemy mortal wound, with a separate six-wound cap per unit.
- **Provider, URL, App-data version or observation timestamp, transcription SHA-256, and source-observation fingerprint:** maintained 40k.app mirror, `https://www.40k.app/rules/15-stratagems`, observed `2026-08-26T11:15:23-04:00`; transcription `63fe27d984e7863a906d1ff7edeaef678fa69cdc1c6a7040869409749353e060`; fingerprint `329f378b3cb1f78f28f7f32047e01e2b78295d155c30df9fde775bf0cab3afa4`. Historical official GW provenance is preserved. Load status remains `loaded`; separate execution status is `executable_engine_runtime`.
- **Scope and explicit exclusions:** Order 48 plus the approved shared destruction continuation and its physical-history authority repair. Orders 49–51 remain separate. No new named handler, faction/display-name gate, speculative registry, unsupported catalog content, or AI/performance optimization was added.
- **Owning state/validation/mutation/event/replay path:** source JSON and typed Core catalog → canonical move-completed trigger and timing batch → finite Stratagem selection/prevalidation → CP/use record and modifier/dice services → shared mortal-wound allocation and retained-packet routing → existing rule destruction/Deadly Demise/removal owner → source-linked completed-packet callback → suspended Charge continuation. Physical-history scans retain complete event evidence. Existing registry bindings and deterministic event/decision records survive facade persistence and replay. R48-001 routes the existing authenticated retention records into packet ownership validation and accepts exactly one later receipt for each casualty handed to retention. R48-002 follows authenticated cause ancestry for collateral retention and closes removed retained continuations after declined rule-reaction dispatch.
- **Decision and viewer-visibility impact:** existing `use_stratagem` options carry the closed enemy/model pair and exact completed-move context. Stale/malformed submissions fail before consumption/spend. Existing defender-owned allocation/FNP/destruction requests remain shared across adapters. The shared redaction owner hides packet receipts and destruction evidence. Contract 19's existing generic schemas cover the public shapes; the adapter contract documents the changed finite choice and continuation.
- **Regression scenarios and same-bug-class search:** MONSTER/VEHICLE, attached source/target identities, distinct selected-model Toughness plus modifiers, both caps, decline, stale CP/phase/keyword/presence/range, retained dead models, both-unit destruction, FNP on both sides, Deadly Demise collateral damage, suspension/restoration, offered and accepted source-backed For the Chapter! reactions, one- and two-level collateral cause ancestry, forged parent/root ownership, completed retained shooting, altered ownership and acceptance records, duplicate/unrelated/missing receipts, exact replay and public redaction. Searches covered immediate/deferred move discovery, duplicate cap calculations, canonical ownership/geometry, both Core damage consumers, and complete-history requirements in physical authority. The initial Charge and Deadly Demise regressions failed before their fixes.
...
- **Validation results:** all required local gates passed after R48-002: Ruff check/format, Mypy, Pyright, 7,976 behavioral tests with 85.12% coverage, 478 code-quality tests, regenerated eight-shard inventory check, import boundaries and pre-commit. Generated source/runtime/exact-base contract checks, installed-wheel smoke, TypeScript client and five unit tests passed; conformance passed 342 assertions. All 56 revised-engine checkpoint measurements restored exactly and met versioned bounds, including offered and accepted/completed collateral reactions. Both accepted/completed scenarios now close retained ownership and resume the parent shooting context. Original Charge and R48-001 timing evidence remain historical; full-game certification remains outstanding. The [scope record](ORDER_48_SCOPE_PLAN.md) and [R48-002 measurements](performance/order48/r48-002/README.md) record commands, results and ownership audits.
...
### P18H — C18-09 (Order 61)

The shared Embark validator now consumes authenticated rules-unit setup history,
scoped to battle round and actual turn player, instead of the selected Transport's
same-phase disembark list. Attached component identity and physical model history
...
The source generator is `tools/build_core_embark_setup_turn_source.py`; final
validation and matched component measurements are recorded in
[Order 61 performance evidence](performance/order61/README.md).

## P18F implementation evidence — Order 62

C18-07's source exception was resolved by the repository owner on 2026-09-19:
Shock passengers may set up engaged with any enemy. Every enemy now engaged
with those passengers that has not been selected to Fight this phase is selected
one at a time through the opponent's canonical Fight path. Prior engagement with
the Transport or another unit does not define the queue. Embarked passengers
inherit no Transport starting engagements. This supersedes the historical P18E
interpretation and the unresolved C18-07 status above, without changing P18G
eligibility or the separately owned P12B source exception.

The retained complete v931 row and immutable source tuple, ownership trace,
regressions and validation are recorded in [Order 62 scope](ORDER_62_SCOPE_PLAN.md).
Contract 28 commits the post-placement engagement evidence independently of the
empty starting snapshot. Source load support and execution status stay separate.
Implementation and local validation are complete: 8,405 behavioral tests passed
...
component sources and conditional Leader `this_model` grants retain explicit
model ownership; Charge and explicit unit grants retain unit scope. Live ordering and
...
## Order 74 / P03C — mandatory endpoint terrain exclusion

The owner approved this scoped prerequisite on 2026-09-22 after the PFINAL
preflight reproduced C03-03 through the ordinary Charge facade. The immutable
source tuple, geometric counterexample, shared-owner trace, exact proof domain,
regressions, adapter impact and validation are recorded in
[Order 74 scope and evidence](ORDER_74_PREFLIGHT_AUDIT.md).

P03C is Order 74 and PFINAL moves to Order 75. The current inventory is 73
implementation PRs, S-MIRRORS and PFINAL (75 rows); previous count statements
retain their historical meaning. The implementation supplies a sufficient
...
## Order 75 / P21C — fixed-target Surge maximum approach

The owner approved this prerequisite on 2026-09-22 after the PFINAL preflight
reproduced C21-03: the engine rejected a legal, provably optimal Surge against
a fixed rectangular target on an empty battlefield. The previous bound
relaxed both footprints into enclosing circles, losing the stationary target's
shape and facing. The shared geometry owner now preserves the fixed target's
measured footprint, uses actual separation for rotation-invariant circular
movers, and retains an enclosing-radius relaxation for rotating movers. Live
Surge and historical proof authentication already consume this same owner.

P21C is Order 75 and PFINAL moves to Order 76. The current inventory is 74
implementation PRs, S-MIRRORS and PFINAL (76 rows); earlier count statements
record their historical review states. The existing source IDs, proposal and
evidence schemas remain unchanged. Path, terrain, coherency and engagement
validation remain mandatory; general obstacle-constrained optimality is not
claimed. See [scope and evidence](ORDER_75_PREFLIGHT_AUDIT.md) and
[matched measurements](performance/order75/README.md).

CAUDIT-01 remains open. PFINAL requires merge of this prerequisite and a fresh
complete 25-category audit. Complete-game performance remains uncertified.

The preflight also retains malformed-ingress observations under open CAUDIT-01:
invalid-kind prevalidation events can drift on exact replay, and a missing
...
The fresh PFINAL preflight of main `dc01911f57024ae69b565a0e965db65abf5fbdbe`
on 2026-09-23 reproduced C09-03 during the September 10 Normal Move consumer
check. A reactive Normal Move by Player B during Player A's Movement phase
incorrectly removes Normal Move from Player B's own Movement options later in
the same battle round. The shared history stores the unit owner but not the
turn owner, conflating two different phase occurrences. The defect and the
declined-reaction control both survive persistence and exact replay.

P09C is inserted as Order 80 and PFINAL moves to Order 81. The current inventory
is 79 implementation PRs, S-MIRRORS and PFINAL (81 rows); earlier counts are
historical. See [Order 80 preflight](ORDER_80_PREFLIGHT_AUDIT.md) for source
evidence, the retained executable probe, owner/consumer audit and proposed
acceptance. The owner approved fixing the identified gaps. This PR implements
P09C with explicit turn identity, shared completion recording, accepted-history
authentication, contract 35 migration and direct consumer regressions.
No PFINAL PR is open, no complete 25-category snapshot is certified, and
...
- Status: merged in [PR #502](https://github.com/SobolGaming/Warhammer_40k_AI/pull/502). P01F is the next prerequisite.
- Finding IDs: C01-05.
...
- Violated invariant: Core 01.02.03 permits engagement with an already-engaged
  enemy rules unit, including another model or attached component of that unit.
- Previous behavior: HealingEffect snapshots and placement validation tracked
  enemy model IDs; producers that omitted the snapshot prohibited all engagement.
- Required behavior: measure canonical enemy units immediately before each return
  using all rules-present models. Phase-start coherency anchors remain separate.
- Controlling statement/source: complete expanded 40k.app Core 01.02.03,
  `gw-11e-core-revival:revival`, observed 2026-09-24T13:16:10+00:00, unversioned.
  URL, complete text, transcription SHA-256 and observation fingerprint are in the
  [registered audit](../data/source_audits/maintained_app_mirrors/revival_2026_09_24.audit.json)
  and its typed source package. No official-App conflict is asserted.
- Scope/exclusions, owner path and same-class search: see the
  [implementation record](ORDER_82_SCOPE_PLAN.md). One shared predicate serves all
  HealingEffect producers and restore; no new faction/content support or named handler.
- Decision/viewer impact: existing facade submissions and redaction; public,
  source-bound unit evidence on accepted revival events. Invalid proposals remain atomic.
- Regression scenarios: other enemy model/component; attached target/enemy;
  retained and removed presence; genuinely new enemy; retry; both viewers;
  checkpoint/replay; forged/missing/source-drifted and legacy historical evidence.
- Generated artifacts/documentation: Core source package and authority registry;
  Contract 36, replay v30, persistence v28; runtime identity, external contract and
...
## Order 83 / P01F — actual phase-start revival anchors

Order 82 merged in PR #502. A fresh bounded preflight reproduced C01-06:
a second revival effect treats a model returned earlier in the same phase as
a phase-start coherency anchor. The owner approved the repair described in
[the Order 83 record](ORDER_83_PREFLIGHT_AUDIT.md). P01F owns Order 83; PFINAL
moves to Order 84. The current inventory is 82 implementation PRs, S-MIRRORS
and PFINAL (84 rows). Earlier order numbers and audit counts remain historical.

P01F authenticates phase-opening membership across shared producers, live
placement and restored histories, with Contract 37 evidence. It does not close
CAUDIT-01. After this prerequisite merges, repeat the complete 25-category audit
from current main. Complete-game performance remains uncertified.
