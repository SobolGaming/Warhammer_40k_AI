...
- **Finding IDs:** `C15-09` / `P15I`; review correction `R48-001` on `a6ab86f7`.
...
- **Validation results:** all required local gates passed after R48-001: Ruff check/format, Mypy, Pyright, 7,958 behavioral tests with 85.11% coverage, 477 code-quality tests, regenerated eight-shard inventory check, import boundaries and pre-commit. Generated source/runtime/base-ref contract checks, installed-wheel smoke, TypeScript client and five unit tests passed; conformance passed 342 assertions. All 42 revised-engine checkpoint measurements restored exactly and met versioned bounds. Original Charge timing evidence remains historical; full-game certification remains outstanding. The [scope record](ORDER_48_SCOPE_PLAN.md) and [R48-001 measurements](performance/order48/r48-001/README.md) record commands, results and ownership audits.
