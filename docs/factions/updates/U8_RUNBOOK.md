...
U5 tombstones and the U6 rewrite procedure run **inside**
`implement_prs` for `structural_remove` and `faction_rewrite` packets.
They are not a licence to skip child packets or to skip regenerate.
...
| U6 overlay | Does not suppress per-entity packets |
...
family. Track D D3 still updates
`ADAPTER_DECISION_CONTRACT.md` only in the same PR as a real new family
or decision kind.

U5 and U6 run here when their packets say so (§5 and §6).

### 4.8 `u7_package`

When the cadence produces a new admitted content-set version, U7
packages it as current. N−1 is packaged only for an owner whose
`first_certified_at_content_set` is already set and for whom this is a
later transition. Orks at FM0.5 stay current-only.

Packaging does not write Q1 `certified` producer coverage (U7a).

### 4.9 `regenerate_artifacts`

One closing regenerate, after the packets that this cadence claims
closed:
...
A `faction_rewrite` overlay packet requires `u6_rewrite_procedure`:

1. Admit a new content-set version for that `owner_faction_id`.
2. Run the same L0–L8 tooling over every owned or inherited entity.
3. Emit per-entity child packets. The overlay must not suppress them.
4. Retire every removed entity through U5.
5. Regenerate Q1 and the D1 guide.
6. Apply U7: `first_certified_at_content_set` does not reset; if already
   certified, the old current becomes previous.

Orks v946 is the mapping example: overlay plus More Dakka! remove plus
Brute Bosses add plus per-entity cost/clause packets. Orks is not
certified there, so no previous Orks version is packaged.
...
| U6 procedure | New current version; child packets; retire removals | Suppress children; reset `first_certified` |
| U7 | Packaging slots | Q1 `certified` producer entries |
...
8. After a certified Chaos Daemons 946, a later rewrite to 960 packages
   960 current and 946 previous. `first_certified` stays 946.
...
| Chaos Daemons certified rewrite | U6 + U7 N−1 | Fixture 8 |
...
- ten ordered stages with U5 / U6 inside `implement_prs`;
- U1 offline-only capture that cannot admit or classify;
- rewrite overlays that cannot suppress children;
- tombstones that cannot unpackage historical N−1;
- regenerate of Q1, D1, and D2 as a required closing step;
