...
Force Disposition pairing through normal lifecycle execution, GameLifecycle
restore replay, and viewer-scoped projections. All 15 pairings now have
executable Primaries in both directions (45 of 45 variants) and pairing-wide
lifecycle, replay, and viewer-scoped certification.
...
two-sided Primary scoring, and Step 5G certifies each pairing through normal
lifecycle execution, restore replay, and viewer-scoped projections. Step 4's mission
...
Step 5G certifies every Force Disposition pairing.
...
`AdapterGameSession` / `LocalGameSession` path. Each pairing instantiates its
source-hashed battlefield package, scores an ordinary turn-end Primary boundary
through `BattleRoundFlow`, round-trips `GameLifecycle` and event-log payloads,
...
ID/hash commitments. Layout A of each pairing is the lifecycle/replay/viewer
certification row; A/B/C variants remain in the fail-closed 45-layout inventory
and instantiate two-sided scoring policies. This does not claim Phase 20A
full-game certification. Replay remains `replay-artifact-v8-phase17n-step5a`.
