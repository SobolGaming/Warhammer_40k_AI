...
non-affiliation marker. It authenticates the audit identity, row fingerprint,
provider surface, policy, and version-or-timestamp identity against the
hash-pinned packaged `source_authority_registry.json`. `RuleSourcePackage`
requires the registry's typed `warhammer_40000_11th_core_rules` scope and an
exact registered package identity; only its allowlisted Core Rules stable
source IDs can cross that boundary. `SourceEvidenceCatalog` compares records
sharing one stable rule source ID and App-data version; if multiple named
providers have different transcription hashes, construction fails pending
official-App comparison. Historical 40k.app records keep their superseded
policy ID only when their evidence identity, stable source ID, and observation
...
all 25 Primary Missions executable**: all 15 Event Companion Force
...
source-backed Primary Mission Actions. Step 5A adds the authenticated
scoring-state bridge and four source-backed generic objective predicates.
Step 5B scores consecrated, decoy, and triangulated markers, promoting
...
explicit first-boot initialization, atomic and read-back-verified SQLite v2
command-state/journal/cursor commits, content-addressed checkpoints, verified
decision-tail replay recovery, and an unpruned per-revision commitment chain
that binds typed mutation origins, authoritative decision/event/RNG prefixes,
adapter checkpoints, lifecycle `started`/`closed` flags, journal outcomes, and
authenticated cursor transitions. The flags preserve typed creation, start,
and final close validation after old full snapshots are pruned.
Recovery also requires the generated runtime-tree build fingerprint and exact
package/schema identities, and fails closed on missing state, corruption, or
drift. The local hashes are an internal-consistency boundary, not resistance to
a malicious database writer or rollback without an external trusted anchor.
**Phase 16A is
...
| 18L | Complete | Explicit durable initialization, exact SQLite v2 validation plus transactional read-back, immutable runtime-tree build identity, unpruned revision commitments, exact recovery, and fail-stop publication |
...
Completed adapter phases such as 18J and 18L remain in the completed table
above; this table contains only unfinished work.
...
separately owned by the shared typed transport-disembark permission service and
its bounded Assault and Shock consumers.
...
- Disembark is legal only for a unit embarked within a `TRANSPORT` model that is on the battlefield and only if that unit did not Embark within that Transport this phase; an Advanced or Fallen Back Transport prohibits it unless an exact source-backed typed override authorizes the applicable mode;
- Disembark uses an explicit source-backed mode enum and replay payload for `rapid_disembark`, `assault_disembark`, `shock_disembark`, `tactical_disembark`, `combat_disembark`, `destroyed_transport`, and `emergency_disembark`; any new adapter-visible mode, option family, proposal payload, or event shape must update `docs/ADAPTER_DECISION_CONTRACT.md` in the same implementation PR;
...
- Assault Disembark is a distinct 3" wholly-within setup whose Core eligibility excludes only Transport Advance and Fall Back. It requires a typed persisting-effect permission naming the Transport, canonical embarked rules unit, and permitting rule source ID; completes that rules unit's Movement activation without further movement; and records the Core 18.06 source and permitting source while preserving charge eligibility through state, replay, decisions, events, and adapters;
- Shock Disembark is a distinct 3" wholly-within setup with no Core Transport movement restriction. It requires the same exact typed source permission ownership, authenticates the canonical enemy rules-unit IDs engaged with the Transport at move start, preserves every one of those engagements after grouped placement, and makes the opponent select each still-unselected enemy one at a time through the ordinary Fight activation request and execution pipeline before Movement resumes; the transient forced context is source-, trigger-, phase-, passenger-, Transport-, and viewer-bound through events and restore;
...
- Assault Disembark tests cover source eligibility, same-phase embark rejection, all Core-eligible Transport movement states, canonical attached rules-unit 3" placement, no-further-move/charge-eligible state, forged Transport/permission rejection before queue pop, event and replay provenance, and charge-phase consumption;
- Shock Disembark tests cover source eligibility, all six Transport movement states, canonical attached enemy identity, 3" placement, exact start-engagement snapshot and preservation, malformed snapshot rejection before queue pop, one-at-a-time opponent Fight activation without a pass, canonical selected-to-fight hooks, completion, event/replay restore, determinism, and both-viewer adapter projection/event visibility;
...
- player army-list conversion recalculates repeated-datasheet brackets,
  ...
  and rejects per-unit or total point drift before mustering; artifact array
  order, not identifier spelling, defines repeated-datasheet pricing order;
- `ArmyMusterRequest` accepts strict roster metadata for source-backed unit
  and Enhancement/Upgrade points, their exact MFM source package, Warlord
  selection, Enhancement assignment, and Dedicated Transport starting-cargo
  manifests;
...
- player-list unit array order is the authoritative repeated-datasheet copy
  order and survives request payload round trips; opaque unit identifiers never
  choose a price bracket;
- source-backed unit and Enhancement/Upgrade point records and their exact MFM
  package identity survive mustering; catalog Enhancement price drift and
  player-supplied Enhancement assignment source drift are invalid;
- `game_result` is optional historical metadata and is absent for normal
  pre-session roster input rather than being fabricated;
...
- player army-list JSON fail-fast loading, MFM source identity, repeated-unit
  price brackets under opaque/reordered identifiers and different unit
  compositions, repeated Upgrade costs, authoritative Enhancement point records,
  stale catalog/assignment-source rejection, optional result metadata,
  per-unit/total point drift rejection, and ordinary mustering through the
  resulting strict request without deleting source-backed wargear options;
...
- `primary_mission_scoring_coverage_rows()` tracks current Primary Mission
  scoring status: 25 of 25 missions are engine-implemented, 0 are source-known
  engine-pending, and 0 await source scoring text.
...
with package hash
`8358cc11078b27e8773f58182e40bbf6194c54c3bda16b3aa1e286ee9b646dd1`
...
#379, including the exact 25 executable versus 0 engine-pending status boundary.
...
already-generic cumulative resolution grammar; Step 5E now scores its
operation-marker bonus.
...
These generic primitives remain available to Locate and Deny, Extract Relic,
Secure Asset, and Punishment. Step 5 later promoted those missions on their
card-specific scoring conditions; the primitives themselves stay generic.
...
Step 5B scores persistent mission markers through the shared generic condition
path. Consecrated, decoy, and triangulated markers are identified by catalog
source identity plus ACTIVE OBJECTIVE ownership; consecrated markers also
require a linked consumed designation. Marker thresholds fail closed without
`PrimaryScoringStateEvidence`. The decoy opponent-territory bonus additionally
requires spatial evidence and does not treat a generic operation marker as a
decoy. This promotes Consecrate, Smoke and Mirrors, and Triangulation only.

Step 5C scores completed Primary Mission Actions through the same shared
condition path. Completed Actions are identified by catalog source identity
plus this-turn COMPLETED status for the scoring player and assigned Primary.
Each-unit branches count unique `unit_instance_id` values; boolean branches
score once. The Sabotage opponent-territory rider requires spatial evidence
...
Surveil the Foe stay pending after Step 5C. The new complete pairings are
...
stay pending after Step 5D.

Step 5E scores operation markers through the same shared condition path.
Terrain occupancy uses current rules-unit position witnesses and logical
terrain-area intersection: exactly one remaining operation marker scores when a
friendly unit occupies that terrain area and no enemy unit does. Gather Intel
end-of-battle branches count extract-intelligence markers by catalog source
identity, including a 3" opponent-home-range check against MissionSetup
objective centers. Vital Link awards 1 VP per maintain-control marker within
range of a controlled central objective. `no_enemy_operation_markers_on_battlefield`
is implemented for later Surveil work and is not enough to promote that card.
This promotes Gather Intel, Extract Relic, Locate and Deny, and Vital Link.
The new complete pairings are disruption versus priority-assets, purge-the-foe
versus priority-assets, and reconnaissance versus reconnaissance.
...
- operational `created_at` and `last_activity_at` timestamps that do not affect
  simulation ordering;
- server-owned principal-to-player assignments and explicit non-player roles;
...
- authenticated administrators alone authorize start, advance, and close,
  while authenticated player principals may submit only for their server-bound
  pending actor; coaches, delayed spectators, and replay viewers are read-only
  according to the Phase 18H role table.
...
`SessionCommandOutcome` schemas are carried forward in contract version `2.0.0`,
and the reference server exposes `ExecuteSessionCommand` as the normative
mutation operation. The formal Phase 18E mutation routes have been removed from
the 2.0 surface; authenticated legacy `/games` routes remain deprecated
development adapters and do not replace the normative command envelope.
...
retries, stale revisions and requests, wrong principals, malformed envelopes,
...
Status: Complete. Every reference-server route authenticates an opaque bearer
credential through an injected server-owned principal registry and derives its
viewer/actor context from the bound role. The 2.0 request schemas accept no
participant assignments, viewer authority, or actor authority from clients.
...
always performs recovery, and a missing file or singleton row is never treated
as an empty authority. An interrupted initialization requires deliberate
operator repair or replacement rather than implicit retry.

Every committed command publishes the staged authoritative session,
idempotency outcome, revision snapshots and unpruned revision commitment,
cursor registry, and server index only after their atomic durable transaction
succeeds. Recorded rejected attempts and successful reads that update
operational activity metadata use the same transaction. Session creation is
also durable before the new session becomes addressable, and the store maintains
one serialized writer per authority. The process arms fail-stop state before it
calls the commit boundary and clears it only after success, including when a
custom store reports an `OSError` or `RuntimeError`. Only a fresh owner may
resume after loading and verifying the durable checkpoint, so an ambiguous
post-commit failure cannot be overwritten by stale in-memory state.

The SQLite reference store uses `server-persistence-store-v2` and
`PRAGMA user_version = 2`. It holds `BEGIN IMMEDIATE` across exact schema
validation, existing-row validation, UPSERT, and exact read-back. Recovery and
commit require the one expected STRICT singleton table, WAL mode, the exact
column and constraint definition, and no extra tables, indexes, foreign keys,
views, or triggers. A suppressed or rewritten write cannot be reported as a
...
- session revision, idempotency results, and every unpruned per-revision
  commitment;
- latest replay inputs/artifact and periodic verified state checkpoint;
- participant/role bindings and cursor policy state;
- server/engine/contract versions, immutable runtime-tree build fingerprint,
  and terminal status.

Each domain-separated revision commitment binds the preceding revision,
typed command or non-command origin, exact decision/event/RNG prefixes,
resulting adapter checkpoint and viewer-independent authoritative state,
explicit `started`/`closed` flags, journal outcome, and authenticated
before/after cursor state. Those flags keep creation, start, and final close
transition validation independent of full-snapshot retention. Full historical
snapshots remain bounded by retention, but their commitments do not. Recovery
can therefore prove command/journal coverage and prefix membership even after
the corresponding full snapshot is pruned.

Recovery procedure:

1. Load the latest verified checkpoint and require exact persistence, package,
   runtime-build, authorization, schema, cursor, and root identities.
2. Validate the contiguous revision chain, typed origins, journal bijection,
   historical prefixes, retained checkpoints, command-to-decision linkage, and
   authenticated cursor transitions.
3. Replay subsequent accepted decisions through
   `GameLifecycle.submit_decision(...)` via the session owner.
4. Verify decision, event, projection, RNG, and package hashes plus deterministic
   event sequence/revision.
5. Resume service only on exact agreement; otherwise return typed corruption or
   drift diagnostics.
...
- explicit initialization versus missing-file/missing-root recovery;
- SQLite ignored/rewritten writes, exact STRICT/singleton schema drift, and a
  schema mutation racing the write transaction;
- decision-tail replay recovery with exact projection/event/RNG hashes;
- package, immutable engine-build, schema, and checkpoint drift fail closed;
- alternate-branch snapshot, removed journal, changed envelope, coherent
  response-projection, and valid-but-wrong cursor drift fail closed;
- participant roles and authenticated viewer cursors restore without visibility
  widening;
- typed and untyped custom-store commit-boundary errors latch fail-stop state.
...
envelopes; validate the exact revision chain and command-to-history evidence;
and compare session revision, decision/event records, RNG state, replay artifact,
projection hashes, authenticated cursor scopes, and idempotent response bytes.
Independent missing-root, SQLite schema/write, corruption, package, ruleset,
catalog, source, runtime-build, external-contract, persistence-schema,
revision-chain, and checkpoint-hash drift cases fail before a session is
registered.
...
The content-addressed root, SQLite row hash, revision commitments, and build
fingerprint provide deterministic internal consistency. They are not keyed
storage attestations and cannot, by themselves, detect a malicious writer that
replaces the complete artifact and recomputes its hashes or rolls storage back
to an older valid root. Deployments whose threat model includes hostile storage
or rollback require a trusted external monotonic, signed, or append-only anchor;
Phase 18L does not claim that protection.
