...
try {
  ...
  if (generated.status === 0) {
    ...
    writeGeneratedModules(compactedSource, outputPath);
  }
} finally {
  rmSync(temporaryDirectory, { recursive: true, force: true });
}

function writeGeneratedModules(source, modelPath) {
  const sourceFile = ts.createSourceFile(modelPath, source, ts.ScriptTarget.Latest, true);
  const operationDeclarations = sourceFile.statements.filter(
    (statement) => ts.isInterfaceDeclaration(statement) && statement.name.text === "operations",
  );
  if (sourceFile.parseDiagnostics.length !== 0 || operationDeclarations.length !== 1) {
    throw new Error("Generated OpenAPI models must contain one valid operations interface.");
  }
  const declaration = operationDeclarations[0];
  const start = declaration.getStart(sourceFile);
  const end = declaration.getEnd();
  const models = source.slice(0, start) + source.slice(end);
  const modelStem = basename(modelPath, ".ts");
  const operationPath = resolve(dirname(modelPath), `${modelStem}.operations.ts`);
  const header = "// @ts-nocheck -- recursive JSON values are generator-owned.\n";
  writeFileSync(
    modelPath,
    `${header}import type { operations } from "./${modelStem}.operations.js";\n`
      + `export type { operations } from "./${modelStem}.operations.js";\n${models.trimEnd()}\n`,
    "utf8",
  );
  writeFileSync(
    operationPath,
    `${header}// Auto-generated by scripts/generate-models.mjs; do not edit.\n`
      + `import type { components } from "./${modelStem}.js";\n\n${source.slice(start, end)}\n`,
    "utf8",
  );
}
