...
export const EXTERNAL_CONTRACT_VERSION = "10.0.0";
...
function assertPublishedOpenApi(contractRoot: string): void {
  ...
  if (info.version !== EXTERNAL_CONTRACT_VERSION) {
    throw new Error(
      `Conformance requires external contract version ${EXTERNAL_CONTRACT_VERSION}.`,
    );
  }
