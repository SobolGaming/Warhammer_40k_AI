"""Build the reviewed Order 86 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_table_quarters_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/table_quarters_2026_09_25.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-table-quarters"
VERSION = "maintained-app-mirrors-observed-2026-09-25"
OBSERVED_AT = "2026-09-25T15:36:30+00:00"
AUDIT_ID = "core-table-quarters-maintained-app-mirrors-2026-09-25"
GDM_URL = "https://game-datamissions.com/11th/rules/core-rules"
TABLE_QUARTERS_TEXT = (
    "When a rule refers to a table quarter, or table quarters, divide the "
    "battlefield with imaginary lines 1mm thick drawn lengthwise and widthwise "
    "through the centre of the battlefield to create four equal rectangles. "
    "These rectangles are the table quarters."
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, url, consumers in (
        (
            ...
            TABLE_QUARTERS_TEXT,
            GDM_URL,
            ["warhammer40k_core.engine.table_quarters:scoring_table_quarter_id_or_none"],
        ),
    ):
        ...
        review = {
            **shared,
            "evidence_id": f"core-table-quarters-review:{slug}",
            ...
            "source_title": f"P01I {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-table-quarters-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-table-quarters-source-v1",
        "source_package_id": PACKAGE_ID,
        "source_version": VERSION,
        "rules": rules,
        "divider_width_mm": 1.0,
        "evidence": evidence,
        "package_hash": "",
    }
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "Complete 01.04.05 observed in the expanded Game Datamissions browser UI "
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P01I source artifact drift: {path.relative_to(ROOT)}")
        else:
