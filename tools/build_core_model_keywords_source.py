"""Build the reviewed Order 31 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_model_keywords_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/model_keywords_2026_09_08.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-model-keywords"
VERSION = "maintained-app-mirrors-observed-2026-09-08"
OBSERVED_AT = "2026-09-08T13:35:55Z"
AUDIT_ID = "core-model-keywords-maintained-app-mirrors-2026-09-08"
GDM_URL = "https://game-datamissions.com/11th/rules/changelog"
KEYWORD_TEXT = (
    "Units have all the keywords of all their models, and attached units have all the keywords "
    "of all of their component units. How are destroyed models taken into account for these "
    "purposes?\n"
    "These statements are only true so long as the models possessing those keywords remain part "
    "of the unit; destroyed models do not contribute their keywords to their prior unit. "
    "Example: The Aspiring Sorcerer model in a unit of Rubric Marines is destroyed; as none of "
    "the remaining models in the unit possess the PSYKER keyword, that unit is no longer a "
    "PSYKER unit."
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, consumers in (
        (
            "destroyed-model-keywords",
            "FAQ",
            KEYWORD_TEXT,
            ["warhammer40k_core.engine.rules_units:RulesUnitView.keywords"],
        ),
    ):
        ...
        review = {
            **shared,
            "evidence_id": f"core-model-keywords-review:{slug}",
            ...
            "source_title": f"P02D {section} {slug}",
        ...
        providers: list[tuple[str, str, str | None]] = [("Game Datamissions", GDM_URL, "931")]
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-model-keywords-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-model-keywords-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "Complete v931 FAQ observed directly in the browser. No co-version comparison "
            "with 40k.app is asserted. P05B retains the separately pinned presence exception."
        ),
    }
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P02D source artifact drift: {path.relative_to(ROOT)}")
        else:
