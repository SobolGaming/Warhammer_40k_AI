# ruff: noqa: E501
"""Reproduce the reviewed P04B source package and observation audit offline."""
...
RULES = (
    (
        "critical-hit-success-faq",
        "04 FAQ v931",
        "04-making-attacks",
        "My attack has a rule that means it results in a critical hit on a hit roll of 4+, and I would otherwise need a 5+ to hit. Does a 4+ hit?\nYes, except when snap shooting: when snap shooting, each attack only hits on a roll of 6.",
    ),
    (
        "snap-critical-hit-faq",
        "04 FAQ v931",
        "04-making-attacks",
        "My attack has a rule that means it results in a critical hit on a hit roll of 4+. Does this work when snap shooting?\nUnless otherwise stated, no: when snap shooting, each attack only hits on a roll of 6.",
    ),
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, _page, source_text in RULES:
        ...
        row: dict[str, object] = {
            "evidence_id": f"core-critical-hits-mirror:{slug}",
            "rule_source_id": source_id,
        ...
        review = {
            **row,
            "evidence_id": f"core-critical-hits-review:{slug}",
    ...
    return artifact, {
        ...
        "co_version_comparison": "Read both complete FAQ entries directly in the browser with App-data 931 selected. No co-versioned observation from a second provider is asserted.",
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P04B source artifact drift: {path.relative_to(ROOT)}")
        else:
