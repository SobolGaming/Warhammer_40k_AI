"""Build the reviewed Order 71 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_engaged_shooting_2026_09/artifacts/package.json"
)
AUDIT_PATH = (
    ROOT / "data/source_audits/maintained_app_mirrors/engaged_shooting_2026_09_21.audit.json"
)
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-engaged-shooting"
VERSION = "maintained-app-mirrors-observed-2026-09-21"
OBSERVED_AT = "2026-09-21T18:41:25Z"
AUDIT_ID = "core-engaged-shooting-maintained-app-mirrors-2026-09-21"
FORTY_K_URL = "https://www.40k.app/rules/17-monsters-and-vehicles"
ENGAGED_TARGET_TEXT = (
    "In your Shooting phase, enemy MONSTER/VEHICLE units that are engaged can be selected "
    "as targets of ranged attacks (excluding attacks made with [BLAST] weapons).\n"
    "Each time a model makes a ranged attack that targets such a unit, subtract 1 from the "
    "hit roll (excluding attacks made with [CLOSE\u2011QUARTERS] weapons by models in a unit "
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, url, consumers in (
        (
            "engaged-monster-vehicle-target",
            "17.03",
            ENGAGED_TARGET_TEXT,
            FORTY_K_URL,
            ["warhammer40k_core.engine.shooting_engagement:engaged_shooting_penalty_sources"],
        ),
    ):
        ...
        review = {
            **shared,
            "evidence_id": f"core-engaged-shooting-review:{slug}",
            ...
            "source_title": f"P17 {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-engaged-shooting-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-engaged-shooting-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "17.03 observed through the 40k.app search index. Direct fetch returned 403. "
            "No App-data version or co-version comparison is asserted. Core 10.06 reuses "
            "the complete authenticated September 9 observation in core_actions_2026_09; "
            "that observation and its stable source ID remain unchanged."
        ),
    }
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P17 source artifact drift: {path.relative_to(ROOT)}")
        else:
