"""Build the reviewed Order 84 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_dice_results_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/dice_results_2026_09_24.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-dice-results"
VERSION = "maintained-app-mirrors-observed-2026-09-24"
OBSERVED_AT = "2026-09-24T20:42:09+00:00"
AUDIT_ID = "core-dice-results-maintained-app-mirrors-2026-09-24"
FORTY_K_URL = "https://www.40k.app/rules/01-core-concepts"
HIGHEST_LOWEST_TEXT = (
    "If a rule refers to the highest dice result and a dice roll involves multiple "
    "dice, if more than one of those dice have the same value, and that value is "
    "the highest, the active player must select one of those dice to be the highest "
    "dice result. If a rule refers to the lowest dice result and a dice roll involves "
    "multiple dice, if more than one of those dice have the same value, and that "
    "value is the lowest, the active player must select one of those dice to be "
    "the lowest dice result."
)
TREATED_AS_SET_TO_TEXT = (
    "When a dice result is \u2018treated as\u2019 or 'set' to another value, any rules that "
    "would take effect if that value had been rolled take effect. This may result "
    "in a value that is greater than a value that could be rolled on a D6 dice."
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, url, consumers in (
        (
            ...
            HIGHEST_LOWEST_TEXT,
            FORTY_K_URL,
            ["warhammer40k_core.engine.dice_extremum:request_dice_extremum"],
        ),
        (
            "treated-as-set-to",
            "01.05.08",
            TREATED_AS_SET_TO_TEXT,
            FORTY_K_URL,
            ["warhammer40k_core.core.dice:DiceRollState.with_result_override"],
        ),
        (
            "default-critical-hit",
            "05.01",
            "Unmodified 6\nCRITICAL HIT",
            "https://www.40k.app/rules/05-attack-sequence",
            ["warhammer40k_core.engine.interpreted_dice:CriticalRollThreshold"],
        ),
        (
            "default-critical-wound",
            "05.02",
            "Unmodified 6\nCRITICAL WOUND",
            "https://www.40k.app/rules/05-attack-sequence",
            ["warhammer40k_core.engine.interpreted_dice:CriticalRollThreshold"],
        ...
        review = {
            **shared,
            "evidence_id": f"core-dice-results-review:{slug}",
            ...
            "source_title": f"P01G {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-dice-results-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-dice-results-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "Complete 01.05.07 and 01.05.08 expanded browser sections and the "
            "05.01/05.02 critical-result table rows observed through "
            "40k.app on September 24. Neither this observation nor corroborating GDM "
            "Core page exposes an App-data version; no co-version equivalence is claimed. "
            "The official PDF remains historical provenance."
        ),
    }
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P01G source artifact drift: {path.relative_to(ROOT)}")
        else:
