"""Build the reviewed Order 65 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_firing_deck_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/firing_deck_2026_09_20.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-firing-deck"
VERSION = "maintained-app-mirrors-observed-2026-09-20"
OBSERVED_AT = "2026-09-20T11:30:00Z"
AUDIT_ID = "core-firing-deck-maintained-app-mirrors-2026-09-20"
FORTY_K_URL = "https://www.40k.app/rules/24-core-abilities"
FIRING_DECK_TEXT = (
    "This ability always takes the form Firing Deck X. In your Shooting phase, each time "
    "this TRANSPORT is selected to shoot, if one or more units are embarked within it, "
    "resolve the following sequence: 1. Select up to X models embarked within this TRANSPORT "
    "(excluding models whose units have already been selected to shoot this phase). "
    "2. For each selected model, select one of its ranged weapons (excluding [ONE SHOT] weapons). "
    "3. Until this TRANSPORT has resolved all of its attacks, it has all of those selected "
    "weapons in addition to its other weapons. 4. Until the end of the turn, units embarked "
    "within this TRANSPORT are not eligible to shoot."
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, consumers in (
        (
            ...
            FIRING_DECK_TEXT,
            [
                "warhammer40k_core.engine.firing_deck_restrictions:record_firing_deck_restriction",
                "warhammer40k_core.engine.shooting_eligibility_state:shooting_state_restriction_reason",
            ],
        ),
    ):
        ...
        review = {
            **shared,
            "evidence_id": f"core-firing-deck-review:{slug}",
            ...
            "source_title": f"P24B {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-firing-deck-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-firing-deck-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "24.14 retained from the 40k.app search-index observation. The direct fetch returned "
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P24B source artifact drift: {path.relative_to(ROOT)}")
        else:
