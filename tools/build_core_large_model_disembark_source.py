"""Reproduce the reviewed Order 55 source package and observation audit offline."""
...
PACKAGE_ID = "gw-11e-core-large-model-disembark"
VERSION = "maintained-app-mirrors-observed-2026-09-17"
OBSERVED_AT = "2026-09-17T00:00:00+00:00"
AUDIT_ID = "core-large-model-disembark-maintained-app-mirrors-2026-09-17"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
SOURCE_ID = f"{PACKAGE_ID}:large-model-disembark"
TEXT = (
    'base within 1" of that Transport\u2019s base (or hull), and not engaged with any enemy units.'
)
...
OBLIGATIONS = [
    (
        "A size exception requires proof no position or orientation fits the base wholly "
        "within the ordinary disembark distance beside the Transport."
    ),
    (
        "Congestion, terrain, enemy models and a poor proposed orientation do not prove "
        "the base is too large."
    ),
    (
        "The exceptional model must have its base within one inch of the Transport base "
        "or hull and be unengaged."
    ),
    (
        "Every other setup requirement remains applicable, including the complete rules "
        "unit's coherency."
    ),
    (
        "The effective ordinary distance is three inches or the disembark mode's six-inch "
        "distance; a model that fits that ordinary distance does not qualify."
    ),
]

DESCRIPTOR = {
    "source_rule_id": SOURCE_ID,
    "requires_impossible_size_fit": True,
    "maximum_base_distance_inches": 1.0,
    "requires_unengaged": True,
    "uses_mode_setup_distance": True,
}

ARTIFACT_PATH = ROOT / (
    "src/warhammer40k_core/rules/source_packages/warhammer_40000_11th/"
    "core_large_model_disembark_2026_09/artifacts/package.json"
)
AUDIT_PATH = (
    ROOT / "data/source_audits/maintained_app_mirrors/large_model_disembark_2026_09_17.audit.json"
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    row: dict[str, object] = {
        "evidence_id": "core-large-model-disembark-mirror:large-model-disembark",
        "rule_source_id": SOURCE_ID,
        ...
        "review_audit_row_id": "large-model-disembark",
        "review_audit_source_observation_sha256": audit_hash,
        "provider_name": "40k.app",
        "source_title": "40k.app 03.02.02 Setting Up Large Models: Disembarking",
        "source_platform": "Web",
        "source_url": URL,
        "observed_at": OBSERVED_AT,
        "app_version": None,
    ...
    review = {
        **row,
        "evidence_id": "core-large-model-disembark-review:large-model-disembark",
    ...
    artifact: dict[str, object] = {
        "artifact_schema": "core-v2-core-large-model-disembark-source-v1",
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"Order 55 source artifact drift: {path.relative_to(ROOT)}")
        else:
