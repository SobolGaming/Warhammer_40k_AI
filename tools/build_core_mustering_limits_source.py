"""Build the reviewed Order 67 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_mustering_limits_2026_09/artifacts/package.json"
)
AUDIT_PATH = (
    ROOT / "data/source_audits/maintained_app_mirrors/mustering_limits_2026_09_20.audit.json"
)
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-mustering-limits"
VERSION = "maintained-app-mirrors-observed-2026-09-20"
OBSERVED_AT = "2026-09-20T18:08:38Z"
AUDIT_ID = "core-mustering-limits-maintained-app-mirrors-2026-09-20"
...
BATTLE_SIZE_TEXT = (
    ...
    "amount shown above, and every EPIC HERO has a unit limit of 1, regardless of the battle size."
)
DETACHMENT_TEXT = (
    "If you are playing an Incursion battle, you can select a 3DP detachment "
    "as your only detachment."
)
UPGRADE_TEXT = (
    "You can include up to three of the same Upgrade in your army (the second and third "
    "instances of the same Upgrade do not count towards the total number of enhancements "
    "in your army, but you must still spend the stated points cost each time)."
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, consumers in (
        (
            ...
            BATTLE_SIZE_TEXT,
            [
                "warhammer40k_core.engine.list_validation:BattleSizeMusteringPolicy.incursion",
                "warhammer40k_core.engine.roster_unit_limits:datasheet_unit_limit",
            ],
        ),
        (
            "single_three_dp_detachment",
            "25.04",
            DETACHMENT_TEXT,
            [
        ...
        review = {
            **shared,
            "evidence_id": f"core-mustering-limits-review:{slug}",
            ...
            "source_title": f"P25A {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-mustering-limits-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-mustering-limits-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "25.03 table/footnote and 25.04 single-detachment/Upgrade accounting clauses retained "
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P25A source artifact drift: {path.relative_to(ROOT)}")
        else:
