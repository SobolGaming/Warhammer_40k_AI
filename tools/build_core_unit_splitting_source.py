"""Build the reviewed Order 24 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_unit_splitting_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/unit_splitting_2026_09_06.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-unit-splitting"
VERSION = "maintained-app-mirrors-observed-2026-09-06"
OBSERVED_AT = "2026-09-06T17:29:03Z"
AUDIT_ID = "core-unit-splitting-maintained-app-mirrors-2026-09-06"
FORTY_K_URL = "https://www.40k.app/rules/01-core-concepts"
SPLITTING_TEXT = (
    "Some rules allow you to split units into smaller units (e.g. the Transport ability of "
    "Drukhari Venoms). When using such rules:\n"
    "The same models can only be subdivided in this way once (e.g. those smaller units cannot "
    "themselves be split further).\n"
    "If a rule is used to split a unit into multiple units before the battle, the starting "
    "strength of each individual unit is changed to be equal to the number of models in that unit."
)
BALANCED_TEXT = (
    "A unit must be split into two units, each containing as equal a number of models as possible "
    "(when splitting a unit in this way, make a note of which models form each "
    "of the two new units). "
    "If a rule states a specific starting strength of the split units, but this is not possible "
    "based on the number of models in the unit being split (e.g. due to attached Leader/Support "
    "models), split the unit as stated above."
)
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, consumers in (
        (
            ...
            "01.02.06",
            SPLITTING_TEXT,
            ["warhammer40k_core.engine.unit_splitting:build_split_army"],
        ),
        (
            "balanced-unit-splitting",
            "01.02.06",
            BALANCED_TEXT,
            ["warhammer40k_core.engine.unit_splitting:build_split_army"],
        ),
        ...
        review = {
            **shared,
            "evidence_id": f"core-unit-splitting-review:{slug}",
            ...
            "source_title": f"P01B {section} {slug}",
        ...
        providers: list[tuple[str, str, str | None]] = (
            [("40k.app", FORTY_K_URL, None)]
            if slug == "unit-splitting"
            else [
                ("Game Datamissions", "https://game-datamissions.com/11th/rules/changelog", "931")
            ]
        )
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-unit-splitting-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-unit-splitting-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "01.02.06 baseline clauses retained from "
            "the 40k.app search-index observation. The direct fetch returned "
            "403. The balancing addition is the complete GDM v931 erratum, also present in "
            "its changed row. No co-version comparison is asserted."
        ),
    }
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P01B source artifact drift: {path.relative_to(ROOT)}")
        else:
