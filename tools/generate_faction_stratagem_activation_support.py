...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate versioned Stratagem activation JSON.")
    ...
    raw = (json.dumps(_artifact_payload(profiles), indent=2, ensure_ascii=False) + "\n").encode()
    if args.check:
        if OUTPUT_PATH.read_bytes() != raw:
            raise SystemExit("Stratagem activation JSON is stale.")
    else:
        OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
        OUTPUT_PATH.write_bytes(raw)
    print(f"Stratagem activation artifact SHA-256: {hashlib.sha256(raw).hexdigest()}")
...
def _artifact_payload(profiles: tuple[ActivationProfile, ...]) -> dict[str, object]:
    rows = []
    rule_irs: dict[str, object] = {}
    for profile in profiles:
        row = asdict(profile)
        del row["compiled_rule_ir_payload"]
        del row["effect_selection_kind"]
        rows.append(row)
        payload = _overlay_rule_ir_payload_or_none(profile.profile_id)
        if payload is None:
            payload = profile.compiled_rule_ir_payload
        rule_irs[profile.profile_id] = (
            payload
            if payload is not None
            else _rule_ir_payload(
                profile_id=profile.profile_id,
                source_id=profile.source_id,
                rule_target_kind=_rule_target_kind(
                    target_kind=profile.target_kind, target_policy_id=profile.target_policy_id
                ),
                rule_ir_hash=profile.rule_ir_hash,
            )
        )
    return {
        "artifact_schema": "core-v2-stratagem-activation-artifact-v1",
        "source_package_id": SOURCE_PACKAGE_ID,
        "source_version": SOURCE_VERSION,
        "source_snapshot_sha256": hashlib.sha256(RAW_STRATAGEMS_PATH.read_bytes()).hexdigest(),
        "profiles": rows,
        "rule_ir_by_profile_id": rule_irs,
        "effect_selection_kind_by_profile_id": {
            profile.profile_id: profile.effect_selection_kind
            for profile in profiles
