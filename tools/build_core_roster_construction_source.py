"""Build the reviewed Order 69 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_roster_construction_2026_09/artifacts/package.json"
)
AUDIT_PATH = (
    ROOT / "data/source_audits/maintained_app_mirrors/roster_construction_2026_09_20.audit.json"
)
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-roster-construction"
VERSION = "maintained-app-mirrors-observed-2026-09-20"
OBSERVED_AT = "2026-09-21T00:32:52Z"
AUDIT_ID = "core-roster-construction-maintained-app-mirrors-2026-09-20"
FORTY_K_URL = "https://www.40k.app/rules/25-muster-armies"
REVIEWED_INPUT = ROOT / "data/source_audits/maintained_app_mirrors/order69-reviewed-input.json"
REVIEWED_INPUT_SHA256 = "7d646c3f76a137eb1694a6ef7aa00a08c8868299e256f8af602bc7cb3e57db4c"


def _reviewed_texts() -> dict[str, str]:
    raw = REVIEWED_INPUT.read_bytes().replace(b"\r\n", b"\n")
    if hashlib.sha256(raw).hexdigest() != REVIEWED_INPUT_SHA256:
        raise ValueError("Reviewed roster construction source input drifted.")
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, source_text in _reviewed_texts().items():
        section = "25.04"
        consumers = [
            {
                "duplicate-detachment": (
                    "warhammer40k_core.engine.list_validation:validate_detachment_selection"
                ),
                "constraints": (
                    "warhammer40k_core.engine.roster_construction_validation:"
                    "append_construction_violations"
                ),
                "unit-admission": (
                    "warhammer40k_core.engine.list_validation:validate_unit_selection_for_army"
                ),
                "support-attachment": (
                    "warhammer40k_core.engine.roster_attachment_validation:"
        ...
        review = {
            **shared,
            "evidence_id": f"core-roster-construction-review:{slug}",
            ...
            "source_title": f"P25C {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-roster-construction-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-roster-construction-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "25.04 detachment, unit admission and Support attachment clauses retained "
            "from the 40k.app search-index observation. The direct fetch returned "
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P25C source artifact drift: {path.relative_to(ROOT)}")
        else:
