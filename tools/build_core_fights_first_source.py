"""Build the reviewed Order 70 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_fights_first_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/fights_first_2026_09_21.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-fights-first"
VERSION = "maintained-app-mirrors-observed-2026-09-21"
OBSERVED_AT = "2026-09-21T12:06:54Z"
AUDIT_ID = "core-fights-first-maintained-app-mirrors-2026-09-21"
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, section, source_text, url, consumers in (
        (
            ...
            FIGHTS_FIRST_TEXT,
            FORTY_K_URL,
            ["warhammer40k_core.engine.fights_first:FightsFirstRegistry.from_state"],
        ),
        (
            "unit-grants",
            "01.02",
            ...
            ["warhammer40k_core.engine.fights_first:fights_first_model_inventory"],
        ),
    ):
        ...
        review = {
            **shared,
            "evidence_id": f"core-fights-first-review:{slug}",
            ...
            "source_title": f"P24J {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-fights-first-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-fights-first-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "24.13 operative predicate and 01.02 unit-effect clause retained from the 40k.app "
            "search index; 24.13 additionally refers to the 12.04 Fight step. "
            "Direct fetch returned "
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P24J source artifact drift: {path.relative_to(ROOT)}")
        else:
