"""Build the reviewed Order 68 source artifacts offline."""
...
ARTIFACT_PATH = (
    ...
    / "core_roster_models_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/roster_models_2026_09_20.audit.json"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
PACKAGE_ID = "gw-11e-core-roster-models"
VERSION = "maintained-app-mirrors-observed-2026-09-20"
OBSERVED_AT = "2026-09-20T20:51:19Z"
AUDIT_ID = "core-roster-models-maintained-app-mirrors-2026-09-20"
FORTY_K_URL = "https://www.40k.app/rules/25-muster-armies"
REVIEWED_INPUT = ROOT / "data/source_audits/maintained_app_mirrors/order68-reviewed-input.json"
REVIEWED_INPUT_SHA256 = "e224a020233713b471e7d1077bc53ce0a7e14c8620bec6ffb858f44ac890a142"
...
def _reviewed_texts() -> dict[str, str]:
    ...
    if hashlib.sha256(raw).hexdigest() != REVIEWED_INPUT_SHA256:
        raise ValueError("Reviewed roster model source input drifted.")
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    for slug, source_text in _reviewed_texts().items():
        section = "25.04"
        consumers = [
            "warhammer40k_core.engine.roster_bearer_validation:append_warlord_violations"
            if slug == "warlord"
            else "warhammer40k_core.engine.roster_bearer_validation:append_enhancement_violations"
        ]
        ...
        review = {
            **shared,
            "evidence_id": f"core-roster-models-review:{slug}",
            ...
            "source_title": f"P25B {section} {slug}",
        ...
        for provider, url, version in providers:
            ...
            mirror = {
                **shared,
                "evidence_id": f"core-roster-models-mirror:{row_id}",
    ...
    payload: dict[str, object] = {
        "artifact_schema": "core-v2-core-roster-models-source-v1",
    ...
    return payload, {
        ...
        "co_version_comparison": (
            "25.04 Warlord and Enhancement model-selection clauses retained "
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"P25B source artifact drift: {path.relative_to(ROOT)}")
        else:
