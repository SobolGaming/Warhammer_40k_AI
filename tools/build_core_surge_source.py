"""Reproduce the reviewed Order 52 source package and observation audit offline."""
...
PACKAGE_ID = "gw-11e-core-surge"
VERSION = "maintained-app-mirrors-observed-2026-09-16"
OBSERVED_AT = "2026-09-16T00:00:00+00:00"
AUDIT_ID = "core-surge-maintained-app-mirrors-2026-09-16"
POLICY = "core-rules-source-policy:maintained-direct-app-data-mirrors:2026-09-02"
SOURCE_ID = f"{PACKAGE_ID}:surge-move"
TEXT = "Each model must end its move engaged with the surge target if possible."
URL = "https://www.40k.app/rules/21-flying-and-surging"
CONSUMERS = [
    "warhammer40k_core.engine.surge_movement:surge_ineligibility",
    "warhammer40k_core.engine.surge_movement:surge_endpoint_evidence",
    "warhammer40k_core.engine.surge_choices:surge_target_options",
    "warhammer40k_core.engine.phase_movement_history:surge_locked",
]
OBLIGATIONS = [
    "A granting rule must have triggered; the unit must be unengaged, not Battle-shocked "
    "and have made no move in this phase.",
    "The granting rule supplies the movement maximum; ordinary Moving restrictions apply.",
    "The player commits a closest enemy as the target before moving.",
    "Every model must engage that target where legally possible, otherwise minimize "
    "its distance to that target.",
    "The endpoint cannot engage any other enemy unit.",
    "Completing Surge prevents every subsequent move in this phase.",
]
ARTIFACT_PATH = ROOT / (
    "src/warhammer40k_core/rules/source_packages/warhammer_40000_11th/"
    "core_surge_2026_09/artifacts/package.json"
)
AUDIT_PATH = ROOT / "data/source_audits/maintained_app_mirrors/surge_2026_09_16.audit.json"
...
def build_payloads() -> tuple[dict[str, object], dict[str, object]]:
    ...
    audit: dict[str, object] = {
        ...
        "transcription_scope": (
            "Short excerpt; complete 21.02 operative text reviewed in the search index."
        ),
        "reviewed_obligations": OBLIGATIONS,
    }
    audit_hash = _hash(audit)
    row: dict[str, object] = {
        "evidence_id": "core-surge-mirror:surge-move",
        "rule_source_id": SOURCE_ID,
        ...
        "review_audit_row_id": "surge-move",
        "review_audit_source_observation_sha256": audit_hash,
        "provider_name": "40k.app",
        "source_title": "40k.app 21.02 Surge Move",
        "source_platform": "Web",
        "source_url": URL,
        "observed_at": OBSERVED_AT,
        "app_version": None,
    ...
    review = {
        **row,
        "evidence_id": "core-surge-review:surge-move",
    ...
    artifact: dict[str, object] = {
        "artifact_schema": "core-v2-core-surge-source-v1",
    ...
    for path, payload in zip((ARTIFACT_PATH, AUDIT_PATH), build_payloads(), strict=True):
        ...
        if args.check:
            if path.read_bytes() != raw:
                raise SystemExit(f"Order 52 source artifact drift: {path.relative_to(ROOT)}")
        else:
