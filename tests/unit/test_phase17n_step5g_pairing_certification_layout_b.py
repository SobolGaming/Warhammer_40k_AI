...
_LAYOUT_B_CERTIFICATION_ROWS = tuple(
    row
    for row in event_companion_pairing_lifecycle_certification_rows()
    if row.layout_variant == "b"
)


@pytest.mark.parametrize(
    "row",
    _LAYOUT_B_CERTIFICATION_ROWS,
    ids=tuple(row.layout_id for row in _LAYOUT_B_CERTIFICATION_ROWS),
)
@pytest.mark.parametrize("scoring_player_id", SCORING_PLAYER_IDS)
def test_phase17n_step5g_layout_b_scores_through_lifecycle_views_and_replay(
