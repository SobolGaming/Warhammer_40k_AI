...
def test_meatgrinder_real_attack_destruction_is_captured_and_scores_current_turn() -> None:
    ...
    sequence_id = "attack-sequence:phase17n-meatgrinder-runtime-attack"
    ...
    assert sum(transaction.amount for transaction in transactions) == 11
...
def test_reserve_deadline_lifecycle_restore_rejects_timeline_corruption(
    ...
    if corruption == "scoring-source":
        ...
        synchronize_secondary_destruction_projection_payload(
            state_payload=payload["state"],
            previous_primary_destruction_id=original_destruction_id,
            primary_destruction_payload=destruction,
        )
        recorded_event = next(
            event
...
def test_purge_and_secure_real_attack_from_objective_scores_through_lifecycle() -> None:
    ...
    transaction = next(
        ...
        if transaction.source_id == "primary-purge-and-secure"
    )
    ...
    projected_ledger = next(
        cast(dict[str, JsonValue], ledger)
        for ledger in projected_ledgers
        if cast(dict[str, JsonValue], ledger)["player_id"] == "player-a"
    )
    projected_transactions = cast(list[JsonValue], projected_ledger["transactions"])
    projected_transaction = next(
        cast(dict[str, JsonValue], transaction)
        for transaction in projected_transactions
        if cast(dict[str, JsonValue], transaction)["source_id"] == "primary-purge-and-secure"
    )
    assert projected_transaction["amount"] == 3
...
def test_return_on_death_same_unit_id_records_a_second_destruction_occurrence() -> None:
    ...
    secondary_occurrences = tuple(state.secondary_unit_destruction_states)
    assert len(secondary_occurrences) == 2
    assert {occurrence.destroyed_unit_instance_id for occurrence in secondary_occurrences} == {
        unit.unit_instance_id
    }
    assert {occurrence.battle_round for occurrence in secondary_occurrences} == {2}
    assert {occurrence.source_primary_destruction_id for occurrence in secondary_occurrences} == {
        first_destruction.destruction_id,
        second_destruction.destruction_id,
    }
...
def test_direct_tactical_scoring_does_not_record_or_discard_at_zero_capacity() -> None:
    ...
    for battle_round in range(1, 6):
        state.battle_round = battle_round
        mission_ids = (
            ("bring-it-down", "a-grievous-blow") if battle_round < 5 else ("bring-it-down",)
        )
        for secondary_mission_id in mission_ids:
            card = SecondaryMissionCardState.active_tactical(
                player_id="player-a",
                secondary_mission_id=secondary_mission_id,
                battle_round=battle_round,
                source_result_id=(
                    f"phase11e-direct-cap-round-{battle_round}:{secondary_mission_id}"
                ),
            )
            state.record_secondary_mission_card_state(card)
            scored = state.score_secondary_mission(
                player_id="player-a",
                secondary_mission_id=secondary_mission_id,
    ...
    assert state.victory_point_total("player-a") == 45
    target = SecondaryMissionCardState.active_tactical(
        player_id="player-a",
        secondary_mission_id="a-grievous-blow",
        battle_round=5,
        source_result_id="phase11e-direct-cap-zero-target",
    )
    state.record_secondary_mission_card_state(target)
    ...
    assert all(transaction.amount > 0 for transaction in ledger_before.transactions)
    assert (
        state.secondary_mission_card_state(
            player_id="player-a",
            secondary_mission_id="a-grievous-blow",
    ...
    award = policies.secondary_award(
        player_id="player-a",
        battle_round=state.battle_round,
        phase=BattlePhase.COMMAND.value,
        secondary_mission_id=target.secondary_mission_id,
    ...
    assert applied_amount == 0
    forged_ledger, zero_transaction = ledger_before.award(
        award,
        applied_amount=applied_amount,
        metadata=transaction_metadata,
    )
    state.victory_point_ledgers = [
        forged_ledger if ledger.player_id == "player-a" else ledger
        for ledger in state.victory_point_ledgers
    ]
    state.replace_secondary_mission_card_state(
        target.score(transaction_id=zero_transaction.transaction_id)
    )
    with pytest.raises(
        GameLifecycleError,
        match="Scored tactical secondary card requires a positive VP transaction",
    ):
        GameState.from_payload(state.to_payload())
...
def test_state_backed_secondary_scoring_closes_zero_award_primary_boundary_once(
    ...
    config = _config_with_player_b_vehicles(
        ("vehicle-unit-3",),
        mission_setup=_chapter_approved_immovable_object_mission_setup(),
    )
    lifecycle = GameLifecycle()
    lifecycle.start(config)
    state = _battle_state_from_config(
...
def test_bring_it_down_scores_each_destroyed_w10_model_and_caps_tactical() -> None:
    ...
    fixed_transaction = next(
        transaction
        for transaction in fixed_state.victory_point_ledger_for_player("player-a").transactions
        if transaction.source_id == "bring-it-down"
    )
    tactical_transaction = next(
        transaction
        for transaction in tactical_state.victory_point_ledger_for_player("player-a").transactions
        if transaction.source_id == "bring-it-down"
    )
    assert fixed_transaction.amount == 5
    assert tactical_transaction.amount == 5
    fixed_metadata = _transaction_metadata(fixed_transaction)
    tactical_metadata = _transaction_metadata(tactical_transaction)
...
def test_overwhelming_force_scores_destroyed_units_that_started_on_objectives_with_cap() -> None:
    ...
    center_marker = next(
        marker
        for marker in mission_setup.objective_markers
        if _objective_marker_matches_suffix(marker.objective_marker_id, "center-central")
    )
    upper_marker = next(
        marker
        for marker in mission_setup.objective_markers
        if _objective_marker_matches_suffix(marker.objective_marker_id, "upper-central")
    )
    state = _battle_state_from_config(
        _config_with_player_b_vehicles(
            ("vehicle-unit-3", "vehicle-unit-4"),
            mission_setup=mission_setup,
        ),
        player_a_secondary=SecondaryMissionMode.TACTICAL,
        turn_start_unit_positions=(
            (
                "army-beta:vehicle-unit-3",
                center_marker.x_inches,
                center_marker.y_inches,
            ),
            (
                "army-beta:vehicle-unit-4",
                upper_marker.x_inches,
                upper_marker.y_inches,
            ),
        ),
    )
    state.battle_phase_index = state.battle_phase_sequence.index(BattlePhase.FIGHT)
    ...
    transaction = next(
        transaction
        for transaction in state.victory_point_ledger_for_player("player-a").transactions
        if transaction.source_id == "overwhelming-force"
    )
    metadata = _transaction_metadata(transaction)
...
def test_no_prisoners_scores_each_destroyed_enemy_unit_with_cap() -> None:
    ...
    transaction = next(
        transaction
        for transaction in state.victory_point_ledger_for_player("player-a").transactions
        if transaction.source_id == "no-prisoners"
    )
    metadata = _transaction_metadata(transaction)
    assert transaction.amount == 5
...
def test_a_grievous_blow_scores_destroyed_starting_strength_thirteen_units() -> None:
    ...
    transaction = next(
        transaction
        for transaction in state.victory_point_ledger_for_player("player-a").transactions
        if transaction.source_id == "a-grievous-blow"
    )
    metadata = _transaction_metadata(transaction)
    assert transaction.amount == 5
...
def test_secure_no_mans_land_scores_two_no_mans_land_objectives_from_control_record() -> None:
    ...
    rule_evidence = evidence["secure-no-mans-land-tactical"]
    ...
    assert rule_evidence["score_count"] == 1
    assert set(cast(list[object], rule_evidence["controlled_objective_ids"])) == (
...
def test_secondary_scoring_evidence_payloads_round_trip_and_fail_fast() -> None:
    ...
    destruction = SecondaryUnitDestructionState(
        ...
        source_primary_destruction_id="primary-unit-destruction:test-vehicle-unit-3",
    ...
    self_destruction = SecondaryUnitDestructionState(
        ...
        source_primary_destruction_id="primary-unit-destruction:test-self",
        game_id="phase11e-game",
        destroying_player_id="player-a",
        destroyed_player_id="player-a",
        active_player_id="player-a",
    ...
    unattributed = SecondaryUnitDestructionState(
        ...
        source_primary_destruction_id="primary-unit-destruction:test-unattributed",
        game_id="phase11e-game",
        destroying_player_id=None,
        destroyed_player_id="player-b",
        active_player_id="player-a",
...
def test_game_state_secondary_scoring_evidence_round_trips_and_rejects_duplicates() -> None:
    ...
    missing_secondary_projection = deepcopy(payload)
    missing_secondary_projection["secondary_unit_destruction_states"] = []
    with pytest.raises(
        GameLifecycleError,
        match="one projection per Primary destruction occurrence",
    ):
        GameState.from_payload(missing_secondary_projection)
    duplicate_unit_state = state.secondary_unit_destruction_states[0]
    forged_primary_link = deepcopy(payload)
    forged_secondary = forged_primary_link["secondary_unit_destruction_states"]
    assert isinstance(forged_secondary, list)
    assert isinstance(forged_secondary[0], dict)
    forged_secondary[0]["source_primary_destruction_id"] = (
        "primary-unit-destruction:unknown-occurrence"
    )
    with pytest.raises(GameLifecycleError, match="lacks its Primary destruction occurrence"):
        GameState.from_payload(forged_primary_link)
...
def test_tactical_secondary_draw_score_discard_flow_is_public_after_reveal() -> None:
    ...
    victory_points_before_scoring = state.victory_point_total("player-a")
    scored = state.score_secondary_mission(
        player_id="player-a",
    ...
    score_transaction = next(
        transaction
        for transaction in state.victory_point_ledger_for_player("player-a").transactions
        if transaction.source_id == active_cards[0].secondary_mission_id
    )
    assert state.victory_point_total("player-a") == (
        victory_points_before_scoring + score_transaction.amount
    )
    ...
    transaction = next(
        cast(dict[str, JsonValue], transaction)
        for transaction in transactions
        if cast(dict[str, JsonValue], transaction)["source_id"]
        == active_cards[0].secondary_mission_id
    )
...
def test_phase14j_tactical_secondary_score_decision_can_score_or_retain_card() -> None:
    ...
    retain_points_before = retain_state.victory_point_total("player-a")
    ...
    assert retain_state.victory_point_total("player-a") == retain_points_before
    ...
    score_points_before = score_state.victory_point_total("player-a")
    ...
    assert score_state.victory_point_total("player-a") == score_points_before + 5
...
def test_local_session_exposes_held_mission_action_and_decline_continues_shooting() -> None:
    ...
    action_initial = action_session.lifecycle.to_payload()
    action_option = next(
        option
        for option in request.options
    ...
    for index in range(30):
        if action_state.active_player_id != "player-a":
            break
        assert rules_unit_started_mission_action_this_turn(
            state=action_state, player_id="player-a", unit_instance_id=started.unit_instance_id
        )
        pending = action_session.advance_until_decision_or_terminal().decision_request
        assert pending is not None
        if action_state.active_player_id != "player-a":
            break
        if pending.decision_type in {"select_shooting_unit", "select_charging_unit"}:
            assert started.unit_instance_id not in {option.option_id for option in pending.options}
        option = next(
            (
                value
                for value in pending.options
                if value.option_id.startswith("complete_")
                or value.option_id == DECLINE_MISSION_ACTION_START_OPTION_ID
                or "decline" in value.option_id
            ),
            pending.options[0],
        )
        action_session.submit_option(
            ...
            result_id=f"order34-action-turn:{index}",
        )
    assert action_state.active_player_id == "player-b"
    assert not rules_unit_started_mission_action_this_turn(
        state=action_state, player_id="player-a", unit_instance_id=started.unit_instance_id
    )
    checkpoint = action_session.to_persistence_payload()
    restored_action_session = LocalGameSession.from_persistence_payload(
        json.loads(json.dumps(checkpoint))
    )
    assert restored_action_session.to_persistence_payload() == checkpoint
    for viewer in ("player-a", "player-b"):
        assert restored_action_session.view(viewer_player_id=viewer) == action_session.view(
            viewer_player_id=viewer
        )
        assert restored_action_session.events_since(
            EventStreamCursor(), viewer_player_id=viewer
        ) == action_session.events_since(EventStreamCursor(), viewer_player_id=viewer)
    replay = ReplayRunner(
        ReplayArtifact.capture(
            artifact_id="order34-action-through-turn-end",
            initial_lifecycle_payload=action_initial,
            final_lifecycle=action_session.lifecycle,
        )
    ).run()
...
def test_order34_action_restriction_survives_phase_and_action_status(
            ...
            army_catalog=replace(
                ...
                datasheets=tuple(
                    replace(
                        row,
                        keywords=replace(
                            row.keywords, keywords=(*row.keywords.keywords, "TITANIC")
                        ),
                    )
    ...
    state.battle_phase_index = state.battle_phase_sequence.index(BattlePhase.SHOOTING)
    unit_id = "army-alpha:intercessor-unit-1"
    action = _start_mission_action_via_lifecycle(
    ...
    assert (
        shooting_unit_can_select_to_shoot(
            ...
            unit=restored_state.army_definitions[0].unit_by_id(unit_id),
    ...
    assert rules_unit_started_mission_action_this_turn(
        state=restored_state, player_id="player-a", unit_instance_id=unit_id
    )
...
def test_order34_activity_restriction_restore_rejects_forged_source_or_duration(
    ...
    if corruption == "schema":
        ...
        payload["state"]["persisting_effects"].append(extra)
    elif corruption == "target":
        effect["target_unit_instance_ids"] = [
            next(
                unit.unit_instance_id
                for unit in state.army_definitions[0].units
                if unit.unit_instance_id not in effect["target_unit_instance_ids"]
            )
        ]
    elif corruption == "player":
...
def test_attached_rules_unit_has_one_canonical_action_option_and_state_identity() -> None:
    ...
    # With no other eligible activation, the facade reaches the opponent's turn.
    assert state.active_player_id == "player-b"
    assert (
        mission_action_unit_ineligibility_reason(
            state=state,
...
def test_attached_rules_unit_canonical_shot_state_blocks_all_component_action_options() -> None:
    ...
    _complete_shooting_for_action_test(lifecycle, unit_instance_id=attached_id)
...
def test_attached_action_history_retains_identity_through_round_trip_and_terminal_replay(
    ...
    action = _start_mission_action_via_lifecycle(
        lifecycle=lifecycle,
        target_suffix="center",
        result_id="phase11e-attached-before-component-loss",
    ...
    retained = state.mission_action_state_by_id(action.action_id)
    assert retained.status is MissionActionStatus.STARTED
    assert retained.unit_instance_id == attached_id
    assert state.army_definitions[0].attached_units[0].attached_unit_instance_id == attached_id
    assert rules_unit_started_mission_action_this_turn(
        state=state,
        player_id="player-a",
        unit_instance_id=leader_id,
    ...
    for viewer_player_id in ("player-a", "player-b"):
        ...
        assert tuple(event_delta["events"]) == ()
    ...
    replay_initial_payload = cast(
        GameLifecyclePayload, json.loads(json.dumps(session.lifecycle.to_payload()))
    )
    session.submit_option(
        request_id=charge_request.request_id,
        option_id="complete_charge_phase",
        result_id="phase11e-complete-charge-after-attached-component-loss",
    )
    replay_result = ReplayRunner.from_payload(
        ReplayArtifact.capture(
            artifact_id="phase11e-attached-retained-action-history",
    ...
    terminal_artifact = ReplayArtifact.capture(
        artifact_id="phase11e-terminal-attached-retained-action-history",
        initial_lifecycle_payload=terminal_payload,
        final_lifecycle=terminal_lifecycle,
    )
    assert terminal_artifact.schema_version == "replay-artifact-v22-shock-engagement-history"
...
def test_order34_action_titanic_exception_uses_current_retained_component_keywords(
    retained: bool,
) -> None:
    ...
    config = replace(
        ...
        army_catalog=replace(
            ...
            datasheets=tuple(
                replace(
                    row,
                    keywords=replace(row.keywords, keywords=(*row.keywords.keywords, "TITANIC")),
                )
    ...
    _start_mission_action_via_lifecycle(
        lifecycle=lifecycle,
        target_suffix="center",
        result_id="order34-retained-action",
    ...
    retain_destroyed_model_for_fixture(
        state=state,
        placement=placement,
        effect_id="order34-retained",
        source_rule_id="order34:retention-source",
    ...
    if not retained:
        ...
        boundary = state.determine_current_phase_end_objective_control()
        lifecycle.decision_controller.event_log.append(
            "end_boundary_objective_control_determined",
            {
                "game_id": state.game_id,
        ...
        begin_retained_destruction_cleanup(
            state=state, decisions=lifecycle.decision_controller, reason="phase_end"
        )
    for unit_id in ("attached-unit:army-alpha:bodyguard-unit", "army-alpha:bodyguard-unit"):
        assert (
            mission_action_prevents_rules_unit_from_shooting_this_phase(
                state=state, player_id="player-a", unit_instance_id=unit_id
            )
            is not retained
        )
        assert rules_unit_started_mission_action_this_turn(
    ...
    assert not rules_unit_started_mission_action_this_turn(
        state=state, player_id="player-a", unit_instance_id="army-alpha:intercessor-unit-2"
    )
    restored = GameLifecycle.from_payload(json.loads(json.dumps(lifecycle.to_payload())))
    assert restored.state is not None
    assert restored.state.to_payload() == state.to_payload()
...
def test_mission_action_can_complete_interrupt_and_score() -> None:
    ...
    interrupted_lifecycle = _battle_lifecycle(
        player_a_fixed_mission_ids=("bring-it-down", "cleanse")
    )
    interrupted_state = interrupted_lifecycle.state
    assert interrupted_state is not None
    interrupted_state.battle_phase_index = interrupted_state.battle_phase_sequence.index(
        BattlePhase.SHOOTING
    )
    interrupted_action = _start_mission_action_via_lifecycle(
        lifecycle=interrupted_lifecycle,
...
def test_mission_action_start_excludes_units_that_shot_this_shooting_phase() -> None:
    ...
    _complete_shooting_for_action_test(lifecycle, unit_instance_id=unit_id)
...
def _complete_shooting_for_action_test(lifecycle: GameLifecycle, *, unit_instance_id: str) -> None:
    ...
    assert state is not None
    assert state.battlefield_state is not None
    target_id = state.army_definitions[1].units[0].unit_instance_id
    ...
    placement = state.battlefield_state.unit_placement_by_id(
        rules_unit.component_unit_instance_ids[0]
    )
    origin = placement.model_placements[0].pose.position
    state.battlefield_state = replace(state.battlefield_state, terrain_features=())
    _place_unit_near_point(
        state, unit_instance_id=target_id, x_inches=origin.x + 12.0, y_inches=origin.y
    )
    for unit in state.army_definitions[0].units:
        if unit.unit_instance_id not in rules_unit.component_unit_instance_ids:
            _place_unit_near_point(
                state,
                unit_instance_id=unit.unit_instance_id,
                x_inches=origin.x,
                y_inches=origin.y + 6.0,
            )
    session = LocalGameSession(lifecycle=lifecycle)
    for index in range(100):
        ...
        if (
            ...
            and unit_instance_id in shooting.shot_unit_ids
        ...
        if request.decision_type == START_MISSION_ACTION_DECISION_TYPE:
        ...
        elif request.decision_type == "select_shooting_type":
            option_id = "normal"
            ...
            status = session.submit_parameterized_payload(
                request_id=request.request_id,
                result_id=f"order34-fixture-shot:{index}",
        ...
        assert status.status_kind is not LifecycleStatusKind.INVALID, status
    raise AssertionError("Action fixture shooting did not complete within its decision budget")
...
def _authentic_primary_destruction_lifecycle_payload() -> GameLifecyclePayload:
    ...
    sequence_id = "attack-sequence:phase17n-destruction-integrity-runtime-attack"
    attack_context_id = f"{sequence_id}:pool-001:attack-001"
    sequence = AttackSequence.start(
        sequence_id=sequence_id,
        attacker_player_id="player-a",
...
def _config_with_player_b_vehicles(
    vehicle_unit_ids: tuple[str, ...],
    *,
    mission_setup: MissionSetup | None = None,
) -> GameConfig:
    catalog = _catalog_with_directional_force_dispositions()
...
def _record_secondary_unit_destruction(
    ...
    *,
    ...
    if started_turn_objective_marker_ids:
        if state.mission_setup is None:
            raise AssertionError("Secondary destruction fixture requires MissionSetup.")
        marker = next(
            marker
            for marker in state.mission_setup.objective_markers
            if marker.objective_marker_id == started_turn_objective_marker_ids[0]
        )
        _place_unit_near_point(
            state,
