...
GENERIC_DICE_ROLL_MODIFICATION_ENHANCEMENT_SOURCE_ROW_IDS = frozenset(
    {
        ...
        "enhancement:chaos-daemons:blood-legion:000009815004",
...
GENERIC_SELECTED_TO_FIGHT_ENHANCEMENT_SOURCE_ROW_IDS = frozenset(
    {
        blood_legion_ir.FURYS_CAGE_SOURCE_ROW_ID,
    }
)
GENERIC_ENHANCEMENT_SOURCE_ROW_IDS = frozenset(
    {
        ...
        *GENERIC_SELECTED_TO_FIGHT_ENHANCEMENT_SOURCE_ROW_IDS,
...
GENERIC_ENGINE_CONSUMED_ENHANCEMENT_RUNTIME_CONSUMERS_BY_SOURCE_ROW_ID = {
    blood_legion_ir.FURYS_CAGE_SOURCE_ROW_ID: blood_legion_ir.FURYS_CAGE_RUNTIME_CONSUMER_IDS,
...
ADEPTUS_CUSTODES_MARTIAL_KATAH_RUNTIME_CONSUMERS = (
...
ADEPTUS_MECHANICUS_DOCTRINA_IMPERATIVES_RUNTIME_CONSUMERS = (
...
FACTION_ARMY_RULE_RUNTIME_CONSUMERS_BY_FACTION_ID = {
    ...
    "adeptus-custodes": ADEPTUS_CUSTODES_MARTIAL_KATAH_RUNTIME_CONSUMERS,
    "adeptus-mechanicus": ADEPTUS_MECHANICUS_DOCTRINA_IMPERATIVES_RUNTIME_CONSUMERS,
    ...
    "thousand-sons": THOUSAND_SONS_CABAL_OF_SORCERERS_RUNTIME_CONSUMERS,
    "tyranids": TYRANIDS_SHADOW_IN_THE_WARP_RUNTIME_CONSUMERS,
...
def test_phase17e_loads_every_seeded_faction_and_detachment() -> None:
    ...
    for detachment_row in detachment_rows:
        ...
        if key in CHAOS_DAEMONS_DETACHMENT_RULE_RUNTIME_CONSUMERS_BY_KEY:
            ...
            generic_runtime_consumers = GENERIC_DETACHMENT_RULE_RUNTIME_CONSUMERS_BY_KEY.get(
                key,
                (),
            )
            assert coverage_row.status is Phase17ECoverageStatus.GENERIC_SUPPORTED
            if generic_runtime_consumers:
                assert coverage_row.runtime_support_status is not None
                assert coverage_row.runtime_support_status.value == "engine_consumed"
            else:
                assert coverage_row.runtime_support_status is None
            assert coverage_row.runtime_consumer_ids == tuple(sorted(generic_runtime_consumers))
...
@pytest.mark.parametrize(
    ("detachment_id", "rule_name", "runtime_consumers"),
    [
        ("blood-legion", "Blood Legion detachment rule", BLOOD_LEGION_RUNTIME_CONSUMERS),
        ("shadow-legion", "Shadow Legion detachment rule", ()),
    ],
)
def test_phase17e_chaos_daemons_detachment_rule_is_generic_rule_ir_supported(
    detachment_id: str,
    rule_name: str,
    runtime_consumers: tuple[str, ...],
) -> None:
