...
def _retain_only_placed_model(
    *,
    ...
    if battlefield is None:
        raise AssertionError("single-model fixture requires battlefield state")
...
def _validate_model_destruction_restore_payloads(
    *,
    state_payload: GameStatePayload,
    decisions_payload: DecisionControllerPayload,
) -> None:
    restored_state = GameState.from_payload(state_payload)
    restored_decisions = DecisionController.from_payload(decisions_payload)
    validate_model_destruction_cause_restore(
        state=restored_state,
        event_records=restored_decisions.event_log.records,
        decision_records=restored_decisions.records,
        pending_decision_requests=restored_decisions.queue.pending_requests,
    )
...
def test_phase13d_deferred_devastating_mortal_wounds_route_each_model_choice() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
        enemy_unit_specs=(
    ...
    assert {option.option_id for option in request.options} == {
        model.model_instance_id for model in target_a.own_models
    }
    assert tuple(
    ...
    drifted = GameLifecycle.from_payload(lifecycle.to_payload())
    drifted_state = _state(drifted)
    apply_damage_to_model(
        state=drifted_state,
        target_unit_instance_id=target_a.unit_instance_id,
        model_instance_id=target_a.own_models[0].model_instance_id,
    ...
    drift_checkpoint = drifted.to_payload()
    drifted_request = drifted.decision_controller.queue.peek_next()
    drift_status = drifted.submit_decision(
        DecisionResult.for_request(
            result_id="phase13d-dev-queue-stale-model-choice",
            request=drifted_request,
            selected_option_id=target_a.own_models[1].model_instance_id,
        )
    )
    assert drift_status.status_kind is LifecycleStatusKind.INVALID
    assert cast(dict[str, object], drift_status.payload) == {
        "invalid_reason": "invalid_mortal_wound_model_result",
        "field": "legal_model_ids",
    }
    assert drifted.to_payload() == drift_checkpoint

    fnp_status = restored.submit_decision(
        DecisionResult.for_request(
            result_id="phase13d-dev-queue-select-target-a-model",
            request=restored_request,
            selected_option_id=target_a.own_models[0].model_instance_id,
        )
    )
    fnp_request = _decision_request(fnp_status)
    assert fnp_request.decision_type == SELECT_FEEL_NO_PAIN_DECISION_TYPE
    assert fnp_request.options[0].option_id == "decline"

    target_b_status = restored.submit_decision(
        DecisionResult.for_request(
    ...
    target_b_request = _decision_request(target_b_status)
    assert target_b_request.decision_type == SELECT_MORTAL_WOUND_MODEL_DECISION_TYPE
    assert {option.option_id for option in target_b_request.options} == {
        model.model_instance_id for model in target_b.own_models
    }

    restored.submit_decision(
        DecisionResult.for_request(
            result_id="phase13d-dev-queue-select-target-b-model",
            request=target_b_request,
            selected_option_id=target_b.own_models[-1].model_instance_id,
        )
    )
...
def test_phase13d_mortal_wound_priority_tiers_cover_attached_characters() -> None:
    ...
    legal_model_ids, tier = mortal_wound_priority_selection(
        state=state,
        target_unit_instance_id=attached_id,
    )
    assert legal_model_ids == tuple(
        sorted(model.model_instance_id for model in bodyguard.own_models)
    )
    assert tier is MortalWoundAllocationPriority.NON_CHARACTER

    wounded_bodyguard = bodyguard.own_models[-1]
    apply_damage_to_model(
        state=state,
        target_unit_instance_id=attached_id,
        model_instance_id=wounded_bodyguard.model_instance_id,
        damage=1,
        damage_kind=DamageKind.NORMAL,
    )
    apply_damage_to_model(
    ...
    assert mortal_wound_priority_selection(
        ...
        (wounded_bodyguard.model_instance_id,),
    ...
    assert mortal_wound_priority_selection(
        ...
        MortalWoundAllocationPriority.CHARACTER,
    )
...
def test_phase14e_grouped_devastating_wounds_cap_each_critical_wound() -> None:
    ...
    remaining_sequence, status = _continue_damage_model_choices(
        ...
        status=status,
        result_id_prefix="phase14e-grouped-devastating-model",
    )
...
def test_phase14h_attached_target_uses_retained_component_geometry_but_living_allocation() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
        enemy_unit_specs=_attached_enemy_unit_specs(),
        enemy_attachment_declarations=_attached_enemy_declarations(),
    ...
    assert set(candidate.target_in_range_model_ids) <= {
        awaiting_model.model_instance_id,
        *living_character_model_ids,
    }
    assert set(candidate.target_visible_model_ids) <= {
        awaiting_model.model_instance_id,
        *living_character_model_ids,
    }
...
def test_phase13d_hazardous_tests_resolve_after_all_attacks(
    ...
    hazardous_spec = DiceRollSpec(
        expression=DiceExpression(quantity=1, sides=6),
        reason=f"Hazardous tests for {attacker.unit_instance_id} after shooting",
    ...
    remaining_sequence, _allocated_ids, status = _drain_damage_model_choices_with_manager(
        lifecycle=lifecycle,
        attack_sequence=remaining_sequence,
        allocated_ids=_allocated_ids,
        status=status,
    ...
    assert hazardous_payload["source_phase"] == BattlePhase.SHOOTING.value
    assert hazardous_payload["hazardous_weapon_instance_ids"] == [
        "weapon-instance:test:phase13d-hazardous"
    ]
    assert hazardous_payload["hazardous_weapon_profile_ids"] == ["phase13d-hazardous"]
    assert hazardous_payload["failed_hazardous_weapon_instance_ids"] == (
        [] if expected_successful else ["weapon-instance:test:phase13d-hazardous"]
    )
    ...
    mortal_wound_application = cast(dict[str, object], applied_payload["mortal_wound_application"])
    applications = cast(list[dict[str, object]], mortal_wound_application["applications"])
    assert hazardous_payload["mortal_wound_application"] is None
    assert hazardous_payload["pending_mortal_wound_request_id"] is not None
...
def test_phase14c_hazardous_mortal_wounds_route_optional_fnp_through_lifecycle() -> None:
    ...
    decisions_payload = cast(dict[str, Any], phase_drift["decisions"])
    pending_request = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], decisions_payload["queue"])["pending_requests"],
    )[0]
    pending_payload = cast(dict[str, Any], pending_request["payload"])
    ...
    source_context["source_phase"] = BattlePhase.FIGHT.value
    request_event = next(
        event
        for event in cast(list[dict[str, Any]], decisions_payload["event_log"])
        if event["event_type"] == "decision_requested"
    ...
    with pytest.raises(
        GameLifecycleError,
        match=r"Pending mortal-wound start authority drift|Hazardous.*phase drift",
    ):
        GameLifecycle.from_payload(phase_drift)
...
def test_p24d_hazardous_preserves_order_through_completion_and_restore(
    ...
    weapon_profile = replace(
        _first_weapon_profile(lifecycle, attacker),
        profile_id="p24d-shared-hazardous-profile",
        keywords=(WeaponKeyword.HAZARDOUS,),
    )
    ...
    roll_state = cast(dict[str, object], payload["roll_state"])
...
def test_phase13c_damage_mortal_wounds_and_feel_no_pain_hosts_round_trip() -> None:
    ...
    _retain_only_placed_model(
        ...
        unit=no_spill_defender,
        model_instance_id=no_spill_defender.own_models[0].model_instance_id,
    )
    ...
    _retain_only_placed_model(
        ...
        unit=overkill_defender,
        model_instance_id=overkill_defender.own_models[0].model_instance_id,
    )
...
def test_phase13e_destroyed_model_reaction_choice_records_removal_and_selection(
    ...
    assert state.battlefield_state is not None
    assert defender_model.model_instance_id in state.battlefield_state.placed_model_ids()
    assert not any(
        payload["model_instance_id"] == defender_model.model_instance_id
        for payload in _event_payloads(lifecycle, "model_destroyed")
    )
    selected_reaction_source = (
        shoot_source
        if selected_source_kind is DestructionReactionKind.SHOOT_ON_DEATH
        else fight_source
    )
    final_status = lifecycle.submit_decision(
        DecisionResult.for_request(
    ...
    selected = _last_event_payload(lifecycle, "fight_on_death_retention_selected")
    assert selected["selected_action"] == (
        "shoot" if expected_action_host == "shooting" else "fight"
    )
    session = LocalGameSession(lifecycle=lifecycle)
    for _ in range(40):
        assert state.battlefield_state is not None
        if defender_model.model_instance_id in state.battlefield_state.removed_model_ids:
            break
        pending = lifecycle.decision_controller.queue.peek_next()
        if pending.decision_type == "submit_shooting_declaration":
            proposal = _proposal_from_request(
                request=pending, target_unit_id=attacker.unit_instance_id
            )
            session.submit_parameterized_payload(
                request_id=pending.request_id,
                result_id="retained-test-shot",
                payload=cast(JsonValue, proposal.to_payload()),
            )
        else:
            submit_fixture_request(session, pending)
    else:
        raise AssertionError("Selected retained attack did not reach physical completion.")
    removed = _last_event_payload(lifecycle, "model_destroyed")
    assert (
        cast(dict[str, object], removed["removal_record"])["model_instance_id"]
...
def test_order_9_p05a_destruction_reaction_waits_for_attacking_unit_attacks() -> None:
    ...
    reaction_window_event = next(
        event for event in events if event.event_type == "fight_on_death_retention_opened"
    )
    assert not any(
        event.event_type in {"model_destroyed", "destruction_reaction_resolved"}
        and isinstance(event.payload, dict)
    ...
    with pytest.raises(
        GameLifecycleError,
        match=(
            r"Pending attack destruction "
            r"(lacks attacks-resolved evidence|boundary evidence drift)"
        ),
    ):
    ...
    apply_retention_selection(
        state=state, decisions=lifecycle.decision_controller, result=declined_record.result
    )
    suffix_sequence, suffix_status = resolve_pending_attack_destruction_until_blocked(
        state=state,
        decisions=lifecycle.decision_controller,
        manager=manager,
        attack_sequence=remaining,
    ...
    suffix_allocated_ids = _allocated_ids
...
def test_phase13e_fight_on_death_model_retains_presence_and_component_keywords() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
    ...
    assert rules_unit.keywords == defender.keywords
    assert rules_unit.faction_keywords == defender.faction_keywords
...
def _retain_attack_casualty_for_fight_on_death(
    *,
    ...
    if reduce_to_last_model:
        target_model = _reduce_unit_to_last_model_with_mortal_wounds(
            ...
            target_unit=defender,
            source_unit=attacker,
            application_id=f"{fixture_id}-preexisting-casualties",
        )
    else:
        ...
        _replace_unit_instance_in_state(
            ...
            replacement=replace(
                defender,
                own_models=(
                    replace(target_model, wounds_remaining=1),
                    *defender.own_models[1:],
                ),
            ),
        )
        target_model = replace(target_model, wounds_remaining=1)
    source = DestructionReactionSource(
        source_id=f"{fixture_id}-source",
        reaction_kind=DestructionReactionKind.FIGHT_ON_DEATH,
        source_rule_id=f"{fixture_id}-rule",
    )
    state.record_model_destruction_reaction_sources(
    ...
    remaining_sequence, allocated_ids, status = resolve_attack_sequence_until_blocked(
        ...
        dice_manager=DiceRollManager(
            ...
            event_log=lifecycle.decision_controller.event_log,
            injected_results=(
                _fixed_roll_result(roll_id=f"{fixture_id}-hit", spec=hit_spec, value=6),
                _fixed_roll_result(
                    roll_id=f"{fixture_id}-wound",
                    spec=wound_spec,
                    value=6,
                ),
                _fixed_roll_result(roll_id=f"{fixture_id}-save", spec=save_spec, value=1),
            ),
        ),
    )
    request = _decision_request(cast(LifecycleStatus, status))
    ...
    reaction_record = lifecycle.decision_controller.submit_result(
        DecisionResult.for_request(
            result_id=f"{fixture_id}-selected",
            request=request,
            selected_option_id=source.source_id,
        )
    )
    apply_retention_selection(
        state=state, decisions=lifecycle.decision_controller, result=reaction_record.result
    )
    continued_sequence, continued_allocated_ids, continued_status = (
        resolve_attack_sequence_until_blocked(
            state=state,
            decisions=lifecycle.decision_controller,
            ruleset_descriptor=_ruleset(),
    ...
    state.shooting_phase_state = shooting_state.with_attack_sequence_update(
        attack_sequence=continued_sequence,
        allocated_model_ids_this_phase=continued_allocated_ids,
    )
    ...
    return target_model.model_instance_id


def test_phase13e_fight_on_death_only_unit_is_targetable_without_new_damage() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1", "fight-on-death-source"),
        enemy_unit_specs=(
            (
    ...
    retained_id = _retain_attack_casualty_for_fight_on_death(
        lifecycle=lifecycle,
        attacker=units["fight-on-death-source"],
        defender=defender,
        fixture_id="phase13e-ranged-target-awaiting",
        reduce_to_last_model=True,
    )
    ...
    assert candidate.is_legal
    assert candidate.target_in_range_model_ids == (retained_id,)
    assert candidate.target_visible_model_ids == (retained_id,)
    ...
    proposal_request = cast(dict[str, object], request_payload["proposal_request"])
    target_candidates = cast(list[dict[str, object]], proposal_request["target_candidates"])
    assert defender.unit_instance_id in {
    ...
    proposal = _proposal_from_request(
        request=declaration_request, target_unit_id=defender.unit_instance_id
    )
    before_destroyed = _event_payloads(lifecycle, "model_logical_death_recorded")
    status = _submit_payload(
        lifecycle,
        request=declaration_request,
        payload=proposal.to_payload(),
    ...
    assert _event_payloads(lifecycle, "model_logical_death_recorded") == before_destroyed
...
def test_phase13e_mixed_fight_on_death_target_replays_geometry_and_living_allocation() -> None:
    ...
    attacker = units["intercessor-1"]
    defender = units["enemy"]
    awaiting_model_id = _retain_attack_casualty_for_fight_on_death(
        lifecycle=lifecycle,
        attacker=units["fight-on-death-source"],
        defender=defender,
        fixture_id="phase13e-mixed-target-awaiting",
        reduce_to_last_model=False,
    )
    scenario = battlefield_scenario_for_state(state=state)
    weapon_profile = _first_weapon_profile(lifecycle, attacker)
    candidate = shooting_target_candidates_for_unit(
    ...
    with pytest.raises(
        GameLifecycleError,
        match="Ranged target model inventory authority drift",
    ):
...
def test_phase14h_destroyed_transport_disembarks_before_removal_and_deadly_demise() -> None:
    ...
    emergency_destroyed_model = replace(passenger.own_models[-1], wounds_remaining=1)
    passenger = replace(
        passenger,
        own_models=(*passenger.own_models[:-1], emergency_destroyed_model),
    )
    _replace_unit_instance_in_state(state=state, replacement=passenger)
    battlefield = state.battlefield_state
    assert battlefield is not None
        ...
        dice_manager=DiceRollManager(
            ...
            injected_results=(
                ...
                *_destroyed_transport_hazard_roll_results_for_test(
                    ...
                    values=(6, 6, 6, 6, 1),
                    roll_id_prefix="phase14h-destroyed-transport-hazard",
                ),
            ),
        ),
    )
    ...
    assert not model_by_id(
        state=state,
        model_instance_id=emergency_destroyed_model.model_instance_id,
    ).is_alive
    assert request.actor_id == "player-b"
    ...
    assert proposal_context["surviving_model_instance_ids"] == [
        model.model_instance_id for model in passenger.own_models[:-1]
    ]
    hazard_context = proposal_context["hazard_rolls"]
    assert isinstance(hazard_context, dict)
    hazard_model_rolls = hazard_context["model_rolls"]
    assert isinstance(hazard_model_rolls, list)
    assert {
        cast(str, cast(dict[str, JsonValue], roll)["model_instance_id"])
        for roll in hazard_model_rolls
    ...
    forged_survivor_checkpoint = cast(
        GameLifecyclePayload,
        json.loads(json.dumps(lifecycle.to_payload(), sort_keys=True)),
    )
    forged_state = cast(dict[str, Any], forged_survivor_checkpoint["state"])
    forged_shooting = cast(dict[str, Any], forged_state["shooting_phase_state"])
    forged_sequence = cast(dict[str, Any], forged_shooting["attack_sequence"])
    forged_pending = cast(
        dict[str, Any],
        forged_sequence["pending_destroyed_transport_disembark"],
    )
    forged_survivors = cast(
        list[str], forged_pending["current_hazard_surviving_model_instance_ids"]
    )
    omitted_living_survivor = forged_survivors.pop()
    forged_decisions = cast(dict[str, Any], forged_survivor_checkpoint["decisions"])
    forged_queue = cast(dict[str, Any], forged_decisions["queue"])
    forged_request = cast(list[dict[str, Any]], forged_queue["pending_requests"])[0]
    forged_request_payload = cast(dict[str, Any], forged_request["payload"])
    forged_proposal_request = cast(dict[str, Any], forged_request_payload["proposal_request"])
    forged_context = cast(dict[str, Any], forged_proposal_request["context"])
    forged_context["surviving_model_instance_ids"] = list(forged_survivors)
    forged_request_event = next(
        event
        for event in cast(list[dict[str, Any]], forged_decisions["event_log"])
        if event["event_type"] == "decision_requested"
        and cast(dict[str, Any], event["payload"])["request_id"] == forged_request["request_id"]
    )
    forged_request_event["payload"] = json.loads(json.dumps(forged_request, sort_keys=True))
    assert model_by_id(
        state=state,
        model_instance_id=omitted_living_survivor,
    ).is_alive
    with pytest.raises(GameLifecycleError, match="exact hazard survivor inventory"):
        GameLifecycle.from_payload(forged_survivor_checkpoint)
    ...
    assert not any(
        payload["model_instance_id"] == emergency_destroyed_model.model_instance_id
        for payload in _event_payloads(lifecycle, "model_destroyed")
    )
    ...
    assert emergency_destroyed_model.model_instance_id in (
        restored_state.battlefield_state.removed_model_ids
    )


@pytest.mark.slow
def test_p18c_fully_destroyed_cargo_skips_its_placement_request() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
        game_id="p18c-fully-destroyed-cargo",
        enemy_unit_specs=(
            ("enemy-transport", "core-transport", "core-transport", 1),
            ("enemy-passenger-a", "core-character-leader", "core-character-leader", 1),
    ...
    pending = _destroyed_transport_pending_for_test(
        sequence_id="p18c-fully-destroyed-cargo",
        attacker=attacker,
        transport=transport,
        passenger=passenger_a,
    )
    survivor = passenger_a.own_models[0]
    passenger_a = replace(
        passenger_a,
        own_models=(replace(survivor, wounds_remaining=1),),
    )
    _replace_unit_instance_in_state(state=state, replacement=passenger_a)
    battlefield = state.battlefield_state
    assert battlefield is not None
    ...
    passenger_a_snapshot = _unit_placement_at(
        passenger_a,
        army_id="army-beta",
        player_id="player-b",
        poses=tuple(
            Pose.at(38.0 + (0.7 * index), 34.0 + (0.5 * index))
            for index, _model in enumerate(passenger_a.own_models)
        ),
    )
    passenger_a_snapshot = replace(
        passenger_a_snapshot,
        model_placements=tuple(
            model_placement
            for model_placement in passenger_a_snapshot.model_placements
            if model_placement.model_instance_id == survivor.model_instance_id
        ),
    )
    passenger_b_snapshot = _unit_placement_at(
        passenger_b,
        army_id="army-beta",
        player_id="player-b",
        poses=tuple(
            Pose.at(38.0 + (0.7 * index), 34.0 + (0.5 * index))
            for index, _model in enumerate(passenger_b.own_models)
        ),
    )
    ...
    updated_sequence, allocated_ids, status = _continue_pending_destroyed_transport_disembark(
        ...
        ruleset_descriptor=_ruleset(),
        manager=DiceRollManager(
            "p18c-fully-destroyed-cargo",
            event_log=lifecycle.decision_controller.event_log,
            injected_results=(
                *_destroyed_transport_hazard_roll_results_for_test(
                    passenger_a_snapshot,
                    values=(1,),
                    roll_id_prefix="p18c-passenger-a-hazard",
                ),
                *_destroyed_transport_hazard_roll_results_for_test(
                    passenger_b_snapshot,
                    values=(6, 6, 6, 6, 6),
                    roll_id_prefix="p18c-passenger-b-hazard",
                ),
            ),
        ),
        attack_sequence=sequence,
    ...
    assert allocated_ids == ()
    assert proposal_request.unit_instance_id == passenger_b.unit_instance_id
    assert (
        updated_sequence.pending_destroyed_transport_disembark.hazard_destroyed_unit_instance_ids
        == (passenger_a.unit_instance_id,)
    )
    assert survivor.model_instance_id in updated_battlefield.removed_model_ids
    assert not any(
        cast(dict[str, object], event.payload).get("unit_instance_id")
        == passenger_a.unit_instance_id
        for event in lifecycle.decision_controller.event_log.records
        if event.event_type == "destroyed_transport_disembark_placement_requested"
    )
    destroyed_before_placement = _last_event_payload(
        lifecycle,
        "destroyed_transport_cargo_destroyed_before_placement",
    )
    assert destroyed_before_placement["unit_instance_id"] == passenger_a.unit_instance_id
    passenger_a_destruction = next(
        destruction
        for destruction in state.primary_unit_destruction_states
        if destruction.destroyed_unit_instance_id == passenger_a.unit_instance_id
    )
    assert passenger_a_destruction.unattributed_cause is (
...
def test_p18c_attached_cargo_hazard_resolves_one_canonical_rules_unit_snapshot() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
        game_id="p18c-attached-cargo-hazard",
        enemy_unit_specs=(
            ("enemy-transport", "core-transport", "core-transport", 1),
            *_attached_enemy_unit_specs()[:2],
        ),
    ...
    assert battlefield is not None
    state.replace_battlefield_state(
        battlefield.without_unit_placement(bodyguard.unit_instance_id).without_unit_placement(
            leader.unit_instance_id
        )
    )
    component_ids = tuple(sorted((bodyguard.unit_instance_id, leader.unit_instance_id)))
    ...
    hazard_results = tuple(
        ...
        for result in _destroyed_transport_hazard_roll_results_for_test(
            ...
            values=tuple(6 for _model in component.own_models),
            roll_id_prefix=f"p18c-attached-{component.unit_instance_id}",
        )
    )

    manager = DiceRollManager(
        "p18c-attached-cargo-hazard",
        event_log=lifecycle.decision_controller.event_log,
        injected_results=hazard_results,
    )
    updated_sequence, allocated_ids, status = _continue_pending_destroyed_transport_disembark(
    ...
    updated_pending = updated_sequence.pending_destroyed_transport_disembark
    assert updated_pending is not None
    hazard_rolls = updated_pending.current_hazard_rolls
    assert hazard_rolls is not None
    assert allocated_ids == ()
    assert hazard_rolls.unit_instance_id == attached.unit_instance_id
    assert hazard_rolls.component_unit_instance_ids == component_ids
    assert hazard_rolls.model_instance_ids == tuple(
        sorted(model.model_instance_id for model in attached.alive_models())
    )
    assert request.decision_type == PLACEMENT_PROPOSAL_DECISION_TYPE
...
def test_phase14h_pending_destroyed_transport_state_round_trips_and_rejects_drift() -> None:
    ...
    assert hazard_rolls is not None
    with pytest.raises(GameLifecycleError, match="current hazard rolls are invalid"):
        replace(pending, current_hazard_rolls=cast(Any, object()))
    with pytest.raises(GameLifecycleError, match="appears in both states"):
        replace(
            pending,
            hazard_destroyed_unit_instance_ids=(passenger.unit_instance_id,),
        )
    with pytest.raises(GameLifecycleError, match="survivors require current hazard rolls"):
        replace(pending, current_hazard_rolls=None)
    with pytest.raises(GameLifecycleError, match="current hazard unit drift"):
        replace(
            pending,
            current_hazard_rolls=replace(
                hazard_rolls,
                unit_instance_id="unit:forged",
                component_unit_instance_ids=("unit:forged",),
            ),
        )
    with pytest.raises(GameLifecycleError, match="current hazard transport drift"):
        replace(
            pending,
            current_hazard_rolls=replace(
                hazard_rolls,
                transport_unit_instance_id="transport:forged",
            ),
        )
    with pytest.raises(GameLifecycleError, match="survivor inventory drift"):
        replace(
            pending,
            current_hazard_surviving_model_instance_ids=("model:forged",),
        )
    with pytest.raises(GameLifecycleError, match="hazard rolls are invalid"):
        pending.with_current_hazard_rolls(cast(Any, object()))
    with pytest.raises(GameLifecycleError, match="hazard rolls already exist"):
        pending.with_current_hazard_rolls(hazard_rolls)
    pending_without_hazard = replace(
        pending,
        current_hazard_rolls=None,
        current_hazard_surviving_model_instance_ids=None,
    )
    with pytest.raises(GameLifecycleError, match="survivors require hazard rolls"):
        pending_without_hazard.with_current_hazard_survivors(())
    with pytest.raises(GameLifecycleError, match="hazard survivors already exist"):
        pending.with_current_hazard_survivors(())
    with pytest.raises(GameLifecycleError, match="destroyed cargo requires zero survivors"):
        pending.with_hazard_destroyed_current_unit()
    hazard_destroyed = replace(
        pending,
        current_hazard_surviving_model_instance_ids=(),
    ).with_hazard_destroyed_current_unit()
    assert hazard_destroyed.pending_unit_instance_ids == ()
    assert hazard_destroyed.hazard_destroyed_unit_instance_ids == (passenger.unit_instance_id,)
...
def test_phase14h_destroyed_transport_proposal_prevalidation_rejects_invalid_payloads() -> None:
    ...
    assert _key_error_field(KeyError("field-a", "field-b")) == "payload"
    assert _key_error_field(KeyError("")) == "payload"
    parse_failures = (
        (GameLifecycleError("proposal_kind drift"), "proposal_kind"),
        (GameLifecycleError("disembark_mode drift"), "disembark_mode"),
        (
            GameLifecycleError("transport_movement_status drift"),
            "transport_movement_status",
        ),
        (
            GameLifecycleError("transport_unit_instance_id drift"),
            "transport_unit_instance_id",
        ),
        (GameLifecycleError("attempted placement drift"), "attempted_placement"),
    )
    for error, expected_field in parse_failures:
        assert (
            _destroyed_transport_proposal_parse_failure(
                proposal_request=base_proposal_request,
                error=error,
            )
            .violations[0]
            .field
    ...
    parsed_submission = PlacementProposalPayload.from_payload(cast(Any, valid_payload))
    assert (
        _missing_destroyed_transport_disembark_field(
            replace(parsed_submission, disembark_mode=None)
        )
        == "disembark_mode"
    )
    assert (
        _missing_destroyed_transport_disembark_field(
            replace(parsed_submission, transport_movement_status=None)
        )
        == "transport_movement_status"
    )

    hazard_context_drift_request = replace(
        base_proposal_request,
        request_id="phase14h-destroyed-transport-hazard-context-drift",
        context={
            **dict(base_proposal_request.context or {}),
            "hazard_rolls": None,
        },
    ).to_decision_request()
    hazard_context_drift_payload = _destroyed_transport_placement_payload_for_test(
        proposal_request=MovementProposalRequest.from_decision_request_payload(
            hazard_context_drift_request.payload
        ),
        unit=passenger,
        transport=transport,
    )
    hazard_context_drift_status = invalid_destroyed_transport_disembark_proposal_status(
        state=state,
        request=hazard_context_drift_request,
        result=_proposal_decision_result(
            request=hazard_context_drift_request,
            payload=hazard_context_drift_payload,
            result_id="phase14h-destroyed-transport-hazard-context-drift",
        ),
        decisions=lifecycle.decision_controller,
        attack_sequence=sequence,
    )
    assert hazard_context_drift_status is not None
    _assert_invalid_proposal_status(
        hazard_context_drift_status,
        expected_code="destroyed_transport_hazard_context_drift",
        expected_field="context",
    )
    ...
    with pytest.raises(GameLifecycleError, match="completed hazard survivors"):
        _request_destroyed_transport_disembark_placement(
            state=state,
            decisions=lifecycle.decision_controller,
            attack_sequence=base_sequence.with_pending_destroyed_transport_disembark(
                pending_without_hazard
            ),
            pending=pending_without_hazard,
        )
    state.transport_cargo_states = []
    with pytest.raises(GameLifecycleError, match="require current cargo state"):
        _continue_pending_destroyed_transport_disembark(
            state=state,
            decisions=lifecycle.decision_controller,
            ruleset_descriptor=_ruleset(),
            manager=DiceRollManager("phase14h-p18c-missing-cargo"),
            attack_sequence=base_sequence.with_pending_destroyed_transport_disembark(
                pending_without_hazard
            ),
            allocated_model_ids=(),
    ...
    survivors = pending.current_hazard_surviving_model_instance_ids
    ...
    with pytest.raises(GameLifecycleError, match="survivor inventory drift"):
        _resolve_destroyed_transport_disembark_submission(
            ...
            pending=replace(
                pending,
                current_hazard_surviving_model_instance_ids=tuple(
                    sorted((*survivors, "model:forged"))
                ),
            ),
            submission=parsed_submission,
        )
    ...
    survivor_ids = tuple(sorted(model.model_instance_id for model in passenger.own_models)[:-1])
    casualty_pending = replace(
        pending,
        current_hazard_surviving_model_instance_ids=survivor_ids,
    )
    casualty_sequence = replace(
        sequence,
        pending_destroyed_transport_disembark=casualty_pending,
    )
    casualty_request = _destroyed_transport_proposal_request_for_test(
        state=state,
        pending=casualty_pending,
        sequence=casualty_sequence,
        unit_instance_id=passenger.unit_instance_id,
        request_id="phase14h-destroyed-transport-hazard-casualty",
    )
    casualty_payload = _destroyed_transport_placement_payload_for_test(
        proposal_request=MovementProposalRequest.from_decision_request_payload(
            casualty_request.payload
        ),
        unit=passenger,
        transport=transport,
    )
    casualty_status = invalid_destroyed_transport_disembark_proposal_status(
        state=state,
        request=casualty_request,
        result=_proposal_decision_result(
            request=casualty_request,
            payload=casualty_payload,
            result_id="phase14h-destroyed-transport-hazard-casualty",
        ),
        decisions=lifecycle.decision_controller,
        attack_sequence=casualty_sequence,
    )
...
def test_phase14h_destroyed_transport_requests_each_embarked_unit_before_removal() -> None:
    ...
    lifecycle.decision_controller.submit_result(result)
    passenger_b_hazard_snapshot = _unit_placement_at(
        passenger_b,
        army_id="army-beta",
        player_id="player-b",
        poses=tuple(
    ...
    updated_sequence, allocated_ids, status = apply_destroyed_transport_disembark_proposal_decision(
        ...
        dice_manager=DiceRollManager(
            ...
            injected_results=_destroyed_transport_hazard_roll_results_for_test(
                passenger_b_hazard_snapshot,
                values=(6, 6, 6, 6, 6),
                roll_id_prefix="phase14h-destroyed-transport-passenger-b",
            ),
        ),
    )
...
def test_phase13e_successful_deadly_demise_applies_mortal_wounds_before_removal() -> None:
    ...
    source_owner_drift = cast(
        GameLifecyclePayload,
        json.loads(json.dumps(lifecycle.to_payload(), sort_keys=True)),
    )
    child_authority = next(
        authority
        for authority in source_owner_drift["state"]["model_destruction_cause_authorities"]
        if cast(dict[str, Any], authority["producer_context"])["destruction_attribution"][
            "destruction_provenance"
        ]["destruction_source_kind"]
        == DestructionSourceKind.DEADLY_DEMISE.value
    )
    child_context = cast(dict[str, object], child_authority["producer_context"])
    child_attribution = cast(dict[str, object], child_context["destruction_attribution"])
    child_attribution["destroying_player_id"] = "player-a"
    stored_destroyed_event = cast(dict[str, object], child_authority["model_destroyed_event"])
    stored_destroyed_payload = cast(dict[str, object], stored_destroyed_event["payload"])
    stored_destroyed_payload["destroying_player_id"] = "player-a"
    canonical_destroyed_event = next(
        event
        for event in source_owner_drift["decisions"]["event_log"]
        if event["event_id"] == stored_destroyed_event["event_id"]
    )
    canonical_destroyed_payload = cast(dict[str, object], canonical_destroyed_event["payload"])
    canonical_destroyed_payload["destroying_player_id"] = "player-a"

    with pytest.raises(
        GameLifecycleError,
        match="Destruction source rules unit must belong to the destroying player",
    ):
        GameLifecycle.from_payload(source_owner_drift)
...
def test_phase13e_deadly_demise_fnp_pauses_before_destroyed_model_removal() -> None:
    ...
    status = lifecycle.submit_decision(
        DecisionResult.for_request(
            result_id="phase13e-fnp-deadly-demise-select-model",
            request=request,
            selected_option_id=attacker_model.model_instance_id,
        )
    )
    request = _decision_request(status)
...
@pytest.mark.parametrize(
    ("secondary_reaction_kind", "source_phase"),
    [
        pytest.param(
...
def test_phase13e_deadly_demise_secondary_casualty_gets_removal_record_and_reaction(
    secondary_reaction_kind: DestructionReactionKind,
    ...
    state = _state(lifecycle)
    assert source_phase is BattlePhase.SHOOTING
    attacker = units["intercessor-1"]
    collateral = units["intercessor-2"]
    defender = units["enemy"]
    collateral_model = collateral.own_models[0]
    battlefield = state.battlefield_state
    assert battlefield is not None
    state.battlefield_state = battlefield.with_unit_placement(
        _unit_placement_at(
            ...
            player_id="player-a",
            poses=_compact_test_unit_poses(
                origin=Pose.at(0.0, 35.0),
                model_count=len(attacker.own_models),
            ),
        )
    ).with_unit_placement(
        _unit_placement_at(
            ...
            player_id="player-a",
            poses=_compact_test_unit_poses(
                origin=Pose.at(18.0, 35.0),
                model_count=len(collateral.own_models),
            ),
        )
    )
    defender_model = _reduce_unit_to_last_model_with_mortal_wounds(
    ...
    secondary_reaction_source = DestructionReactionSource(
        source_id=f"phase13e-secondary-{secondary_reaction_kind.value}",
        reaction_kind=secondary_reaction_kind,
        source_rule_id=f"phase13e-secondary-{secondary_reaction_kind.value}-rule",
    )
    state.record_model_destruction_reaction_sources(
        model_instance_id=collateral_model.model_instance_id,
    ...
    status = lifecycle.submit_decision(
        DecisionResult.for_request(
            result_id="phase13e-secondary-casualty-select-model",
            request=request,
            selected_option_id=collateral_model.model_instance_id,
        )
    )
    request = _decision_request(status)
    shooting_state = state.shooting_phase_state
    assert shooting_state is not None
    remaining_sequence = shooting_state.attack_sequence
    allocated_ids = shooting_state.allocated_model_ids_this_phase
    assert state.battlefield_state is not None
    assert collateral_model.model_instance_id in state.battlefield_state.placed_model_ids()
    assert not any(
        payload["model_instance_id"] == collateral_model.model_instance_id
        for payload in _event_payloads(lifecycle, "model_destroyed")
    )
    assert request.decision_type == SELECT_DESTRUCTION_REACTION_DECISION_TYPE
    assert request.actor_id == "player-a"
    ...
    pending_checkpoint = lifecycle.to_payload()
    _validate_model_destruction_restore_payloads(
        state_payload=pending_checkpoint["state"], decisions_payload=pending_checkpoint["decisions"]
    )
    status = lifecycle.submit_decision(
        DecisionResult.for_request(
            request=request,
            result_id="decline-secondary-retained-attack",
            selected_option_id=DECLINE_DESTRUCTION_REACTION_OPTION_ID,
        )
    )
    assert status.status_kind is not LifecycleStatusKind.INVALID
    ...
    assert attribution.destroying_player_id == "player-b"
    assert attribution.source_rules_unit_instance_id == defender.unit_instance_id
    assert attribution.source_model_instance_id == defender_model.model_instance_id
    removal = cast(dict[str, object], secondary[0]["removal_record"])
    assert removal["model_instance_id"] == collateral_model.model_instance_id
    assert removal["removal_kind"] == "destroyed"
    completed = lifecycle.to_payload()
    _validate_model_destruction_restore_payloads(
        state_payload=completed["state"], decisions_payload=completed["decisions"]
    )
    ...
    start_after_collateral_logical = cast(
        dict[str, Any],
        json.loads(json.dumps(checkpoint, sort_keys=True)),
    )
    reordered_collateral_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], start_after_collateral_logical["decisions"])["event_log"],
    )
    reordered_collateral_start = next(
        event
        for event in reordered_collateral_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
        and cast(dict[str, Any], event["payload"])["target_unit_instance_id"]
        == collateral.unit_instance_id
    )
    reordered_collateral_logical = next(
        event
        for event in reordered_collateral_events
        if event["event_type"] == MODEL_LOGICAL_DEATH_RECORDED_EVENT
        and cast(dict[str, Any], event["payload"])["model_instance_id"]
        == collateral_model.model_instance_id
    )
    (
        reordered_collateral_start["event_type"],
        reordered_collateral_logical["event_type"],
    ) = (
        reordered_collateral_logical["event_type"],
        reordered_collateral_start["event_type"],
    )
    reordered_collateral_start["payload"], reordered_collateral_logical["payload"] = (
        reordered_collateral_logical["payload"],
        reordered_collateral_start["payload"],
    )
    reordered_collateral_authority = next(
        authority
        for authority in cast(
            list[dict[str, Any]],
            cast(dict[str, Any], start_after_collateral_logical["state"])[
                "model_destruction_cause_authorities"
            ],
        )
        if authority["model_instance_id"] == collateral_model.model_instance_id
    )
    reordered_collateral_authority["logical_death_event"] = json.loads(
        json.dumps(reordered_collateral_start, sort_keys=True)
    )
    with pytest.raises(
        GameLifecycleError,
        match=r"start must precede completed logical death|logical-death identity drift",
    ):
        _validate_model_destruction_restore_payloads(
            state_payload=cast(GameStatePayload, start_after_collateral_logical["state"]),
            decisions_payload=cast(
                DecisionControllerPayload,
                start_after_collateral_logical["decisions"],
            ),
        )

    def _collateral_checkpoint_copy() -> dict[str, Any]:
        return cast(dict[str, Any], json.loads(json.dumps(checkpoint, sort_keys=True)))

    def _collateral_finalization_event(payload: dict[str, Any]) -> dict[str, Any]:
        events = cast(
            list[dict[str, Any]],
            cast(dict[str, Any], payload["decisions"])["event_log"],
        )
        return next(
            event
            for event in events
            if event["event_type"] == MORTAL_WOUND_MODEL_DESTRUCTIONS_FINALIZED_EVENT
            and cast(dict[str, Any], event["payload"])["finalization_kind"]
            == MortalWoundDestructionFinalizationKind.DEADLY_DEMISE_COLLATERAL_CAUSE.value
        )

    missing_finalization = _collateral_checkpoint_copy()
    _collateral_finalization_event(missing_finalization)["event_type"] = (
        "tampered_deadly_demise_collateral_finalization"
    )
    with pytest.raises(GameLifecycleError, match="lacks exactly one continuation"):
        _validate_model_destruction_restore_payloads(
            state_payload=cast(GameStatePayload, missing_finalization["state"]),
            decisions_payload=cast(DecisionControllerPayload, missing_finalization["decisions"]),
        )

    duplicate_finalization = _collateral_checkpoint_copy()
    duplicate_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], duplicate_finalization["decisions"])["event_log"],
    )
    duplicate_event = json.loads(
        json.dumps(_collateral_finalization_event(duplicate_finalization), sort_keys=True)
    )
    duplicate_event["event_id"] = f"event-{len(duplicate_events) + 1:06d}"
    duplicate_events.append(duplicate_event)
    with pytest.raises(GameLifecycleError, match="lacks exactly one continuation"):
        _validate_model_destruction_restore_payloads(
            state_payload=cast(GameStatePayload, duplicate_finalization["state"]),
            decisions_payload=cast(DecisionControllerPayload, duplicate_finalization["decisions"]),
        )

    wrong_finalization_kind = _collateral_checkpoint_copy()
    wrong_kind_payload = cast(
        dict[str, Any],
        _collateral_finalization_event(wrong_finalization_kind)["payload"],
    )
    wrong_kind_payload["finalization_kind"] = (
        MortalWoundDestructionFinalizationKind.APPLICATION_PACKET.value
    )
    with pytest.raises(GameLifecycleError):
        _validate_model_destruction_restore_payloads(
            state_payload=cast(GameStatePayload, wrong_finalization_kind["state"]),
            decisions_payload=cast(DecisionControllerPayload, wrong_finalization_kind["decisions"]),
        )

    drifted_finalization = _collateral_checkpoint_copy()
    drifted_payload = cast(
        dict[str, Any],
        _collateral_finalization_event(drifted_finalization)["payload"],
    )
    drifted_source_context = cast(dict[str, Any], drifted_payload["source_context"])
    drifted_source_context["model_destroyed_event_id"] = "event-999999"
    with pytest.raises(GameLifecycleError, match="finalization binding drift"):
        _validate_model_destruction_restore_payloads(
            state_payload=cast(GameStatePayload, drifted_finalization["state"]),
            decisions_payload=cast(DecisionControllerPayload, drifted_finalization["decisions"]),
        )
...
def test_phase13e_deadly_demise_secondary_deadly_demise_chains_before_removal() -> None:
    ...
    remaining_sequence, allocated_ids, status = _drain_damage_model_choices_with_manager(
        ...
        status=status,
        dice_manager=dice_manager,
        result_id_prefix="phase13e-secondary-chain-model",
    )
    event_records = lifecycle.decision_controller.event_log.records
...
def test_phase13e_destruction_reaction_invalid_submission_does_not_mutate_queue() -> None:
    ...
    reaction_payload = _last_event_payload(lifecycle, "fight_on_death_retention_selected")

    _assert_waiting_for_movement_unit(decline_status)
    assert reaction_payload["selected_source_id"] is None
    assert reaction_payload["selected_action"] is None
    assert reaction_payload["stage"] == "declined"
...
def test_phase13d_direct_mortal_wound_helper_rejects_choice_routing_contexts() -> None:
    ...
    assert (state.to_payload(), lifecycle.decision_controller.to_payload()) == (
        invalid_route_checkpoint
    )

    model_choice_checkpoint = (
        state.to_payload(),
        lifecycle.decision_controller.to_payload(),
    )
    with pytest.raises(GameLifecycleError, match="model choices require lifecycle routing"):
        apply_mortal_wounds_to_unit(
            state=state,
            decisions=lifecycle.decision_controller,
            application_id="phase13d-model-choice-direct-mortal",
            source_rule_id="phase13d:test-mortal-wounds",
            source_context={"source_kind": "phase13d_model_choice_test"},
            destruction_evidence=_test_mortal_wound_destruction_evidence(state),
            target_unit_instance_id=defender.unit_instance_id,
            mortal_wounds=1,
        )
...
def test_phase13d_multi_wound_mortal_destruction_round_trips_repeated_model_rows() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
        game_id="phase13d-multi-wound-mortal-destruction-restore",
    )
    state = _state(lifecycle)
    defender = units["enemy"]
    defender_model = defender.own_models[0]
    ...
    application = apply_mortal_wounds_to_unit(
        state=state,
        decisions=lifecycle.decision_controller,
        application_id="phase13d-multi-wound-mortal-destruction",
        source_rule_id="phase13d:test-multi-wound-mortal-destruction",
    ...
    assert _state(restored).to_payload() == checkpoint["state"]
    assert restored.decision_controller.to_payload() == checkpoint["decisions"]


def test_phase13d_multi_model_mortal_fnp_pause_persists_first_logical_death_boundary() -> None:
    lifecycle, units = _shooting_lifecycle(
        alpha_unit_ids=("intercessor-1",),
        game_id="phase13d-multi-model-mortal-fnp-logical-death",
    )
    state = _state(lifecycle)
    defender = units["enemy"]
    first_model, second_model = defender.own_models[:2]
    _replace_unit_instance_in_state(
        state=state,
        replacement=replace(
            defender,
            own_models=(
                replace(first_model, wounds_remaining=1),
                *defender.own_models[1:],
            ),
        ),
    )
    source_a = FeelNoPainSource(
        source_id="phase13d-multi-model-mortal-fnp-a",
        threshold=5,
    )
    source_b = FeelNoPainSource(
        source_id="phase13d-multi-model-mortal-fnp-b",
        threshold=6,
    )
    state.record_model_feel_no_pain_sources(
    ...
    progress = MortalWoundApplicationProgress.start(
        application_id="phase13d-multi-model-mortal-fnp",
        source_rule_id="phase13d:multi-model-mortal-fnp",
        source_context={"source_kind": "phase13d_multi_model_mortal_fnp"},
        target_unit_instance_id=defender.unit_instance_id,
        defender_player_id="player-b",
        mortal_wounds=2,
        spill_over=True,
    ...
    routing = continue_mortal_wound_application(
        state=state,
        decisions=lifecycle.decision_controller,
        request_id="phase13d-multi-model-mortal-fnp-request",
        progress=progress,
    )
    ...
    lifecycle.decision_controller.request_decision(routing.request)
    model_request = routing.request
    model_result = DecisionResult.for_request(
        result_id="phase13d-multi-model-mortal-select-second",
        request=model_request,
        selected_option_id=second_model.model_instance_id,
    )
    lifecycle.decision_controller.submit_result(model_result)
    routing = resolve_mortal_wound_decision(
        ...
        request=model_request,
        result=model_result,
        next_request_id="phase13d-multi-model-mortal-fnp-request",
    )
    assert routing.request is not None
    assert is_mortal_wound_feel_no_pain_request(routing.request)
    lifecycle.decision_controller.request_decision(routing.request)
    assert (
        model_by_id(
            state=state,
    ...
    assert state.battlefield_state is not None
    assert first_model.model_instance_id not in state.battlefield_state.placed_model_ids()
    request_payload = cast(dict[str, JsonValue], routing.request.payload)
    pending_progress = MortalWoundApplicationProgress.from_feel_no_pain_context(
        request_payload["lost_wound_context"]
    )
    assert len(pending_progress.logical_death_events) == 1
    first_logical_death_event = pending_progress.logical_death_events[0]
    first_logical_death = model_logical_death_record_from_event(first_logical_death_event)
    assert first_logical_death.model_instance_id == first_model.model_instance_id
    assert not first_logical_death.placement_retained
    assert not state.model_destruction_cause_authorities
    records = lifecycle.decision_controller.event_log.records
    event_indexes = {event.event_id: index for index, event in enumerate(records)}
    started_events = tuple(
        event for event in records if event.event_type == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )
    assert len(started_events) == 1
    started_event = started_events[0]
    fnp_request_index = next(
        index
        for index, event in enumerate(records)
        if event.event_type == "decision_requested"
        and cast(dict[str, JsonValue], event.payload)["request_id"] == routing.request.request_id
    )
    assert event_indexes[started_event.event_id] < event_indexes[first_logical_death_event.event_id]
    assert event_indexes[first_logical_death_event.event_id] < fnp_request_index
    assert all(
        not (
            event.event_type == "model_destroyed"
            and cast(dict[str, JsonValue], event.payload)["model_instance_id"]
            == first_model.model_instance_id
        )
    ...
    def _checkpoint_copy() -> dict[str, Any]:
        return cast(dict[str, Any], json.loads(json.dumps(checkpoint, sort_keys=True)))

    missing_start = _checkpoint_copy()
    missing_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], missing_start["decisions"])["event_log"],
    )
    next(
        event
        for event in missing_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )["event_type"] = "tampered_mortal_wound_application_started"
    with pytest.raises(GameLifecycleError, match="lacks its start authority"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, missing_start))

    duplicate_start = _checkpoint_copy()
    duplicate_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], duplicate_start["decisions"])["event_log"],
    )
    ...
    duplicate_authority["event_id"] = f"event-{len(duplicate_events) + 1:06d}"
    duplicate_events.append(duplicate_authority)
    with pytest.raises(GameLifecycleError, match="authority is duplicated"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, duplicate_start))

    reordered_start = _checkpoint_copy()
    reordered_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], reordered_start["decisions"])["event_log"],
    )
    reordered_authority = next(
        event
        for event in reordered_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )
    reordered_request = next(
        event
        for event in reordered_events
        if event["event_type"] == "decision_requested"
        and cast(dict[str, Any], event["payload"])["request_id"] == routing.request.request_id
    )
    reordered_authority["event_type"], reordered_request["event_type"] = (
        reordered_request["event_type"],
        reordered_authority["event_type"],
    )
    reordered_authority["payload"], reordered_request["payload"] = (
        reordered_request["payload"],
        reordered_authority["payload"],
    )
    with pytest.raises(GameLifecycleError, match="must precede logical death"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, reordered_start))

    def _rewrite_pending_logical_death(
        payload: dict[str, Any],
        *,
        game_id: str,
        producer_id: str,
    ) -> None:
        decisions_payload = cast(dict[str, Any], payload["decisions"])
        queue_payload = cast(dict[str, Any], decisions_payload["queue"])
        pending_requests = cast(list[dict[str, Any]], queue_payload["pending_requests"])
        assert len(pending_requests) == 1
        pending_request = pending_requests[0]
        request_payload = cast(dict[str, Any], pending_request["payload"])
        progress_payload = cast(dict[str, Any], request_payload["lost_wound_context"])
        logical_events = cast(list[dict[str, Any]], progress_payload["logical_death_events"])
        assert len(logical_events) == 1
        logical_payload = cast(dict[str, Any], logical_events[0]["payload"])
        model_instance_id = cast(str, logical_payload["model_instance_id"])
        ...
        binding = cast(dict[str, Any], progress_payload["logical_death_cause_binding"])
        binding["fixed_producer_id"] = producer_id
        event_log = cast(list[dict[str, Any]], decisions_payload["event_log"])
        canonical_event = next(
            event for event in event_log if event["event_id"] == logical_events[0]["event_id"]
        )
        canonical_event["payload"] = json.loads(json.dumps(logical_payload, sort_keys=True))
        request_event = next(
            event
            for event in event_log
            if event["event_type"] == "decision_requested"
            and cast(dict[str, Any], event["payload"])["request_id"]
            == pending_request["request_id"]
        )
        request_event["payload"] = json.loads(json.dumps(pending_request, sort_keys=True))

    rewritten_producer = _checkpoint_copy()
    _rewrite_pending_logical_death(
        rewritten_producer,
        game_id=state.game_id,
        producer_id="phase13d-coherently-rewritten-foreign-producer",
    )
    with pytest.raises(GameLifecycleError, match="start binding drift"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, rewritten_producer))

    rewritten_game = _checkpoint_copy()
    foreign_game_id = "phase13d-coherently-rewritten-foreign-game"
    _rewrite_pending_logical_death(
        rewritten_game,
        game_id=foreign_game_id,
        producer_id=progress.application_id,
    )
    rewritten_game_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], rewritten_game["decisions"])["event_log"],
    )
    rewritten_start = next(
        event
        for event in rewritten_game_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )
    cast(dict[str, Any], rewritten_start["payload"])["game_id"] = foreign_game_id
    with pytest.raises(GameLifecycleError, match="authority game drift"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, rewritten_game))

    rewritten_defender = _checkpoint_copy()
    rewritten_defender_decisions = cast(dict[str, Any], rewritten_defender["decisions"])
    rewritten_defender_request = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], rewritten_defender_decisions["queue"])["pending_requests"],
    )[0]
    rewritten_defender_request["actor_id"] = "player-a"
    rewritten_defender_progress = cast(
        dict[str, Any],
        cast(dict[str, Any], rewritten_defender_request["payload"])["lost_wound_context"],
    )
    rewritten_defender_progress["defender_player_id"] = "player-a"
    rewritten_defender_events = cast(
        list[dict[str, Any]], rewritten_defender_decisions["event_log"]
    )
    rewritten_defender_start = next(
        event
        for event in rewritten_defender_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )
    cast(dict[str, Any], rewritten_defender_start["payload"])["defender_player_id"] = "player-a"
    rewritten_defender_request_event = next(
        event
        for event in rewritten_defender_events
        if event["event_type"] == "decision_requested"
        and cast(dict[str, Any], event["payload"])["request_id"]
        == rewritten_defender_request["request_id"]
    )
    rewritten_defender_request_event["payload"] = json.loads(
        json.dumps(rewritten_defender_request, sort_keys=True)
    )
    with pytest.raises(GameLifecycleError, match="defender owner drift"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, rewritten_defender))

    foreign_priority = _checkpoint_copy()
    foreign_priority_decisions = cast(dict[str, Any], foreign_priority["decisions"])
    foreign_priority_request = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], foreign_priority_decisions["queue"])["pending_requests"],
    )[0]
    foreign_model_id = units["intercessor-1"].own_models[0].model_instance_id
    foreign_priority_progress = cast(
        dict[str, Any],
        cast(dict[str, Any], foreign_priority_request["payload"])["lost_wound_context"],
    )
    foreign_priority_progress["priority_model_ids"] = [foreign_model_id]
    foreign_priority_events = cast(list[dict[str, Any]], foreign_priority_decisions["event_log"])
    foreign_priority_start = next(
        event
        for event in foreign_priority_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )
    cast(dict[str, Any], foreign_priority_start["payload"])["priority_model_ids"] = [
        foreign_model_id
    ]
    foreign_priority_request_event = next(
        event
        for event in foreign_priority_events
        if event["event_type"] == "decision_requested"
        and cast(dict[str, Any], event["payload"])["request_id"]
        == foreign_priority_request["request_id"]
    )
    foreign_priority_request_event["payload"] = json.loads(
        json.dumps(foreign_priority_request, sort_keys=True)
    )
    with pytest.raises(GameLifecycleError, match="outside the target rules unit"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, foreign_priority))

    restored_state = _state(restored)
    restored_request = restored.decision_controller.queue.peek_next()
    result = DecisionResult.for_request(
        result_id="phase13d-multi-model-mortal-fnp-resumed",
        request=restored_request,
        selected_option_id=source_a.source_id,
    )
    restored.decision_controller.submit_result(result)
    resumed = resolve_mortal_wound_feel_no_pain_decision(
        state=restored_state,
        decisions=restored.decision_controller,
        request=restored_request,
        result=result,
        next_request_id="phase13d-multi-model-mortal-fnp-next",
        dice_manager=DiceRollManager(
            state.game_id,
            event_log=restored.decision_controller.event_log,
            injected_results=(
                _fixed_roll_result(
                    roll_id="phase13d-multi-model-mortal-fnp-roll",
    ...
    assert resumed.request is None
    completed_checkpoint = restored.to_payload()
    GameLifecycle.from_payload(completed_checkpoint)
    completed_without_start = cast(
        dict[str, Any],
        json.loads(json.dumps(completed_checkpoint, sort_keys=True)),
    )
    completed_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], completed_without_start["decisions"])["event_log"],
    )
    next(
        event
    ...
    with pytest.raises(GameLifecycleError, match="terminal lacks exactly one start authority"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, completed_without_start))

    completed_start_after_logical_death = cast(
        dict[str, Any],
        json.loads(json.dumps(completed_checkpoint, sort_keys=True)),
    )
    reordered_completed_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], completed_start_after_logical_death["decisions"])["event_log"],
    )
    reordered_completed_start = next(
        event
        for event in reordered_completed_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
    )
    reordered_completed_logical = next(
        event
        for event in reordered_completed_events
        if event["event_type"] == MODEL_LOGICAL_DEATH_RECORDED_EVENT
        and cast(dict[str, Any], event["payload"])["model_instance_id"]
    ...
    (
        reordered_completed_start["event_type"],
        reordered_completed_logical["event_type"],
    ) = (
        reordered_completed_logical["event_type"],
        reordered_completed_start["event_type"],
    )
    reordered_completed_start["payload"], reordered_completed_logical["payload"] = (
        reordered_completed_logical["payload"],
        reordered_completed_start["payload"],
    )
    reordered_completed_authorities = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], completed_start_after_logical_death["state"])[
            "model_destruction_cause_authorities"
        ],
    )
    reordered_completed_authority = next(
        authority
        for authority in reordered_completed_authorities
        if authority["model_instance_id"] == first_model.model_instance_id
    )
    reordered_completed_authority["logical_death_event"] = json.loads(
        json.dumps(reordered_completed_start, sort_keys=True)
    )
    with pytest.raises(
        GameLifecycleError,
        match="start must precede completed logical death",
    ):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, completed_start_after_logical_death))
    assert (
        sum(
            event.event_type == MORTAL_WOUND_APPLICATION_STARTED_EVENT
...
def test_phase13d_mortal_wound_routing_rejects_invalid_lifecycle_edges() -> None:
    ...
    no_models_checkpoint = cast(
        GameLifecyclePayload,
        json.loads(json.dumps(lifecycle.to_payload(), sort_keys=True)),
    )
    GameLifecycle.from_payload(no_models_checkpoint)

    foreign_source_checkpoint = cast(
        dict[str, Any],
        json.loads(json.dumps(no_models_checkpoint, sort_keys=True)),
    )
    foreign_source_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], foreign_source_checkpoint["decisions"])["event_log"],
    )
    foreign_source_start = next(
        event
        for event in foreign_source_events
        if event["event_type"] == MORTAL_WOUND_APPLICATION_STARTED_EVENT
        and cast(dict[str, Any], event["payload"])["application_id"]
        == no_models_progress.application_id
    )
    foreign_source_terminal = next(
        event
        for event in foreign_source_events
        if event["event_type"] == MORTAL_WOUND_MODEL_DESTRUCTIONS_FINALIZED_EVENT
        and cast(dict[str, Any], event["payload"])["application_id"]
    ...
    for event in (foreign_source_start, foreign_source_terminal):
        event_payload = cast(dict[str, Any], event["payload"])
        evidence_payload = cast(dict[str, Any], event_payload["destruction_evidence"])
        attribution_payload = cast(dict[str, Any], evidence_payload["destruction_attribution"])
        attribution_payload["destroying_player_id"] = "foreign-player"
    with pytest.raises(GameLifecycleError, match="source player is not in the game"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, foreign_source_checkpoint))

    invented_inventory_checkpoint = cast(
        dict[str, Any],
        json.loads(json.dumps(no_models_checkpoint, sort_keys=True)),
    )
    invented_inventory_events = cast(
        list[dict[str, Any]],
        cast(dict[str, Any], invented_inventory_checkpoint["decisions"])["event_log"],
    )
    invented_terminal = next(
        event
        for event in invented_inventory_events
        if event["event_type"] == MORTAL_WOUND_MODEL_DESTRUCTIONS_FINALIZED_EVENT
        and cast(dict[str, Any], event["payload"])["application_id"]
        == no_models_progress.application_id
    )
    invented_terminal_payload = cast(dict[str, Any], invented_terminal["payload"])
    invented_terminal_payload["destroyed_model_instance_ids"] = ["invented-model"]
    invented_terminal_payload["model_destroyed_event_ids"] = ["event-999999"]
    invented_terminal_payload["physical_unit_instance_ids"] = [defender.unit_instance_id]
    invented_terminal_payload["rules_unit_instance_ids"] = [defender.unit_instance_id]
    with pytest.raises(GameLifecycleError, match="destroyed application inventory drift"):
        GameLifecycle.from_payload(cast(GameLifecyclePayload, invented_inventory_checkpoint))
