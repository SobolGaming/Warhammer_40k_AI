...
def test_phase18m_package_pins_generation_typecheck_and_live_conformance() -> None:
    ...
    assert 'export type { operations } from "./openapi.operations.js";' in generated
    assert "export interface operations {" not in generated
    operations = (CLIENT_ROOT / "src" / "generated" / "openapi.operations.ts").read_text(
        encoding="utf-8"
    )
    assert 'import type { components } from "./openapi.js";' in operations
    assert "export interface operations {" in operations


def test_phase18m_conformance_version_matches_published_contract() -> None:
    contract = (CLIENT_ROOT / "src" / "contract.ts").read_text(encoding="utf-8")
    version_match = re.search(
        r'^export const EXTERNAL_CONTRACT_VERSION = "([^"]+)";$',
        contract,
        flags=re.MULTILINE,
    )
    assert version_match is not None

    openapi = json.loads((REPO_ROOT / "contracts" / "openapi.yaml").read_text(encoding="utf-8"))
    manifest = json.loads((REPO_ROOT / "contracts" / "manifest.json").read_text(encoding="utf-8"))

    assert version_match.group(1) == openapi["info"]["version"]
    assert version_match.group(1) == manifest["contract_version"]
