...
CORE_ABILITIES_SOURCE_PACKAGE = EDITION_SOURCE_PACKAGES / "core_abilities_2026_09"
...
_EDITION_SOURCE_PACKAGE_CLASSIFICATION = {
    "core_random_profiles_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_table_quarters_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_base_contact_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    ...
    "core_reserve_lifetimes_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_reserve_transport_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_embark_setup_turn_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_emergency_disembark_placement_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_empty_dedicated_transport_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_failed_save_damage_timing_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_measuring_to_destroyed_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_large_model_disembark_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_large_model_setup_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_super_heavy_walker_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_surge_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ...
    "core_lethal_hits_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_critical_hits_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_target_replacement_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_sequencing_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_actions_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_indirect_shooting_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_modifiers_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    "core_embarked_abilities_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
    ...
    "core_roster_construction_2026_09": (
        "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    ),
...
def test_core_command_phase_source_uses_one_typed_json_artifact() -> None:
    python_modules = tuple(
        sorted(path.name for path in CORE_COMMAND_PHASE_SOURCE_PACKAGE.glob("*.py"))
    )
    json_artifacts = tuple(
        sorted(
            path.name for path in (CORE_COMMAND_PHASE_SOURCE_PACKAGE / "artifacts").glob("*.json")
        )
    )

    assert python_modules == ("__init__.py", "_artifacts.py")
    assert json_artifacts == ("package.json",)
    assert _line_count(CORE_COMMAND_PHASE_SOURCE_PACKAGE / "_artifacts.py") < 1500


def test_core_command_phase_source_artifact_is_current() -> None:
    completed = subprocess.run(
        (sys.executable, "tools/build_core_command_phase_source.py", "--check"),
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
...
def test_core_movement_phase_source_uses_one_current_typed_json_artifact() -> None:
    python_modules = tuple(
        sorted(path.name for path in CORE_MOVEMENT_PHASE_SOURCE_PACKAGE.glob("*.py"))
    )
    json_artifacts = tuple(
        sorted(
            path.name for path in (CORE_MOVEMENT_PHASE_SOURCE_PACKAGE / "artifacts").glob("*.json")
        )
    )
    completed = subprocess.run(
        (sys.executable, "tools/build_core_movement_phase_source.py", "--check"),
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
...
def test_core_attached_units_source_uses_one_current_typed_json_artifact() -> None:
    python_modules = tuple(
        sorted(path.name for path in CORE_ATTACHED_UNITS_SOURCE_PACKAGE.glob("*.py"))
    )
    json_artifacts = tuple(
        sorted(
            path.name for path in (CORE_ATTACHED_UNITS_SOURCE_PACKAGE / "artifacts").glob("*.json")
        )
    )
    completed = subprocess.run(
        (sys.executable, "tools/build_core_attached_units_source.py", "--check"),
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    assert python_modules == ("__init__.py", "_artifacts.py")
    assert json_artifacts == ("package.json",)
    assert _line_count(CORE_ATTACHED_UNITS_SOURCE_PACKAGE / "_artifacts.py") < 1500
    assert completed.returncode == 0, completed.stderr


def test_core_abilities_source_uses_one_current_typed_json_artifact() -> None:
    python_modules = tuple(sorted(path.name for path in CORE_ABILITIES_SOURCE_PACKAGE.glob("*.py")))
    json_artifacts = tuple(
        sorted(path.name for path in (CORE_ABILITIES_SOURCE_PACKAGE / "artifacts").glob("*.json"))
    )
    completed = subprocess.run(
        (sys.executable, "tools/build_core_abilities_source.py", "--check"),
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    assert python_modules == ("__init__.py", "_artifacts.py")
    assert json_artifacts == ("package.json",)
    assert _line_count(CORE_ABILITIES_SOURCE_PACKAGE / "_artifacts.py") < 1500
    assert completed.returncode == 0, completed.stderr


def test_core_attack_sequence_source_uses_one_current_typed_json_artifact() -> None:
    python_modules = tuple(
        sorted(path.name for path in CORE_ATTACK_SEQUENCE_SOURCE_PACKAGE.glob("*.py"))
    )
    json_artifacts = tuple(
        sorted(
            path.name for path in (CORE_ATTACK_SEQUENCE_SOURCE_PACKAGE / "artifacts").glob("*.json")
        )
    )
    completed = subprocess.run(
        (sys.executable, "tools/build_core_attack_sequence_source.py", "--check"),
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
    ...
    assert python_modules == ("__init__.py", "_artifacts.py")
    assert json_artifacts == ("package.json",)
    assert _line_count(CORE_ATTACK_SEQUENCE_SOURCE_PACKAGE / "_artifacts.py") < 1500
    assert completed.returncode == 0, completed.stderr
...
def test_core_rules_app_mirror_sources_use_project_authority_not_official_capture() -> None:
    ...
    assert (
        _EDITION_SOURCE_PACKAGE_CLASSIFICATION["core_abilities_2026_09"]
        == "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    )
    assert (
        _EDITION_SOURCE_PACKAGE_CLASSIFICATION["core_attached_units_2026_09"]
        == "project_reviewed_transcription_with_project_authoritative_app_mirror_source"
    )
    assert (
