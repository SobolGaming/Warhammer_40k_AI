...
def test_official_gw_faction_pack_manifest_uses_tracked_pdf_policy() -> None:
    ...
    assert all("tracked as current source evidence" in entry.license_note for entry in entries)


def test_official_gw_predecessor_faction_pack_manifest_retains_historical_evidence() -> None:
    ...
    assert all(entry.package_id.endswith("-2026-06") for entry in entries)
    assert all(entry.package_id != "gw-11e-deathwatch-faction-pack-2026-06" for entry in entries)
    assert all(entry.local_cache_path is not None for entry in entries)
    assert all(
        PurePosixPath(entry.local_cache_path or "").match("data/raw/faction_packs/*.pdf")
        for entry in entries
